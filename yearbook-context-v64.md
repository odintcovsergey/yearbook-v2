...
# Версия: v64
# Обновлено: 16.05.2026 (РЭ.8 закрыт — seed --write UPSERTит правила и пресеты)
#
...
# Подэтапы РЭ.1+РЭ.2+РЭ.3+РЭ.3.5+РЭ.4+РЭ.5+РЭ.6+РЭ.7+РЭ.8 закодированы.
...
# ✅ РЭ.8 (cd0599b): scripts/seed-rule-engine.ts --write теперь UPSERTит
#    три сущности по очереди (порядок диктуется FK):
#      template_families → rules → presets
#    Каждый UPSERT — {onConflict: 'id', count: 'exact'}.
#    Поле rules.rule_json хранит весь объект правила целиком (так
#    buildFromRules работает с одним полем вместо склейки колонок и JSON).
#    Поле rules.tenant_id = null для всех правил из репо (глобальные).
#    После seed --write в БД будет: 7 families + 36 rules + 7 presets.
#    Запуск seed --write на боевой Supabase Сергей делает сам.
# Состав 36 правил:
...
# Следующий шаг: РЭ.9 (алгоритм buildFromRules — большая работа,
...
#    Подэтапы РЭ.1-РЭ.8 ✅ выполнены, РЭ.9-РЭ.13 запланированы.
#
