...
# Версия: v144
# Обновлено: 21.05.2026 (Б.1 закрыт — SQL миграция глобальных пресетов
# на семантическую модель закоммичена, ждёт применения в Supabase).
#
# ⏳ ФАЗА РЭ.30 ИДЁТ — чистка путей сборки альбома (1/7 коммитов)
# ──────────────────────────────────────────────────────────────────
# Spec: docs/phase-Р30-spec.md (283 строки, commit 8d5b996)
# Диагностика: docs/phase-Р30-diagnostic.md (commit fe3ce67)
# Следующий шаг: Б.2 (fix PresetEditorModal — не записывать
# density/sheet_type при сохранении пресета).
#
# ⚠️ ДЕЙСТВИЕ ОТ СЕРГЕЯ ПЕРЕД Б.2 (или параллельно):
# Применить миграцию `migrations/2026-05-21-presets-to-semantic.sql`
# в Supabase SQL Editor. Идемпотентно — повторное применение
# безопасно. Проверка после применения:
#   SELECT id, density, student_layout_mode, student_grid_size,
#          student_friend_photos, student_has_quote, template_set_id
#   FROM presets WHERE tenant_id IS NULL ORDER BY id;
# Ожидание: density=NULL у всех 7, student_layout_mode заполнен,
# template_set_id = 08baf556-7831-44e9-9ba8-4af20f19ee44.
#
# Цель фазы:
#   Партнёр в /app/templates/[designId] создаёт шаблон через
#   единственный путь — Режим (page/spread/grid) + параметры.
#   Без винегрета: убираем «Плотность портретов», селект
#   Section Structure, блок Комплектация+Тип печати.
#
# Что доказала диагностика 21.05.2026:
#   • Движок buildFromSectionStructure рабочий
#   • Семантический поиск мастеров (findTeacherMaster,
#     findStudentMaster) работает
#   • После UPDATE'ов глобальных пресетов на семантику — все 7
#     режимов собирают альбомы (standard, universal, maximum,
#     individual, medium, light, mini-soft)
#   • Проблема НЕ в коде engine. Проблема в данных и UI.
#
# Состояние данных после Б.1 (после применения миграции):
#   • Все 7 глобальных пресетов на семантической модели
...
#   • 2 партнёрских пресета Сергея ВСЁ ЕЩЁ смешанные (density +
#     layout_mode) — чистка будет в Б.3
#
# Прогресс по подэтапам РЭ.30:
#   ✅ Б.1 — SQL миграция глобальных пресетов (commit 2992821)
#   ⏳ Б.2 — fix PresetEditorModal (не сохранять density/sheet_type)
#   ⏳ Б.3 — SQL миграция: cleanup смешанных пресетов Сергея
#   ⏳ В.1 — убрать селект Section Structure
#   ⏳ В.2 — убрать поле «Плотность портретов» из редактора
#   ⏳ В.3 — убрать блок «Комплектация + Тип печати» из формы альбома
#   ⏳ Г.1 — summary + закрытие фазы
#
# Эстимация: ~1 сессия, 7 коммитов, 2 SQL миграции.
#
