...
# Версия: v129
# Обновлено: 21.05.2026 (после РЭ.27.2 — API даёт явный print_type).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0..27.2 закрыты. Следующий — 27.3 (engine: resolvePrintType
# + правила first/last + фильтр spread-мастера + unit-тесты).
# Spec: docs/phase-Р27-spec.md v1.0 (с §2.0 после открытия).
# Последний commit фазы: 3096c18 (РЭ.27.2).
#
# ✅ Миграция РЭ.27.1 применена в Supabase 21.05.2026.
#
# Что готово после 27.2:
#   • БД: albums.print_type существует с 8 мая 2026, CHECK constraint
#     ('layflat'|'soft'), индекс idx_albums_print_type (создан в 27.1).
#   • API create_album:
#     - body.print_type явно передан и валиден → приоритет (партнёр
#       может создать альбом с типом листов, отличным от пресета).
#     - Иначе → копирование из preset.print_type (текущее поведение).
#     - Иначе → null (для engine fallback в 27.3).
#   • API update_album:
#     - body.print_type принимается через allowedFields.
#     - При смене section_structure_preset_id: подтягиваем print_type
#       из нового пресета ТОЛЬКО если body.print_type не передан.
#       Если передан явно — приоритет за ним.
#     - Явная валидация ('layflat'|'soft' иначе 400).
#   • types/index.ts: PrintType type + Album.print_type? поле.
#
# Что ещё НЕ работает:
#   • Engine ВСЁ ЕЩЁ читает print_type из ПРЕСЕТА. resolvePrintType
#     ещё не существует. Сборка альбомов идёт по preset.print_type
#     независимо от того что в albums.print_type. Это работа 27.3.
#   • Правила first/last для soft (форзац-страницы) не применяются.
#   • Spread-мастера НЕ блокируются для soft.
#   • Превью SVG без водяного знака «Форзац».
#   • UI каталога мастеров без фильтра.
#   • UI формы альбома без явного селекта «Тип листов» (поле есть
#     в типах но не отображается отдельным контролом).
#   • 6 NULL-альбомов в проде не заполнены.
#   • Дубль-пресеты «layflat»/«soft» не слиты.
#
# Что зафиксировано в spec (7 архитектурных развилок утверждены):
#   A) Один разворотный редактор для всех (НЕ двойной).
#   B) print_type живёт в albums + fallback на preset через
#      resolvePrintType(album, preset).
#   C) Автозаглушка «Форзац» — водяной знак на первом левом
#      и последнем правом развороте для soft.
#   D) Миграция print_type для существующих альбомов — автоматически
#      из preset.print_type (это подэтап 27.7).
#   E) Фильтр spread-мастера для soft — защита в ДВУХ местах
#      (engine + UI).
#   F) Слияние дубль-пресетов «layflat»/«soft» в один (Вариант A).
#   G) Подтверждение в UI при смене типа листов если уже выбраны
#      spread-мастера.
#
...
#   ⏳ 27.3 — engine: resolvePrintType + правила first/last + фильтр
#             spread-мастера + unit-тесты
#   ⏳ 27.4 — превью SVG: водяной знак «Форзац» для soft
...
#
# Тесты на main: 407/407 passing (стабильно с РЭ.25.3).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v129
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v129 — РЭ.27.0..27.2 закрыты. Фаза в работе.
#      • docs/phase-Р27-spec.md §3 «Engine: правила сборки» —
#        главный раздел для 27.3.
#      • docs/phase-Р25-summary.md — образец как делали фильтр в РЭ.25
#        (паттерн чистой функции в lib/ + unit-тесты).
#   4. Применены в Supabase: все РЭ.22..РЭ.25.1 + РЭ.27.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.3 (engine):
#      Это самый содержательный подэтап фазы. Три части:
#
#      a) resolvePrintType(album, preset) — чистая функция:
#         - album.print_type приоритетнее
#         - fallback на preset.print_type
#         - если оба null/undefined → 'layflat' (бэк-default)
#         - вынести в отдельный модуль lib/album-builder/print-type-resolver.ts
#           (чистая функция без зависимостей, тестируется напрямую —
#           паттерн как с filter-by-purchase из РЭ.25.3).
#         - встроить в buildAlbum / buildFromSectionStructure
#
#      b) Правила first/last для soft:
#         - первый разворот: левая = синтетическая «Форзац», правая = первый
#           контентный мастер
#         - последний разворот: левая = последний контентный мастер,
#           правая = синтетическая «Форзац»
#         - для layflat правила не применяются
#         - Нужен новый тип элемента в layout (EndpaperPlaceholder):
#             { type: 'endpaper', side: 'first_left'|'last_right', print_type: 'soft' }
#         - Реальная отрисовка водяного знака — в 27.4 (превью SVG).
#
#      c) Фильтр spread-мастера для soft:
#         - При findMaster: если print_type=soft и мастер имеет
#           «spread»-роль → пропуск + warning 'spread_master_skipped_for_soft'
#         - Тесты: для layflat ничего не меняется, для soft spread-мастера
#           не выбираются.
#
#      Unit-тесты (минимум 8-10):
#         - resolvePrintType: album → возвращает; null → fallback на пресет;
#           оба null → layflat
#         - applyEndpaperRules: для soft вставляет заглушки; для layflat нет
#         - isSpreadMasterAllowedForPrintType: для soft false на spread;
#           для layflat true для всех
#
#      Существующие 407 тестов engine ДОЛЖНЫ продолжать проходить.
#      Дальше — 27.4 (превью SVG), 27.5..27.8.
#      Все детали в docs/phase-Р27-spec.md §3 и §6.
