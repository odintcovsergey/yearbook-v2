...
# Версия: v92
# Обновлено: 20.05.2026 (после РЭ.22.1 — миграция БД для двух-осевой модели).
#
...
#    constraint'а через DO-блоки. Старые student_pages_per_student/
#    friend_photos/has_quote остаются как deprecated.
#    ⬜ Применить в Supabase SQL Editor:
#       migrations/2026-05-20-presets-student-layout-mode.sql
#
# ⏳ Следующий подэтап: РЭ.22.2 — типы + API валидация.
#    - Расширить interface Preset в PresetEditorModal.tsx и/или в
#      lib/rule-engine/types.ts (student_layout_mode, student_grid_size).
#    - Расширить SELECT в action='rule_presets_list' в app/api/tenant/route.ts
#      (добавить student_layout_mode, student_grid_size в выборку).
#    - Добавить валидацию body в action='rule_preset_update' (новые
#      поля по образцу student_pages_per_student в РЭ.21.8.15).
#    - UI и engine в этом подэтапе НЕ ТРОГАЕМ — только типы и API.
...
#      с двумя CHECK через DO-блоки.
#      ⬜ Применить в Supabase SQL Editor (партнёрская задача Сергея).
#
#   ⏳ Следующий подэтап: РЭ.22.2 — типы + API валидация
#      Расширить Preset type, SELECT в rule_presets_list и валидацию
#      в rule_preset_update. UI и engine не трогаем.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v92
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v92 — что закрыто (РЭ.22.0 + РЭ.22.1)
#      • docs/phase-Р22-spec.md v1.0 (564 строки) — ПОЛНЫЙ план фазы
#        с зафиксированными развилками. Основной источник истины для
#        всех 10 подэтапов РЭ.22.
#      • migrations/2026-05-20-presets-student-layout-mode.sql — что
#        добавили в БД (две колонки + два CHECK через DO-блоки).
#      • lib/rule-engine/sections/students.ts — текущая реализация
#        (buildAlternatingLR + buildOnePerSpread + buildOnePerSpreadAdaptive
#        + buildGrid). РЭ.22 переведёт всё это на семантический поиск.
#      • lib/rule-engine/master-finder.ts — задел из РЭ.21.8.15, который
#        будет расширен в РЭ.22.6 (снятие ограничения studentsCap===1).
#      • app/super/presets/_components/PresetEditorModal.tsx — текущий UI.
#        В РЭ.22.3 он переезжает на двух-осевую модель.
#   4. Применены в Supabase:
#      • Все миграции до v87 ✅
#      • 2026-05-19-presets-student-layout-fields.sql ✅ (РЭ.21.8.15)
...
#        ПРИМЕНИТЬ ДО деплоя РЭ.22.2-РЭ.22.3 (правило ADD COLUMN:
#        сначала SQL → потом код, см. правила безопасных миграций).
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.2 — типы + API валидация.
#      Файлы (которые трогаем):
#        - app/super/presets/_components/PresetEditorModal.tsx —
#          interface Preset, добавить student_layout_mode и
#          student_grid_size (опциональные, типы из spec §3).
#        - app/api/tenant/route.ts — расширить SELECT в action=
#          'rule_presets_list' (добавить новые колонки в выборку),
#          добавить валидацию body в action='rule_preset_update'
#          (по образцу student_pages_per_student).
#        - lib/rule-engine/types.ts — если есть PresetSnapshot,
#          расширить аналогично.
#      Что НЕ трогаем в этом подэтапе:
#        - UI рендер (PresetEditorModal.tsx не меняем визуально, только тип)
#        - engine — buildAlternatingLR/buildGrid/buildOnePerSpread/
#          buildOnePerSpreadAdaptive не трогаем
#      tsc + next build зелёные. Один атомарный коммит.
#      Дальше идём по плану spec'а РЭ.22.3..РЭ.22.10.
