...
# Версия: v87
# Обновлено: 19.05.2026 (РЭ.21.8 закрыта на 8 коммитов из 12 планируемых:
#   21.8.1..21.8.7c — новый build engine buildFromSectionStructure
#   подключен к боевому handleBuildAlbum через albums.section_structure_preset_id.
#   21.8.8 — autopack режим общего раздела + реальные bindings.
#   Осталось РЭ.21.8.9..12 — таблица автоверстки OkeyBook для общего
#   раздела + переходная страница из личного в общий.
#   Sandbox-проверка на 7 пресетах прошла успешно (РЭ.21.8.6 + 21.8.6a).)
#
# ⚠️ ВАЖНО: ПРЕЖДЕ ЧЕМ ПРИНИМАТЬСЯ ЗА ЛЮБУЮ ДРУГУЮ РАБОТУ — ПРОЧИТАЙ:
#   docs/phase-Р20-spec.md                — ✅ ТЗ РЭ.20 (18.05.2026, главное)
...
#     OkeyBook автоверстки общего раздела (Сергей, 19.05.2026):
#     обязательный + дополнительный разделы, переходная страница,
#     5 комплектаций × плотные/мягкие × интервалы количества учеников.
#     См. docs/okeybook/README.md. Будет зашита в код в РЭ.21.8.9..12.
...
#   ✅ Контент-редактор фото (scale + offset)
...
#
# Section Structure engine end-to-end в проде (РЭ.21.8.1..8, май 2026):
...
#      сбоях.
#   ✅ 314 unit tests passing — больше всего покрытие в проекте.
...
#   ✅ Manual режим: { type: 'common', slots: [H, Q, flex_A, ...] }.
#   ✅ Auto режим: { type: 'common', mode: 'auto', max_spreads: N }
#      жадный по крупности (full → half → quarter → sixth), принцип
#      «лучше меньше разворотов, чем пустые слоты».
#
#   ⚠️ Что не закрыто (РЭ.21.8.9..12 — план ниже):
#   - Общий раздел не верстается по эталонной таблице OkeyBook.
#     Сейчас autopack — простая жадная стратегия. Партнёр-фотограф
#     не получает разнообразный общий раздел (где 1/4 на первых страницах,
#     потом 1/2, потом «или 6×1/6, или 2×1/2, или 1 общая»).
#   - Нет переходной страницы между личным и общим разделом. Сейчас если
#     учеников не кратно слотам сетки — последняя страница доделывается
#     null-падингом. По таблице нужно «до N фото учеников + снизу
#     1 общая» или несколько фото 1/2 / 1/6 в стиле сетки.
#   - Нет разделения общего раздела на «обязательный» (входит в стоимость)
#     и «дополнительный» (продаём как платную допуслугу для увеличения
#     конверсии). Бизнес-логика OkeyBook это требует.
#
...
#
# ════════════════════════════════════════════════════════════════════════
# План РЭ.21.8.9..12 — таблица автоверстки OkeyBook (НЕ СДЕЛАНО)
# ════════════════════════════════════════════════════════════════════════
#
# Контекст: 19.05.2026 Сергей подсветил что autopack режим из РЭ.21.8.8
# работает но это «полуфабрикат» — продукт OkeyBook задумывался как «нажал
# кнопку, получил 80-90% готового результата», а сейчас партнёр получает
# базовую сборку и должен доделывать руками. Решение: запрограммировать
# в Section Structure engine эталонный алгоритм OkeyBook автоверстки
# общего раздела по таблице (xlsx-файл от Сергея, 19.05.2026).
#
# Таблица описывает на пересечении (комплектация × тип листов × кол-во
# учеников):
# - что находится на ПОСЛЕДНЕЙ странице личного раздела (переходная
#   страница). Например для Лайт плотных 13-15 учеников: левая «до 3
#   фото учеников + снизу 1 общая», правая «в приоритете 2 фото 1/2
#   класса, либо 6 фото 1/6, либо 1 общая».
# - что находится в ОБЯЗАТЕЛЬНОМ общем разделе (входит в стоимость
#   альбома, всегда). Жёсткая последовательность 1-6 страниц с возможными
#   «или-или» выборами на конкретных страницах.
# - что находится в ДОПОЛНИТЕЛЬНОМ общем разделе (платная допуслуга
#   OkeyBook для увеличения конверсии — родители докупают развороты).
#   1-5 страниц, тоже с «или-или» выборами.
#
# Пример «или-или»: «либо 6 фото 1/6, либо 2 по 1/2 класса, либо 1
# общая» = engine жадно по крупности выбирает что есть в пуле фото
# партнёра. Загрузил 6 коллажных → J-Collage-6. Не загрузил → 2 half →
# J-Half. И т.д.
#
# Решения Сергея (19.05.2026):
# - Это ВНУТРЕННИЙ стандарт OkeyBook, не отраслевой. Партнёры могут
#   хотеть другое, но «потом разберёмся с реальными партнёрами».
# - Дополнительный общий раздел внешне НЕ отличается от обязательного —
#   просто другие строки в таблице. Разница только в продаже
#   (входит/не входит в стоимость альбома).
# - Переходная страница КРИТИЧНА — без неё личный раздел не стыкуется
#   с общим и верстка нерабочая. Делать сейчас, не откладывать.
# - Для Индивидуальной комплектации (которой нет в таблице) использовать
#   логику Максимум, но это разовое решение — у партнёров может быть
#   другая структура.
#
# Подэтапы:
#
# ⬜ РЭ.21.8.9 — Обязательный общий раздел по таблице OkeyBook
#    Новая секция в section_structure: { type: 'common_required' }.
#    Engine читает таблицу (константа в коде, ~150 строк JSON), смотрит
#    на preset.density × sheet_type × input.students.length, находит
#    нужную строку, выкладывает страницы. Логика «или-или» на странице:
#    жадно по крупности из того что есть в пуле. ~250 строк + 8-10 тестов.
#
# ⬜ РЭ.21.8.10 — Дополнительный общий раздел
#    Новая секция: { type: 'common_additional', max_spreads: N }. Использует
#    таблицу, колонки «дополнительный». ВНЕШНЕ страницы дополнительного
#    раздела НЕ отличаются от обязательного — это просто строки таблицы
#    которые добавляются если партнёр продал родителю допуслугу
#    «больше общих разворотов в альбоме». Бизнес-цель: верстать ВСЕГДА
#    (даже если не куплено), чтобы партнёр мог показать готовое превью
#    родителям и продать конверсию. Партнёр в редакторе альбома выставляет
#    common_section_max_spreads — это N. Если 0 — секция не строится.
#    ~150 строк + 5 тестов.
#
# ⬜ РЭ.21.8.11 — Переходная страница в students секции
#    Последняя страница(ы) личного раздела доделывается общими фото
#    согласно таблице. Например для Лайт плотных 13-15 учеников
#    последняя левая = «до 3 фото учеников + снизу 1 общая». Принцип
#    «вариации комбо-мастеров» (Сергей, 19.05.2026): если предпочтительная
#    комбинация недоступна (например партнёр не загрузил фото 1/2 класса),
#    engine выбирает альтернативный комбо-мастер с другим набором фото —
#    «5 фотографий», «4 вертикальные», «6 коллажных 1/6» и т.д.
#    Сергей-дизайнер готов прописать все нужные комбинации и проставить
#    labels на placeholder'ах. Технически:
#    - Каждая «вариация» = отдельный мастер с уникальным именем
#      (E-Light-Transition-3stud-1full, E-Light-Transition-3stud-6sixth, ...)
#    - В sections/students.ts появляется выбор мастера для последней
#      страницы по комбинации (density × students.length % slots × sheet_type)
#      → пытаемся каждую вариацию по приоритету, останавливаемся на
#      первой подходящей по наличию фото в пуле.
#    - Сейчас фолбэк через *-Combined-Page (M/L/N) даёт частичное
#      покрытие — есть в template_set okeybook-default. Использовать
#      как временный фолбэк пока новые мастера не нарисованы.
#    ~200 строк + 8 тестов.
#
# ⬜ РЭ.21.8.12 — UI редактора пресета и альбома
#    В редакторе пресета (РЭ.21.3-7) убрать кнопки выбора слотов H/Q/flex_*
#    для common — заменить на радио-кнопки «по таблице OkeyBook /
#    вручную / автоподбор». Для каждого радио свой UI:
#    - «По таблице» → секции common_required + common_additional
#      автоматически.
#    - «Вручную» → текущие slots[].
#    - «Автоподбор» → mode: 'auto' + поле max_spreads.
#    В редакторе альбома переименовать «РАЗВОРОТОВ В ОБЩЕМ РАЗДЕЛЕ»
#    → «Дополнительных разворотов общего раздела» (это N для
#    common_additional). ~200 строк.
#
# Замечание про Индивидуальную комплектацию (Сергей, 19.05.2026):
#    В таблице её нет. По верстке = Максимум (один ученик на разворот,
#    справа N фото с друзьями), но разворот может быть разным —
#    3, 4, 5, 6 фото с друзьями в разных раскладках. Эта вариативность
#    — другая задача (не РЭ.21.8.11): нужно дать партнёру выбор раскладки
#    личного разворота на уровне пресета. Сергей готов нарисовать
#    все вариации мастеров. После общего раздела индивидуальной
#    комплектации идёт сетка портретов (12 на страницу), потом общий
#    раздел как у Максимум.
#    КРИТИЧНО: индивидуальная сохраняется как N альбомов (по 1 на ученика),
#    каждый содержит ТОЛЬКО свой личный разворот + общую часть. Это
#    бизнес-задача про экспорт, не про сборку — отдельная фаза (не в 21.8).
#
# Альтернатива: после РЭ.21.8.9+10 можно остановиться и собирать обратную
# связь от реальных партнёров. Переходная страница и UI селектор —
# отдельные большие задачи, имеет смысл сделать паузу перед ними.
#
# Файлы которые точно затронем в 21.8.9..12:
# - lib/rule-engine/sections/common-required.ts (новый)
# - lib/rule-engine/sections/common-additional.ts (новый)
# - lib/rule-engine/sections/students.ts (расширение buildGrid /
#   buildAlternatingLR для transition page)
# - lib/rule-engine/album-structure-okeybook.ts (новый, константа
#   с таблицей)
# - lib/rule-engine/types.ts (новые type'ы секций)
# - app/api/tenant/route.ts (валидатор)
# - app/api/layout/route.ts (handleBuildAlbum остаётся без изменений)
# - app/app/page.tsx (UI редактор пресета и альбома) — для РЭ.21.8.12
#
# Уроки sandbox-тестирования (19.05.2026):
# - Реальные имена placeholder labels в okeybook-default отличаются от
#   ожидаемых. classphotoframe / halfphoto_N / collagephoto_N подтверждены
#   через скриншоты template_set. Если будущий мастер придёт с другими
#   labels — placeholder-driven mapping не упадёт, просто оставит
#   bindings пустыми.
# - Standard density технически = Universal density в template_set. Это
#   архитектурное упрощение — раньше я ожидал двухстраничный E-Student-
#   Standard, в реальности один и тот же двухстраничный паттерн через
#   два одностраничных мастера.
# - Master J-Spread не существует в template_set. Категория spread в
#   common_photos сейчас бесполезна — фото есть, но мастера нет. Партнёр
#   вставит вручную через TemplatePickerModal. Дизайнер должен нарисовать
#   J-Spread (master-cleanup-tz §A5).
...
#
# 🎯 ЧТО ДАЛЬШЕ — ПЛАН ДО АВГУСТА 2026 (запуск партнёрки)
#
# Все главные технические блокеры закрыты в проде:
#   ✅ Rule engine + Content-editor + Balance — работают end-to-end
#
# Главное что осталось — реализация ДИЗАЙНЕРСКОЙ СПЕЦИФИКАЦИИ структуры
# альбома (полная матрица из xlsx) + UI редактор пресетов чтобы партнёры
# могли настраивать свои альбомы. Это **самое важное** до запуска.
#
# Оставшиеся работы (по приоритету):
#
#   1. РЭ.20 + РЭ.12 — ВЗАИМОЗАВИСИМЫ, реализуются вместе:
#
#      РЭ.20 (26-35 часов) — реализация матрицы из xlsx:
#        А) Подготовка: миграция БД (total_pages в presets) — 1 коммит
#        Б) Типы и контекст: Preset/RuleContext расширения — 2-3 коммита
#        В) Код-генератор правил из JSON-матрицы — 3-4 коммита
#        Г) Тестирование на реальных альбомах — 1-2 коммита
#        Д) Документация + контекст — 1 коммит
#
#      РЭ.12 (16-24 часа) — UI редактор пресетов /app/presets:
#        Партнёр копирует глобальный пресет → редактирует total_pages
#        + другие параметры → выбирает для своего альбома.
#        5-8 коммитов.
#
#      Порядок: РЭ.20.А + РЭ.20.Б → РЭ.12 → РЭ.20.В + РЭ.20.Г + РЭ.20.Д
#      Итого 40-60 часов = 2-3 недели работы.
#
#   2. РЭ.11 — фильтр TemplatePickerModal по family_id (1-2 коммита)
#      При замене мастера partner видит подходящие, не все.
#
#   3. Параллельно июнь — переезд с Vercel (1-2 коммита Dockerfile +
#      GitHub Actions deploy на Timeweb / YC App Platform). Без этого
#      okeybook.ru остаётся за VPN.
#
# 📍 КАК НАЧАТЬ СЛЕДУЮЩУЮ СЕССИЮ:
#   1. cd ~/yearbook-v2 && git pull
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v81
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • docs/album-structure-inventory.md — 10 разделов, картина системы.
#      • build_album.jsx (Сергей прислал) — функция pushCommonSlot,
#        строки 913-991, эталон логики слотов.
#      • Раздел РЭ.21 в этом файле — финальная архитектура.
#   4. Применены в Supabase:
#      • 2026-05-18-presets-total-pages-density-sheet-type.sql ✅
#      • 2026-05-18-presets-density-sheet-type-values.sql ✅
#      • 2026-05-18-rollback-mini-hard.sql ✅
#      • 2026-05-18-presets-section-structure.sql ✅ (РЭ.21.1)
#      • 2026-05-18-presets-section-structure-values.sql ✅ (РЭ.21.2)
#   5. UI пресетов работает в /app — кнопка «Пресеты», просмотр + создание.
#   6. СЛЕДУЮЩИЕ ШАГИ (в порядке приоритета, обсуждены 18.05):
#      a. min_pages/max_pages в presets вместо total_pages — диапазон.
#         Миграция + форма создания + просмотр.
#      b. Подключение template_set к пресету (presets.template_set_id).
#         Партнёр в форме выбирает «дизайн» из глобальных template_set.
#      c. Редактор структуры пресета — drag-and-drop секций + слотов
#         в общем разделе. После этого партнёр может реально менять
#         свои пресеты.
#      d. Подключение section_structure к build engine — engine начинает
#         читать поле при сборке. Это финальный шаг чтобы пресеты
#         реально работали.
...
#   7. Через 2с авто-сохранение (Save indicator показывает 'Сохранено')
#   8. На фото с кастомным crop появляется '⚙' бейдж в углу
#   9. Экспорт PDF — те же crop попадают в финальный файл
#  10. 'Сбросить' в panel — удаляет ключи __scale__/__offset__,
#      фото возвращается к default cover
#
