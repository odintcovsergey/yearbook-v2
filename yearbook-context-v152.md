...
# Версия: v152
# Обновлено: 23.05.2026 (вечер)
#
# ┌────────────────────────────────────────────────────────────────────┐
# │ ЧТО НОВОГО В V152 (23.05.2026 ВЕЧЕР)                               │
# │                                                                    │
...
# │                                                                    │
# │ ОБНАРУЖЕНЫ ВТОРОСТЕПЕННЫЕ БАГИ (НЕ ЗАКРЫТЫ, ОТЛОЖЕНЫ):             │
# │   1. Дублирование фото в common_required.J-Collage-4 (два соседних │
# │      J-Collage-4 имеют ОДИНАКОВЫЕ collagephoto_1..4). Причина:     │
# │      рассинхрон bindCommonPhotos (читает из input.common_photos.   │
# │      sixth) и decrementAvailable (декрементит quarter — потому     │
# │      что analyzeMasterCapability классифицирует J-Collage-4 как    │
# │      category='quarter'). Cursor sixthUsed не сдвигается.          │
...
# │   1. Сергей дорисовывает недостающие мастера в InDesign            │
# │      (Приоритет 1 из docs/master-catalog.md)                       │
# │   2. Тестируем белый дизайн на 1-2 реальных альбомах               │
# │   3. РЭ.37 — конструктор переходного раздела (отдельный чат)       │
# │   4. Симметризация хвоста students (отдельный чат)                 │
# │   5. Фикс дублирования фото в common_required (отдельный чат)      │
# │   6. UI warnings для партнёра                                      │
