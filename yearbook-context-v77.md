...
# Версия: v77
# Обновлено: 18.05.2026 (РЭ.20.6.1 — инфраструктура consumes.pages
#                       + mandatory_section.pages; rollback mini-hard;
#                       АРХИТЕКТУРНОЕ РЕШЕНИЕ: sheet_type — атомарная
#                       настройка, не отдельный пресет)
#
...
#      • Existing rules без новых полей работают как раньше.
#      • 200/200 тестов проходят, tsc и next build чистые.
#   ⏳ Дальше: РЭ.20.6.2 (первое правило с новыми consumes-полями +
#      integration-тест), РЭ.20.6.3 (генератор правил из матрицы),
#      РЭ.12 (UI пресетов + sheet_type на уровне альбома).
#      Осталось ~14-18 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v77
#   3. Открыть docs/phase-Р20-spec.md — план фазы.
...
#        (применить, если ещё не применена)
#      • 2026-05-18-rollback-mini-hard.sql ⏳
#        (применить, удаляет лишний пресет mini-hard)
#   5. Код: РЭ.20.2/3/4/5/6.1 готовы.
#   6. Следующий шаг — РЭ.20.6.2: первое правило с новыми consumes
#      (pages + mandatory_section.pages) + integration-тест.
#      Это докажет работоспособность инфраструктуры.
#
...
#   ✅ ROLLBACK mini-hard (127f50f, 18.05.2026) — лишний пресет удалён.
#   ✅ РЭ.20.6.1 (6be5ab5) — инфраструктура consumes.pages и
#      consumes.mandatory_section.pages.
#   1. РЭ.20.6.2 — первое правило-эталон + integration-тест.
#   2. РЭ.20.6.3 — генератор правил из матрицы (scripts/generate-rules-from-matrix.ts).
#   3. РЭ.20.6.4 — seed-rule-engine.ts грузит generated/ + деактивация
#      legacy common-section-*-pair для пресетов с density.
#   4. РЭ.20.7 — unit-тесты по каждой строке матрицы.
#   5. РЭ.12.A — UI /app/presets список + ALTER TABLE albums ADD sheet_type
#      (sheet_type теперь живёт у альбома, не у пресета).
#   6. РЭ.12.Б — UI редактор пресета (total_pages — главное! партнёр
#      выставляет под свой альбом).
