...
# Версия: v149
# Обновлено: 21.05.2026 (В.3 закрыт — блок «Комплектация + Тип печати»
# убран из формы альбома, осталась только Г.1).
#
# ⏳ ФАЗА РЭ.30 ПОЧТИ ЗАКРЫТА — чистка путей сборки альбома (6/7 коммитов)
# ──────────────────────────────────────────────────────────────────
# Spec: docs/phase-Р30-spec.md (283 строки, commit 8d5b996)
# Диагностика: docs/phase-Р30-diagnostic.md (commit fe3ce67)
# Следующий шаг: Г.1 (написать docs/phase-Р30-summary.md и
# закрыть фазу).
#
# ✅ Состояние данных после применения Б.1 + Б.3 в Supabase:
...
#   Смешанных пресетов нет (mixed_remaining=0).
#
# Цель фазы:
...
#   ✅ В.1 — убран селект Section Structure из обзора (commit 4455a8b)
#   ✅ В.2 — убраны поля «Плотность портретов» и «Тип листов» (commit 1bc8d76)
#   ✅ В.3 — убран блок «Комплектация + Тип печати» из формы (commit 74438d4)
#   ⏳ Г.1 — summary + закрытие фазы
#
# После В.3:
#   • Форма альбома: единственный путь задания сборки — виджет «Шаблон»
#     (section_structure_preset_id). Selectов «Комплектация» и «Тип печати»
#     больше нет.
#   • В обзоре альбома плашка «Пресет вёрстки»: amber-warning переписан
#     («выберите шаблон в каталоге»), tooltip кнопки сборки тоже.
#   • Payload create_album/update_album: больше не отправляет
#     preset_slug. albums.config_preset_id остаётся в БД для legacy
#     совместимости, но новые альбомы её не заполняют.
#   • Удалены неиспользуемые: state `presets`, useEffect presets_list.
#
# Винегрет убран. Один Шаблон — одна сборка. Осталось закрыть фазу
# документом-summary и контекстом v150.
#
# Эстимация: ~1 сессия, 7 коммитов, 2 SQL миграции — почти всё сделано.
#
