...
# фаза 0.5 в работе (refactor под гибкие пресеты в БД).
#
# Что нового в v40 относительно v39:
# - Фаза 0 продукта B полностью закрыта (как было в v39).
# - Запущена фаза 0.5 (refactor builder под пресеты в БД).
# - Сделано: 0.5.1 (миграция БД — таблицы config_presets + FK),
#   0.5.2 (TypeScript типы Preset + seed 14 пресетов в БД).
# - Bonus: 0.15 — fix slot_capacity.photos_friend для E-Student мастеров
#   (был баг парсера IDML).
# - Архитектурное решение: путь Г1 (прямое переписывание builder под Preset),
#   не адаптер. Чистая модель.
# - НЕ сделано: 0.5.3 (рефакторинг buildAlbum), 0.5.4-0.5.7.
#   Делаем 0.5.3 в следующей сессии.
#
# Подробности фазы 0.5 — в docs/phase-0.5-spec.md (v4, финальная).
...
### Продукт B — фаза 0.5 (refactor builder под пресеты) — В РАБОТЕ

После фазы 0 обнаружено что builder работает с 7 жёстко-зашитыми
комплектациями (config_type enum в коде). Это негибко — партнёр не может
создать свой пресет без правки кода.

**Цель фазы 0.5:** перенести описание комплектаций из кода в БД.
7 текущих пресетов мигрируют в таблицу config_presets как «глобальные
дефолты». Builder читает пресет из БД и работает универсально.

**Архитектурный принцип:** богатая БД (структура config в JSONB готова под
все будущие фичи: additional_spreads, financial_mode, common_section, etc.),
простая логика (builder в 0.5 читает только подмножество полей).

**Архитектурный путь:** Г1 — прямое переписывание builder под Preset, без
адаптера. Чистая модель, никакой сосуществующей legacy.

**Сделано:**
- 0.5.1 — миграция БД (config_presets + FK в albums и children, удалена albums.config_type)
- 0.5.2 — TypeScript типы Preset/PresetConfig в lib/album-builder/types.ts
  + seed 14 глобальных пресетов (7 комплектаций × 2 print_type) в БД
- Bonus: 0.15 — fix slot_capacity.photos_friend у E-Student мастеров

**Не сделано:**
- 0.5.3 — рефакторинг buildAlbum под Preset (СЛЕДУЮЩАЯ задача)
- 0.5.4 — адаптация 58 smoke-сцен
- 0.5.5 — endpoint Build Test переключить на preset_slug
- 0.5.6 — UI выбор пресета per-child
- 0.5.7 — контекст v40 → v41 (закрытие фазы 0.5)

Подробности: `docs/phase-0.5-spec.md` (v4, финальная).
...
| 🔵 0.5 | Refactor под пресеты в БД | В работе |
