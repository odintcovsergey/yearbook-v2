...
# Версия: v105
# Обновлено: 20.05.2026 (после РЭ.23.2 — утилита renderPreviewSvg).
#
# ФАЗА РЭ.23 — Визуальный каталог мастеров (В РАБОТЕ, начата 20.05.2026)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.23.0 (5d6a276) — docs/phase-Р23-spec.md v1.0 (157 строк).
# ✅ РЭ.23.1 (e4b7a2f) — миграция БД display_label, ✅ применена 20.05.2026.
# ✅ РЭ.23.2 (18dfb3c) — утилита renderPreviewSvg + 9 тестов.
#    lib/album-builder/render-preview-svg.ts: чистая функция
#    placeholders + size → SVG-строка. Без зависимостей от
#    React/Konva. Круглые аватарки, rotation, разворотные мастера
#    (gutter 4mm). Размер SVG < 5KB для N-Grid 36 placeholders.
#    369/369 vitest passing (было 360).
#
# ⏳ Следующий подэтап: РЭ.23.3 — API endpoints.
#    В app/api/tenant/route.ts добавить два action'а:
#      • template_set_list_with_previews — возвращает массив мастеров
#        с готовыми SVG-превью + display_label + slot_capacity + page_role.
#        Использует renderPreviewSvg для каждой записи.
#        Доступ только для super-admin.
#      • template_set_update_display_label — обновляет display_label
#        одного мастера. Body: {template_id, display_label}.
#        Доступ только для super-admin.
#    Permission через существующий middleware (нужно посмотреть какой
#    именно — возможно requireSuperAuth или аналогичный).
#
# Контекст фазы:
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v105
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v105 — что закрыто (РЭ.23.0..23.2)
#      • docs/phase-Р23-spec.md v1.0 — план фазы (157 строк)
#      • lib/album-builder/render-preview-svg.ts — функция уже готова
...
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.23.3 — API endpoints.
#      Файл: app/api/tenant/route.ts (расширение существующего)
#      Два новых action:
#        a) template_set_list_with_previews — выгружает все мастера
#           template_set'а из spread_templates, для каждого вызывает
#           renderPreviewSvg, возвращает массив:
#             { id, name, display_label, page_role, slot_capacity,
#               is_spread, preview_svg }
#           Доступ: super-admin. Посмотреть какой именно middleware
#           используется в существующих /super endpoints (например
#           rule_presets_list — должен быть аналогичный).
#        b) template_set_update_display_label — body
#             { template_id: string, display_label: string | null }
#           Обновляет одну запись, возвращает success.
#           Доступ: super-admin. Tenant-aware (только мастера своего
#           template_set'а).
#      Что НЕ трогаем: engine, существующие endpoints. UI — РЭ.23.4.
