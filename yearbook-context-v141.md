...
# Версия: v141
# Обновлено: 21.05.2026 (после РЭ.28.5 — модалка ввода размеров).
#
# ⏳ ФАЗА РЭ.28 ПОЧТИ ЗАКРЫТА — осталось только summary (28.6)
# ──────────────────────────────────────────────────────────────────
# Все функциональные подэтапы (28.0..28.5) закрыты. Остался 28.6:
# summary + закрытие фазы.
# Spec: docs/phase-Р28-spec.md v1.0.
# Последний commit фазы: f98df1c (РЭ.28.5).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v141
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v141 — вся функциональность РЭ.28 готова, остался summary
#      • docs/phase-Р28-spec.md — спецификация фазы
#      • docs/phase-Р27-summary.md — образец summary (closed phase)
#   4. Применены в Supabase: все миграции ✅
#   5. ФИНАЛ ФАЗЫ РЭ.28 — подэтап 28.6 (summary + закрытие фазы):
#
#      Цель: написать docs/phase-Р28-summary.md по образцу
#      phase-Р27-summary.md (~250-350 строк) с разделами:
#
#      a) Что было ДО фазы (партнёр не мог менять размеры под свою
#         типографию, все template_set'ы создавались Сергеем через
#         /super загрузкой IDML).
#
#      b) Что сделано — по подэтапам с ссылками на коммит-хэши:
#         28.0 spec (1193b5d), 28.1 миграция (68a7f0a),
#         28.2 чистые утилиты (888f58c, 55 тестов),
#         28.3 API endpoints (0b8eced),
#         28.4 UI каталога (e22ceae),
#         28.5 UI модалка (f98df1c).
#
#      c) Архитектурное решение (resize-pipeline через чистую функцию
#         prepareTemplateSetClone в lib/template-set-clone, API
#         остаётся тонким слоем).
#
#      d) 8 архитектурных развилок A..H (см. spec §7) одной таблицей.
#
#      e) Применение урока из РЭ.27: перед миграцией 28.1 явно
#         выгружена схема через information_schema. Никаких новых
#         «открытий» в фазе РЭ.28 — это успех правила.
#
#      f) Что НЕ сделано (зафиксировано как долг):
#         - Партнёрский IDML с нуля (сценарий B — РЭ.29+)
#         - Графический редактор placeholder'ов
#         - DPI ≠ 300 (240/600 экзотика)
#         - Обновление клонов при изменении оригинала
#         - Шеринг клонов между партнёрами
#         - Свой cover_preview_url у клона
#
#      g) Статистика фазы:
#         - 6 коммитов основной фазы (1193b5d..f98df1c)
#         - 1 SQL миграция (28.1)
#         - 55 новых unit-тестов (437 → 492)
#         - 6 файлов кода: 5 lib + 2 UI компонента
#
#      h) Ключевые файлы фазы.
#
#      После summary — финальный контекст v142 «ФАЗА РЭ.28 ЗАКРЫТА»
#      по образцу v135 (РЭ.27 закрытая).
