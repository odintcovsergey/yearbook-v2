...
# Версия: v134
# Обновлено: 21.05.2026 (после РЭ.27.7 — миграция данных + cleanup кода).
#
# ⏳ ФАЗА РЭ.27 ПОЧТИ ЗАКРЫТА — осталось только summary (27.8)
# ──────────────────────────────────────────────────────────────────
# Все функциональные подэтапы (27.0..27.7) закрыты. Остался 27.8:
# написать docs/phase-Р27-summary.md и закрыть фазу.
# Spec: docs/phase-Р27-spec.md v1.0.
# Последний commit фазы: 8cd1090 (РЭ.27.7c cleanup кода).
#
# ✅ Миграции применены в Supabase (Сергей подтвердил 21.05.2026):
#    - РЭ.27.1 (индекс idx_albums_print_type)
#    - РЭ.27.7 основная (слияние 7 пар + заполнение NULL)
#    - РЭ.27.7b (заполнение 6 черновиков layflat)
#
# Финальное распределение albums.print_type (12 альбомов):
#    layflat: 9 (3 исходных + 6 черновиков мигрированных)
#    soft:    3 (2 исходных + 1 ssId-альбом который уже был layflat)
#    NULL:    0 ✅
#
# config_presets после слияния (7 записей):
#    individual, light, maximum, medium, mini, standard, universal
#    Все print_type=layflat (legacy, не используется engine'ом).
#
# Что готово после 27.7 (вся функциональность фазы):
#   • Бэкенд:
...
#   • UI формы альбома:
#     - Селект «Тип листов в альбоме» — Из шаблона / Твёрдые / Мягкие (РЭ.27.6)
#   • Миграция данных:
#     - Все 6 NULL-альбомов заполнены (РЭ.27.7 + 27.7b)
#     - 14 дубль-пресетов слиты в 7 чистых (РЭ.27.7)
#     - Submit-path в форме обновлён под новый формат slug (РЭ.27.7c)
#
...
#   ✅ 27.4 fix — config_presets для legacy fallback (commit 504e0c4)
#   ✅ 27.5 — UI палитры: spread-мастера серые при soft (commit ed380a3)
#   ✅ 27.6 — UI формы: селект «Тип листов» (commit c421897)
#   ✅ 27.7 — миграция данных + слияние пресетов v2 (commits ec08837 + 199e2a9)
#   ✅ 27.7b — заполнение черновиков (commit a936964)
#   ✅ 27.7c — cleanup кода (commit 8cd1090)
#   ⏳ 27.8 — summary + закрытие фазы
#
# Тесты на main: 437/437 passing (стабильно после 27.3).
#
# ⚠️ УРОК ФАЗЫ (4 открытия о расхождении ментальной модели и реальности БД):
#   1. РЭ.27.1: колонка albums.print_type УЖЕ существовала с 8 мая 2026.
#      Миграция оказалась no-op в части ADD COLUMN, полезна только индексом.
#   2. РЭ.27.2: API update_album УЖЕ принимал print_type, но перетирал
#      из пресета в одном кейсе. Работа = инвертировать приоритет.
#   3. РЭ.27.4: попытка JOIN через 'presets' (новая таблица) для
#      legacy-альбомов с config_preset_id. Правильно — 'config_presets'.
#      Fix потребовал отдельный коммит 504e0c4.
...
#      Транзакция BEGIN/COMMIT защитила — откатилось, БД цела.
#      Миграция v2 переписана через UUID, отработала чисто.
#
#   Правило на будущее (обязательное действие, не «принцип»):
#   ПЕРЕД КАЖДЫМ ПОДЭТАПОМ, особенно с миграцией:
#   - SELECT column_name, data_type FROM information_schema.columns
#     WHERE table_name = '...';
#   - Проверить распределение через test JOIN на 5 строках.
#   - Только потом писать код миграции / запрос.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v134
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v134 — все функциональные подэтапы 27.0..27.7 закрыты
#      • docs/phase-Р27-spec.md — спецификация фазы (7 архитектурных
#        развилок, план подэтапов)
#      • docs/phase-Р25-summary.md — образец summary прошлой фазы
#   4. Все миграции применены в Supabase ✅
#   5. ФИНАЛ ФАЗЫ РЭ.27 — подэтап 27.8 (summary + закрытие фазы):
#
#      Цель: написать docs/phase-Р27-summary.md по образцу
#      phase-Р25-summary.md (≈200-250 строк) с разделами:
#
#      a) Что было ДО фазы (preset.print_type в config_presets,
#         дубль-пресеты, engine жёстко читал из пресета).
#
#      b) Что сделано — по подэтапам с ссылками на commit-хэши:
#         27.0 (spec, 266341a), 27.1 (5a52544), 27.2 (3096c18),
#         27.3 (d8ea615), 27.4 (2ca86f2 + 504e0c4 fix), 27.5 (ed380a3),
#         27.6 (c421897), 27.7 (199e2a9 + a936964 + 8cd1090).
#
#      c) Архитектурное решение (resolvePrintType в lib/album-builder,
#         тонкий override-слой перед engine, не переписывание engine).
#
#      d) Ключевые решения — 7 архитектурных развилок A..G (одна
#         сводная таблица), отдельно ключевой переход от двойного
#         редактора к одному разворотному.
#
#      e) ⚠️ ОБЯЗАТЕЛЬНЫЙ РАЗДЕЛ «Уроки фазы» — 4 открытия о расхождении
#         ментальной модели и схемы БД (см. шапку v134). Это важно
#         для будущих фаз — задокументировать правило «перед каждой
#         миграцией проверять information_schema».
#
#      f) Что НЕ сделано (зафиксировать как долг):
#         - Удаление колонки config_presets.print_type (оставлена как
#           legacy fallback; удалить в будущей зачистке)
#         - Подтверждение в UI при смене типа листов если уже выбраны
#           spread-мастера (развилка G — пока не нужно, в текущем
#           template_set нет spread-мастеров)
#         - PDF-экспорт с водяным знаком «Форзац» (UI-визуализация
#           работает, в PDF — отдельная фаза)
#         - Расширенные роли page_role ('common_spread', 'student_spread')
#           — функция isSpreadMaster их учитывает, но IDML-конвертер
#           ещё не размечает
#
#      g) Статистика фазы:
#         - 11 коммитов в фазе (исключая контексты): 266341a..8cd1090
#         - 3 SQL миграции
#         - 30 новых unit-тестов (407 → 437)
#         - ~7 файлов кода затронуто
#
#      h) Ключевые файлы фазы (для трейса в будущем):
#         lib/album-builder/print-type-resolver.ts
#         lib/album-builder/endpaper-rules.ts
#         lib/album-builder/spread-master-filter.ts
#         lib/album-builder/__tests__/...
#         migrations/2026-05-21-albums-print-type*.sql (3 файла)
#         app/api/tenant/route.ts (create_album, update_album, action=album)
#         app/api/layout/route.ts (override print_type перед engine)
#         app/app/album/[id]/layout/page.tsx (EndpaperPlaceholder + плашка)
#         app/app/_components/TemplatePickerModal.tsx (spread фильтр)
#         app/app/page.tsx (AlbumFormModal с print_type_override)
#         types/index.ts (PrintType type + Album.print_type? поле)
#
#      После summary — финальный контекст v135 «ФАЗА РЭ.27 ЗАКРЫТА»
#      по образцу v125 (РЭ.25 закрытая). После этого фаза полностью
#      закрыта.
