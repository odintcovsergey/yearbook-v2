...
# Версия: v123
# Обновлено: 21.05.2026 (после РЭ.25.5 — UI родителя + CSV колонка готовы).
#
# ⏳ ФАЗА РЭ.25 В РАБОТЕ — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтапы 25.0..25.5 закрыты. Следующий — 25.6 (счётчик 'N из M'
# в карточке альбома). После него 25.7 — summary и закрытие фазы.
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: de4182a (РЭ.25.5).
#
# ✅ Миграция РЭ.25.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026).
#
# Что готово после 25.5:
#   • Бэкенд закрыт полностью (миграция + API + engine-фильтр).
#   • UI фотографа в /app:
#     - Чекбокс «Включить в личный раздел всех учеников» в форме
#       альбома (включает мягкий режим).
#     - Колонка «Заказ» в таблице учеников с чекбоксами на каждой
#       строке (быстрое переключение без раскрытия). Клик по чекбоксу
#       не раскрывает строку (stopPropagation).
#     - Бейдж «не заказывает» + приглушение строки для is_purchased=false.
#   • UI родителя в /[token]:
#     - Чекбокс «Мы заказываем альбом» на основном flow (постоянный
#       блок над шагами, виден всегда).
#     - Тот же чекбокс на экране 'Выбор уже сделан' — родитель может
#       передумать после submit.
#     - Optimistic update с откатом, ошибки показываются inline.
#   • CSV-экспорт: колонка «Заказ» (да/нет/— для учителей) после ФИО.
#     Для скрипта автовёрстки InDesign.
#
# Что ещё НЕ работает:
#   • Карточка альбома без счётчика 'N из M заказали' (это 25.6,
#     остался только этот пункт + summary).
#
...
#   ✅ 25.4 — UI фотографа (commits f3cdbf4 + 30aece9 fix колонки)
#   ✅ 25.5 — UI родителя + CSV колонка 'Заказ' (commit de4182a)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v123
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v123 — РЭ.25.0..25.5 закрыты. Фаза почти завершена.
#      • docs/phase-Р25-spec.md v1.0 (334 строки) — спецификация фазы,
#        особенно §5.4 (счётчик 'N из M' в карточке альбома)
#      • docs/phase-Р24-summary.md — последняя закрытая фаза
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.6:
#      Счётчик 'Заказали: N из M' в карточке альбома (app/app/page.tsx).
#      • Где: в карточке альбома в списке /app, рядом со статистикой
#        'Завершили: X из Y' (или вместо неё / дополнительно — обсудить).
#      • Цифры:
#        - N = COUNT children WHERE album_id=X AND is_purchased=true
#        - M = COUNT children WHERE album_id=X (total)
#      • Откуда брать данные:
#        - API /api/tenant?action=albums_list возвращает массив альбомов
#          со stats. Туда нужно добавить purchased_count.
#        - Альтернатива: вычислять на клиенте из children[], но это
#          возможно только когда альбом открыт. На карточках лучше из
#          API.
#      • Логика отображения:
#        - Если include_non_purchasers=true → счётчик НЕ показывается
#          (мягкий режим, в альбоме все, не релевантно).
#        - Если 0 учеников в альбоме — счётчик НЕ показывается.
#        - Если N === M (все заказали) — показывается обычно,
#          подсветка не нужна.
#        - Если N < M — небольшая визуальная индикация (приглушённый
#          цвет / иконка), но не алярм.
#      Дальше — 25.7 (summary + закрытие фазы).
#      Все детали в docs/phase-Р25-spec.md §5.4 и §7.
