...
# Версия: v73
# Обновлено: 18.05.2026 (РЭ.20.2 — миграция БД: total_pages + density
#                       + sheet_type в presets)
#
...
#   ⏳ Дальше: РЭ.20.3 (типы Preset/RuleContext), РЭ.20.5 (loader пресетов),
#      затем РЭ.12 (UI). Всего осталось ~22-30 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v73
#   3. Открыть docs/phase-Р20-spec.md — это план работы
#   4. РЭ.20.2 миграция БД уже сделана (715add1).
#      ⚠️ Сергей, не забудь применить её в Supabase SQL Editor:
#      migrations/2026-05-18-presets-total-pages-density-sheet-type.sql
#   5. Следующий шаг — РЭ.20.3: расширить TypeScript типы Preset
#      (total_pages/density/sheet_type) + RuleContext (pages_remaining,
#      mandatory_section) + RulesAlbumInput (deprecate common_section_max_spreads).
#      Спека: docs/phase-Р20-spec.md §3.
#
# 📍 КАК ПРОБОВАТЬ КОНТЕНТ-РЕДАКТОР НА ЖИВОМ АЛЬБОМЕ:
#   1. Открой любой собранный альбом → 'Открыть редактор'
#   2. Клик левой кнопкой по любому фото (одиночный, не drag)
...
#      (РЭ.20.2 — применить вручную в Supabase SQL Editor)
...
#      common_section_max_spreads на albums НЕ удалён — это задача РЭ.20.3.
#   1. РЭ.20.3 — расширение TypeScript типов Preset + RuleContext +
#      RulesAlbumInput. Deprecate common_section_max_spreads (mark @deprecated,
#      пока оставить в коде чтобы не сломать build-from-preset.ts).
#   2. РЭ.20.4 — расширение build.ts: pages_remaining + mandatory_section
#   3. РЭ.20.5 — loader/migrate существующих 7 пресетов: проставить
#      реальные значения total_pages/density/sheet_type через SQL UPDATE
#      (отдельная миграция-данных + seed-обновление в коде).
#   4. РЭ.12.A — UI /app/presets список + create form
#   5. РЭ.12.Б — UI редактор пресета (total_pages, sections, sheet_type)
#   6. РЭ.20.6 — генератор правил из xlsx-матрицы
#   7. РЭ.20.7 — unit-тесты для каждой строки матрицы
#   8. РЭ.20.8 — финальная документация
#
