...
# Версия: v98
# Обновлено: 20.05.2026 (после РЭ.22.6 — engine semantic для mode='grid').
#
...
# ✅ РЭ.22.6.0 (5cbd51a) — data-миграция grid-мастеров, ✅ применена 20.05.2026.
# ✅ РЭ.22.6 (1c993ee) — engine semantic для mode='grid'.
...
#    - В sections/students.ts: новая buildGridSemantic — полный аналог
#      buildGrid через семантический поиск (base + combined-tail +
#      adaptive-tail + null-padding fallback).
#    - +10 тестов в sections-students-grid-semantic.test.ts.
#    - Всего тестов в проекте: 341/341 passing (было 331/331).
#
# Все три режима двух-осевой модели заработали через семантический поиск:
#   - mode='page'   — ✅ РЭ.22.4
#   - mode='spread' — ✅ РЭ.22.5
#   - mode='grid'   — ✅ РЭ.22.6
#
# ⏳ Следующий подэтап: РЭ.22.7 — teachers секция: семантический поиск.
#    - В sections/teachers.ts заменить slot-chains (F-Head-WithPhoto /
#      F-Head-SmallGrid / F-Head-LargeGrid + G-HalfClass / G-FullClass /
#      G-Teachers-3x3/4x3/4x4) на семантический поиск через page_role +
#      slot_capacity (teachers, head_teacher, photos_full/half).
#    - Объёмный подэтап — tablиц subjects 0 / 1-4 / 5-8 / 9 / 10-12 /
#      13-16 / 17+ нужно покрыть через slot_capacity teachers и проверить
#      что engine выбирает правильного мастера.
#    - Может быть разбит на 7.1 (head_teacher левая) + 7.2 (правая).
#
...
#
#   ⏳ Следующий подэтап: РЭ.22.7 — teachers секция: семантический поиск
#      Большой подэтап (lib/rule-engine/sections/teachers.ts, 435 строк).
#      Slot-chains F-Head-* + G-* → семантика через page_role +
#      slot_capacity. Может быть разбит на 7.1 (head_teacher левая) +
#      7.2 (правая страница).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v98
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v98 — что закрыто (РЭ.22.0..РЭ.22.6, students полностью)
#      • docs/phase-Р22-spec.md v1.0 — §6.5 (teachers семантика).
#      • lib/rule-engine/sections/teachers.ts (435 строк) — текущая таблица
#        subjects 0/1-4/5-8/9/10-12/13-16/17+ через жёсткие имена
#        F-Head-WithPhoto / F-Head-SmallGrid / F-Head-LargeGrid +
#        G-HalfClass / G-FullClass / G-Teachers-3x3/4x3/4x4.
#      • lib/rule-engine/master-finder.ts — у findStudentMaster sql фильтр
#        studentsCap===1, для teachers нужно создать аналогичную findTeacherMaster
#        с фильтром по teachers + head_teacher slot_capacity (page_role
#        теперь поддерживает teacher_left/teacher_right).
...
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.7 — teachers семантический поиск.
#      Большой подэтап, рекомендуется разбить:
#        РЭ.22.7.1 — data-миграция teachers-мастеров (page_role +
#                    slot_capacity для F-Head-* и G-*). Аналог РЭ.22.6.0.
#                    Сначала SELECT для выгрузки текущих имён и placeholder'ов.
#        РЭ.22.7.2 — engine teachers семантический.
#                    Новая findTeacherMaster в master-finder.ts.
#                    Перепись fillTeachersSection через семантический поиск
#                    (left: page_role='teacher_left', teachers/head_teacher
#                    из slot_capacity; right: page_role='teacher_right',
#                    teachers / photos_full / photos_half).
#                    Сохранить fallback на legacy logic когда teachers-мастера
#                    не размечены (для обратной совместимости).
#                    Тесты.
#      Что НЕ трогаем: students (закрыты), soft_intro/soft_final (РЭ.22.8),
#                      common_* и transition (не входят в скоуп фазы).
#      Проверки: tsc + next build + vitest зелёные.
