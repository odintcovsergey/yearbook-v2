...
# Версия: v112
# Обновлено: 20.05.2026 (после РЭ.24.4 — 5 API endpoints).
#
# ФАЗА РЭ.24 — Готовые шаблоны для быстрого старта партнёра (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.24.0 (614e6a7) — spec v1.0
# ✅ РЭ.24.1 (037f9ff) — миграция (is_recommended + parent_preset_id), применена
# ✅ РЭ.24.2 (e526965) — validatePreset + 18 тестов
# ✅ РЭ.24.3 (a6a7ee3) — buildPresetPreviewBundle + 8 тестов
# ✅ РЭ.24.4 (f828c46) — 5 API endpoints в app/api/tenant/route.ts:
#    GET actions:
#      • templates_list_global — каталог (только is_recommended=true).
#        validatePreset → loadBundle → buildPresetPreviewBundle.
#      • templates_list_my — личная библиотека партнёра, с valid+errors.
#    POST actions:
#      • template_clone — копия глобального в свой tenant_id с
#        parent_preset_id ссылающимся на оригинал. is_recommended=false.
#      • template_create_blank — минимальный пустой шаблон.
#      • template_delete — с проверкой активных альбомов (409 если есть).
#    Хелперы: rowToPresetForValidation + rowToDescription.
#    395/395 vitest без изменений (API не покрыт unit-тестами,
#    протестируется через UI в РЭ.24.5).
#
# ⏳ Следующий подэтап: РЭ.24.5 — страница /app/templates (UI).
#    Файлы:
#      • app/app/templates/page.tsx (новый) — клиентский компонент
#        с двумя секциями:
#          1. «Готовые от OkeyBook» — карточки глобальных шаблонов
#             (fetch templates_list_global) с кнопкой «Сохранить в мои»
#          2. «Мои шаблоны» — fetch templates_list_my, кнопки
#             «Редактировать», «Удалить», и кнопка «Создать свой шаблон»
#      • Карточка шаблона: 1 крупное превью (students) + 3 малых
#        (cover/teachers/soft) + название + описание + бейджи.
#      • Невалидные мои шаблоны: красный бейдж «Доработай» + список errors.
#      • Редактирование: открывается тот же PresetEditorModal что и в
#        /super/presets (можно переиспользовать компонент).
#      • Ссылка на /app/templates из главного меню /app.
#    Что НЕ трогаем: модалка создания альбома (это 24.6),
#    /super/presets (это 24.7).
...
#   2. КАТАЛОГ /app/templates — витрина (новая страница, делаем сейчас)
#   3. ЛИЧНАЯ БИБЛИОТЕКА — presets WHERE tenant_id = X
#   4. ВЫБОР ПРИ АЛЬБОМЕ — модалка (правка существующей, РЭ.24.6)
#
# Подэтапы фазы (8 шт):
#   ✅ 24.0 — spec
#   ✅ 24.1 — миграция (is_recommended + parent_preset_id)
#   ✅ 24.2 — validatePreset + 18 тестов
#   ✅ 24.3 — buildPresetPreviewBundle + 8 тестов
#   ✅ 24.4 — API: 5 новых action'ов
