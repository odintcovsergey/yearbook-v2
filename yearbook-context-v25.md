...
# Обновлено: 17.04.2026 (этап 3.5 ПОЛНОСТЬЮ закрыт: a+b)
...
### Этап 3.6 — интеграция брендинга в страницу родителя (следующий)
- Страница /[token] (сценарий родителя) подтягивает tenant.settings:
  ...
  - Подпись footer_text в подвале
- URL /t/[slug]/[token] как альтернатива для брендированных ссылок
- Бэк: /api/child GET обогащается tenant (через album → tenant_id → 
  tenants.logo_url, name, settings)
- ВАЖНО: родительская страница /[token] — та страница, что НЕ трогать
  переписыванием. Добавляем брендинг аккуратно, без изменения логики отбора.
