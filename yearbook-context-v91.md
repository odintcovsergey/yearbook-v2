...
# Версия: v91
# Обновлено: 20.05.2026 (после старта фазы РЭ.22 — РЭ.22.0 spec'ификация).
#
# СТАРТ ФАЗЫ РЭ.22 — Конструктор пресетов с двух-осевой моделью
# ────────────────────────────────────────────────────────────────
# РЭ.22.0 (коммит de27d00) — docs/phase-Р22-spec.md v1.0 зафиксировал
# архитектуру фазы. Двух-осевая модель «режим × параметры» для личного
# раздела:
...
#
# Новые колонки БД (РЭ.22.1): student_layout_mode TEXT,
# student_grid_size INT. Старые student_pages_per_student/friend_photos/
# has_quote — deprecated, не удаляем (зачистка отдельной сессией с
# двойным подтверждением Сергея).
#
...
#
# 10 подэтапов (РЭ.22.1..РЭ.22.10), порядок:
#   миграция БД → типы+API → UI → engine page → engine spread →
#   data-миграция grid-мастеров → engine grid → teachers →
#   soft_intro+soft_final → диагностика → финал.
#
...
#
#   ⏳ Следующий подэтап: РЭ.22.1 — миграция БД
#      ALTER TABLE presets ADD COLUMN student_layout_mode TEXT,
#      student_grid_size INT + два CHECK через DO-блоки.
#      После коммита — Сергей применяет в Supabase SQL Editor.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v91
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v91 — старт фазы РЭ.22 (РЭ.22.0 spec закрыт)
#      • docs/phase-Р22-spec.md v1.0 (564 строки) — ПОЛНЫЙ план фазы
#        с зафиксированными развилками. Это основной источник истины
#        для всех 10 подэтапов РЭ.22.
#      • Секция "ФАЗА РЭ.21.8 — ИТОГИ" в этом файле — что было сделано
#        в предыдущей большой фазе.
...
#      • 2026-05-XX-presets-student-layout-mode.sql ⬜ БУДЕТ В РЭ.22.1
#        — добавит student_layout_mode TEXT + student_grid_size INT
#        с CHECK'ами через DO-блок.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.1 — миграция БД для двух-осевой модели.
#      Файл: migrations/2026-05-XX-presets-student-layout-mode.sql
#      Содержание: ALTER TABLE presets ADD COLUMN student_layout_mode
#      TEXT, student_grid_size INT + два CHECK constraint'а через
#      DO-блоки (как РЭ.21.8.15).
#      После коммита Сергей применяет в Supabase SQL Editor.
#      Дальше идём по плану spec'а:
#        РЭ.22.2 — типы + API валидация
#        РЭ.22.3 — UI двух-осевая модель
#        РЭ.22.4 — engine mode='page'
#        РЭ.22.5 — engine mode='spread'
#        РЭ.22.6.0 — data-миграция grid-мастеров
#        РЭ.22.6 — engine mode='grid'
#        РЭ.22.7 — teachers семантический
#        РЭ.22.8 — soft_intro+soft_final семантический
#        РЭ.22.9 — диагностика недостающих мастеров
#        РЭ.22.10 — финал, контекст
