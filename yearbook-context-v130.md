...
# Версия: v130
# Обновлено: 21.05.2026 (после РЭ.27.3 — engine читает albums.print_type).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0..27.3 закрыты. Следующий — 27.4 (превью SVG + вставка
# форзацев в layout).
# Spec: docs/phase-Р27-spec.md v1.0.
# Последний commit фазы: d8ea615 (РЭ.27.3).
#
# ✅ Миграция РЭ.27.1 применена в Supabase.
#
# Что готово после 27.3 (главная работа фазы):
#   • Бэкенд:
#     - albums.print_type (с 8 мая 2026, CHECK constraint, индекс из 27.1)
#     - API create_album/update_album с явным print_type (РЭ.27.2)
#   • Engine: чистые модули + интеграция:
#     - resolvePrintType(albumPrintType, presetPrintType): PrintType
#       приоритет album → preset → 'layflat' (default)
#     - printTypeToSheetType / sheetTypeToPrintType — bridge
#     - getEndpaperRules(printType): EndpaperSpec[]
#       описание (ПЛАН) правил first_left / last_right для soft.
#       Применение и отрисовка — в 27.4.
#     - isSpreadMaster / isMasterAllowedForPrintType — для UI 27.5
#   • Интеграция в app/api/layout/route.ts:
#     - SELECT albums расширен print_type
#     - Перед buildAlbum v2 (legacy): override preset.print_type
#     - Перед buildFromSectionStructure v3: override bundle.preset
#       (print_type + sheet_type синхронно)
#   • 30 новых unit-тестов (437/437 на main, было 407)
#
# Что ещё НЕ работает:
#   • Вставка EndpaperPlaceholder в layout (правила работают как ПЛАН,
#     но реальные форзацы не появляются в собранном альбоме). Это 27.4.
#   • Водяной знак «Форзац» в превью SVG — это 27.4.
#   • UI каталога мастеров — фильтр spread для soft (это 27.5).
#   • UI формы альбома — селект «Тип листов» (это 27.6).
#   • 6 NULL-альбомов в проде не заполнены (это 27.7).
#   • Дубль-пресеты «layflat»/«soft» не слиты (это 27.7).
#
# ⚠️ ВАЖНО ПОСЛЕ ДЕПЛОЯ 27.3: engine теперь УВАЖАЕТ albums.print_type.
#   Если в проде у альбома albums.print_type='soft' при preset.print_type=
#   'layflat' — engine соберёт по soft-правилам (intro_section вставит
#   и т.д.). Это и есть цель РЭ.27, но если на проде есть случайные
#   такие альбомы — поведение сборки изменится. Проверить распределение:
#     SELECT a.print_type AS a, p.print_type AS p, COUNT(*)
#     FROM albums a LEFT JOIN presets p ON a.config_preset_id = p.id
#     GROUP BY a.print_type, p.print_type;
#   Ожидание: a и p совпадают (так как 27.2 копирует из пресета).
#   Расхождение значит что кто-то менял через API явно — это OK.
#
# Что зафиксировано в spec (7 архитектурных развилок утверждены):
#   A..G (см. v126/v127/v128 для деталей)
#
...
#   ⏳ 27.4 — превью SVG: водяной знак «Форзац» + вставка форзацев в layout
...
#
# Тесты на main: 437/437 passing (407 + 30 в 27.3).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v130
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v130 — РЭ.27.0..27.3 закрыты, главная работа сделана
#      • docs/phase-Р27-spec.md §3.4 «Автозаглушка Форзац» — описание
#        того что нужно отобразить
#      • lib/album-builder/endpaper-rules.ts — функция-план
#        (она уже написана и протестирована, её надо применить)
#   4. Применены в Supabase: все РЭ.22..РЭ.25.1 + РЭ.27.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.4 (превью SVG):
#      Две задачи:
#
#      a) Вставка форзацев в собранный layout:
#         - В app/api/layout/route.ts после buildFromSectionStructure
#           (и buildAlbum для legacy) — применить getEndpaperRules
#           и вставить EndpaperPlaceholder в spreads.
#         - Новый тип элемента в AlbumLayout (надо аккуратно — может
#           ломать тесты render-preview).
#         - Альтернатива: вынести логику в адаптер, не трогать
#           основной layout type.
#
#      b) Отрисовка водяного знака «Форзац» в lib/album-builder/render-preview-svg.ts:
#         - Когда встречается EndpaperPlaceholder — отрисовать SVG
#           страницу с текстом «Форзац» бледным курсивом по центру.
#         - Размер шрифта примерно 24-30pt, цвет #BBBBBB или
#           opacity 0.4.
#         - Тестировать визуально на примере альбома с print_type=soft.
#
#      Что МОЖНО подумать заранее:
#         - Где хранится layout — в БД (album_layouts.spreads jsonb)
#           или генерится каждый раз? Если в БД — миграция нужна?
#         - Стоит ли отображать форзац как полноценный spread или
#           как «специальный элемент» рядом с настоящими spreads?
#
#      После 27.4 — soft-альбомы будут визуально показывать форзацы.
#      Дальше — 27.5 (UI каталога), 27.6 (форма альбома), 27.7
#      (миграция данных), 27.8 (summary).
