...
# Версия: v120
# Обновлено: 21.05.2026 (после РЭ.25.2 — API + типы готовы и закоммичены).
#
# ⏳ ФАЗА РЭ.25 В РАБОТЕ — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтапы 25.0 (spec), 25.1 (миграция), 25.2 (API) закрыты.
# Следующий — 25.3 (engine: фильтр в build-album-input.ts + тесты).
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: b0d76ca (РЭ.25.2 API).
#
# ✅ Миграция РЭ.25.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026
#    проверочным SQL — обе колонки видны в information_schema.columns).
#
# Что готово после 25.2:
#   • API /api/tenant POST: новый action 'update_child' принимает
#     { child_id, full_name?, class?, is_purchased? } — патч-апдейт.
#   • API /api/tenant POST: 'create_album' insert и 'update_album'
#     allowedFields принимают include_non_purchasers (default false).
#   • API /api/tenant GET action='children' возвращает is_purchased
#     в каждом элементе (для UI фотографа 25.4).
#   • API /api/select PUT: новый action 'set_purchased' — родитель
#     меняет children.is_purchased в любой момент, даже после
#     submitted_at !== null.
#   • API /api/child GET: SELECT ребёнка расширен полем is_purchased
#     (для UI родителя 25.5).
#   • types/index.ts: Album.include_non_purchasers? и Child.is_purchased?
#     (оба опциональны, бэк-совместимо).
#   • app/app/page.tsx локальные типы готовы к UI 25.4.
#
# Что ещё НЕ работает (приедет в следующих подэтапах):
#   • Engine ещё НЕ фильтрует is_purchased=false из секции students
#     (это 25.3).
...
#   В lib/smart-fill/build-album-input.ts — фильтр применяется ДО
...
#   ⏳ 25.3 — engine: фильтр в build-album-input.ts + тесты
...
#
# Тесты на main: 395/395 passing (стабильно с РЭ.24.8).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v120
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v120 — РЭ.25.0..25.2 закрыты. Фаза РЭ.25 в работе.
...
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.3 (engine-фильтр):
#      • Расширить lib/smart-fill/build-album-input.ts:
#        - SELECT из albums: добавить include_non_purchasers
#        - SELECT из children: добавить is_purchased
#        - После сборки children[] — применить фильтр:
#          if (!album.include_non_purchasers)
#            children = children.filter(c => c.is_purchased === true)
#        - Фильтр применяется ДО формирования students[] в AlbumInput
#      • Engine (lib/album-builder/*) НЕ трогать — он остаётся чистым.
#      • Новые тесты (~5 кейсов) на build-album-input:
#        1. Все is_purchased=true → все попадают (бэк-совместимость).
#        2. is_purchased=false + include_non_purchasers=false →
#           отфильтрованы (строгий режим).
#        3. is_purchased=false + include_non_purchasers=true →
#           все попадают (мягкий режим).
#        4. Mixed: 22 true + 3 false, строгий режим → 22 в students.
#        5. Mixed: 22 true + 3 false, мягкий режим → 25 в students.
#      • Файл тестов: lib/smart-fill/__tests__/build-album-input.test.ts
#        (если ещё нет — создать; если есть — расширить).
#      Дальше — 25.4 (UI фотографа), 25.5 (UI родителя + CSV),
#      25.6 (счётчик), 25.7 (summary).
#      Все подэтапы расписаны в docs/phase-Р25-spec.md §6 и §7.
