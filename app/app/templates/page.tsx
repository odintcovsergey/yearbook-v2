/**
 * РЭ.24.5: страница каталога шаблонов для партнёра /app/templates.
 *
 * Назначение:
 * - Партнёр видит две секции:
 *     1. «Готовые от OkeyBook» — глобальные шаблоны (is_recommended=true)
 *        с кнопкой «Сохранить в мои».
 *     2. «Мои шаблоны» — личная библиотека, с кнопками «Редактировать»
 *        и «Удалить», + кнопка «Создать свой шаблон».
 *
 * Карточка содержит:
...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    api('/api/auth')
      ...
      .then((d: AuthData | null) => {
        ...
        // /app/templates доступен для всех партнёров (включая viewer
        // в режиме чтения — без кнопок действий).
  ...
  const loadAll = useCallback(async () => {
    ...
    setError(null)
    try {
      const [globalResp, myResp] = await Promise.all([
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Готовые шаблоны от OkeyBook и ваша личная библиотека.
              Шаблон описывает структуру альбома: какие разделы и как они
              выглядят. При создании альбома выберете один из шаблонов.
            </p>
          </div>
          <button
