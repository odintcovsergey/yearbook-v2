...
# Версия: v75
# Обновлено: 18.05.2026 (РЭ.20.4 — модуль album-structure-matrix
#                       + интеграция findMatrixEntry в buildContext)
#
...
#      • lib/rule-engine/album-structure-matrix.ts — findMatrixEntry,
#        parseCellToPattern, mandatory/additionalSectionPatternsFor,
#        resolveAlternative.
#      • 41 unit-тест (200/200 общий счёт).
#      • buildContext инициализирует pages_remaining + mandatory_section
#        когда preset.density+sheet_type заданы И есть entry в матрице.
#      • Известные ограничения парсинга (зафиксированы в коде и тестах):
#         — '1/4 класса' одиночка → не покрыт PagePattern, в alternative
#           игнорируется. Расширим в РЭ.20.6 при необходимости.
#         — 'до N фото учеников + 1 общая' (personal_final) → парсится
#           как full_one, но эти ячейки имеют отдельную семантику и
#           парсятся другим API в РЭ.20.6.
#   ⏳ Дальше: РЭ.20.5 (loader/migrate существующих 7 пресетов: проставить
#      total_pages/density/sheet_type + ЗАВЕСТИ mini-hard), затем РЭ.12 (UI),
#      РЭ.20.6 (правила generator). Осталось ~18-22 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v75
#   3. Открыть docs/phase-Р20-spec.md — это план работы
#   4. РЭ.20.2 (миграция БД) — применена 18.05.
#      РЭ.20.3 (типы) — готова.
#      РЭ.20.4 (album-structure-matrix модуль + интеграция) — готова.
#   5. Следующий шаг — РЭ.20.5: проставить реальные total_pages/density/
#      sheet_type для 7 существующих глобальных пресетов + завести
#      mini-hard. Требует решений Сергея:
#        — какие total_pages у каждого пресета (формула? фикс?)
#        — какой density для 'maximum' пресета (standard? отдельный?)
#        — mini-hard: total_pages, состав sections?
#      Делается отдельной data-миграцией (INSERT mini-hard + UPDATE
#      остальных) + seed-обновлением в коде.
#
# 📍 КАК ПРОБОВАТЬ КОНТЕНТ-РЕДАКТОР НА ЖИВОМ АЛЬБОМЕ:
#   1. Открой любой собранный альбом → 'Открыть редактор'
#   2. Клик левой кнопкой по любому фото (одиночный, не drag)
...
#      + sheet_type в presets. Применена в Supabase.
#   ✅ РЭ.20.3 (45848e2, 18.05.2026) — расширение TypeScript типов.
#   ✅ РЭ.20.4 (f0d9c23, 18.05.2026) — модуль album-structure-matrix
#      + интеграция в buildContext (initial state). 41 unit-тест,
#      200/200 общий счёт.
#   1. РЭ.20.5 — loader/migrate существующих 7 пресетов:
#      • UPDATE total_pages, density, sheet_type для всех пресетов
#      • INSERT mini-hard (нет в БД, копия mini-soft с sheet_type='hard')
#      • Решения от Сергея по конкретным значениям (см. открытые
#        вопросы в phase-Р20-spec.md §10).
#   2. РЭ.12.A — UI /app/presets список + create form
#   3. РЭ.12.Б — UI редактор пресета (total_pages, sections, sheet_type)
#   4. РЭ.20.6 — генератор правил из xlsx-матрицы + декремент cursor'ов
#      mandatory_section в advanceCursors. Удаление @deprecated
#      common_section_max_spreads после доказательства что новая логика
#      работает.
#   5. РЭ.20.7 — unit-тесты для каждой строки матрицы
