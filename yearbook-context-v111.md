...
# Версия: v111
# Обновлено: 20.05.2026 (после РЭ.24.3 — утилита buildPresetPreviewBundle).
#
# ФАЗА РЭ.24 — Готовые шаблоны для быстрого старта партнёра (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.24.0 (614e6a7) — spec v1.0 (227 строк).
...
#    lib/presets/preview-bundle.ts: чистая функция
#      buildPresetPreviewBundle(bundle) → {students, cover, teachers, soft}
#    Двухпроходный алгоритм:
#      1) Cover ищется напрямую по page_role='cover' (не через engine)
#      2) Dry-run buildFromSectionStructure → распределение страниц
#         по family_id из decision_trace → первый из корзины
#      3) Fallback: пустые поля → поиск по page_role в template_set
#    Никогда не бросает исключения. 395/395 vitest passing (+8).
#
# ⏳ Следующий подэтап: РЭ.24.4 — API endpoints.
#    В app/api/tenant/route.ts добавить 5 action'ов:
#      • templates_list_global (GET) — глобальные is_recommended=true
#        с превью. Только для авторизованных партнёров.
#      • templates_list_my (GET) — все шаблоны с tenant_id=auth.tenantId
#        с превью + флаг valid из validatePreset.
#      • template_clone (POST) — клонирует глобальный в свой tenant_id.
#        Body: { template_id }. Возвращает id новой копии.
#        Записывает parent_preset_id = template_id.
#      • template_create_blank (POST) — создаёт пустой шаблон с
#        tenant_id партнёра. Минимальные поля: display_name. Остальное
#        партнёр доработает через существующий PresetEditorModal.
#      • template_delete (POST) — удаление шаблона партнёра.
#        Проверка: НЕТ активных альбомов с этим preset_id → удаляем,
#        иначе 409 со списком альбомов.
#    Tenant-aware строго. Глобальные read-only для не-superadmin.
#
