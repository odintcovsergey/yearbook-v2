...
# Версия: v125
# Обновлено: 21.05.2026 (после РЭ.25.7 — ФАЗА РЭ.25 ЗАКРЫТА).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v125
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v125 — РЭ.25 ЗАКРЫТА (8 подэтапов, 14 коммитов).
#      • docs/phase-Р25-summary.md — полный summary фазы (210 строк).
#      • docs/phase-Р24-spec.md §13 — архитектурный долг РЭ.27
#        (print_type должен жить в альбоме, не в пресете).
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#   5. СТАРТ ФАЗЫ РЭ.27 — print_type в АЛЬБОМ + двойная модель редактора.
#      БОЛЬШАЯ ФАЗА ~10-15 коммитов. Spec по образцу phase-Р22/Р23/Р24/Р25.
#      Подэтап 27.0: написать docs/phase-Р27-spec.md.
#      Перед началом — обсудить с Сергеем архитектурные развилки:
#        • Где живёт print_type на новом пути — поле albums.print_type
#          или вычисляется из пресета как fallback?
#        • Как мигрировать существующие альбомы (заполнить из preset.print_type)?
#        • Какие два режима редактора:
#          - layflat → разворотный (две страницы как одна зона работы)
#          - soft → постраничный + правила «первая правая, последняя левая»
#        • Как переключение между режимами в существующих альбомах
#          (после миграции) — автоматически по print_type или ручной toggle?
#        • Что с типографскими правилами soft (титульная правая, последняя
#          левая) — engine их сейчас знает или нужно добавить?
#        • Что с двойным набором пресетов «Стандарт-layflat» / «Стандарт-soft» —
#          сливаем в один «Стандарт» с выбором листов в альбоме?
#      После согласования развилок — spec, план подэтапов, реализация.
#
#      Альтернатива: РЭ.28 (партнёрские дизайны сценарий A) — 5-7 коммитов,
#      проще. См. docs/phase-Р24-spec.md §14.
#
#      Альтернатива: пауза, тестирование РЭ.25 на боевых альбомах.
