...
# Версия: v74
# Обновлено: 18.05.2026 (РЭ.20.3 — расширение TypeScript типов:
#                       Preset.total_pages/density/sheet_type +
#                       PagePattern + SheetType + RuleContext.mandatory_section)
#
...
#      в presets (3 колонки + 3 check-constraint'a). 18.05.2026.
#      Применена в Supabase, проверено SELECT'ом: 7 пресетов, все
#      total_pages=24, density=NULL, sheet_type=NULL.
#      ⚠️ В БД нет 'mini-hard' пресета (только mini-soft). Подтверждено
#      Сергеем: Мини бывает на плотных листах. В РЭ.20.5 завести
#      mini-hard как отдельный пресет (копия mini-soft с sheet_type='hard',
#      возможно другим total_pages).
...
#      • RuleContext.pages_remaining + mandatory_section (optional, заполняются в РЭ.20.4)
#      • RulesAlbumInput.common_section_max_spreads → @deprecated (не удалён)
#      • loaders.ts: presetRowToPreset читает 3 новые колонки из БД
#      • Все 159 unit-тестов проходят, tsc и next build чистые
#   ⏳ Дальше: РЭ.20.4 (build.ts читает total_pages, заполняет mandatory_section),
#      РЭ.20.5 (loader/migrate существующих пресетов: проставить реальные
#      total_pages/density/sheet_type, ЗАВЕСТИ mini-hard), затем РЭ.12 (UI).
#      Осталось ~20-25 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v74
#   3. Открыть docs/phase-Р20-spec.md — это план работы
#   4. РЭ.20.2 миграция БД применена в Supabase (15add1, проверено 18.05).
#   5. РЭ.20.3 типы расширены (45848e2). Preset/RuleContext готовы.
#   6. Следующий шаг — РЭ.20.4: расширить lib/rule-engine/build.ts
#      чтобы buildContext заполнял pages_remaining и mandatory_section
#      на основе loaded preset (total_pages/density/sheet_type) и
#      строки матрицы для текущего students_count.
#      Спека: docs/phase-Р20-spec.md §3.4.
#
# 📍 КАК ПРОБОВАТЬ КОНТЕНТ-РЕДАКТОР НА ЖИВОМ АЛЬБОМЕ:
#   1. Открой любой собранный альбом → 'Открыть редактор'
#   2. Клик левой кнопкой по любому фото (одиночный, не drag)
...
#   ✅ РЭ.20.2 (715add1, 18.05.2026) — миграция БД: total_pages + density
#      + sheet_type в presets. 3 колонки + 3 check'a. Аддитивная.
#      Применена в Supabase, проверено SELECT'ом.
#   ✅ РЭ.20.3 (45848e2, 18.05.2026) — расширение TypeScript типов:
#      Preset.total_pages/density/sheet_type + SheetType + PresetDensity
#      + PagePattern + RuleContext.pages_remaining + mandatory_section.
#      common_section_max_spreads → @deprecated. loaders.ts читает новые
#      поля. 159/159 тестов проходят.
#   1. РЭ.20.4 — расширение build.ts: buildContext заполняет
#      pages_remaining + mandatory_section из preset.total_pages и
#      строки матрицы.
#   2. РЭ.20.5 — loader/migrate существующих пресетов: проставить
#      реальные значения total_pages/density/sheet_type через SQL UPDATE
#      + ЗАВЕСТИ mini-hard (отсутствует в БД, Сергей подтвердил что
#      Мини существует и на плотных листах).
#   3. РЭ.12.A — UI /app/presets список + create form
#   4. РЭ.12.Б — UI редактор пресета (total_pages, sections, sheet_type)
#   5. РЭ.20.6 — генератор правил из xlsx-матрицы
#   6. РЭ.20.7 — unit-тесты для каждой строки матрицы
#   7. РЭ.20.8 — финальная документация
#
