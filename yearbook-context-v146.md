...
# Версия: v146
# Обновлено: 21.05.2026 (этап Б закрыт — все 3 миграции данных
# применены, остался этап В по UI).
#
# ⏳ ФАЗА РЭ.30 ИДЁТ — чистка путей сборки альбома (3/7 коммитов)
# ──────────────────────────────────────────────────────────────────
# Spec: docs/phase-Р30-spec.md (283 строки, commit 8d5b996)
# Диагностика: docs/phase-Р30-diagnostic.md (commit fe3ce67)
# Следующий шаг: В.1 (убрать селект Section Structure из обзора
# альбома — `app/app/page.tsx` ~строка 1122, константа `SS_PRESETS`).
#
# ⚠️ ДЕЙСТВИЕ ОТ СЕРГЕЯ ПЕРЕД В.1 (или параллельно):
# Применить миграцию `migrations/2026-05-21-presets-cleanup-mixed.sql`
# в Supabase SQL Editor. Идемпотентно. Проверка:
...
# Ожидание: 0.
#
# ✅ Состояние данных после применения Б.1 в Supabase (21.05.2026):
...
#           применена в Supabase 21.05.2026)
#   ✅ Б.2 — PresetEditorModal не пишет density/sheet_type
#           (commit 73cd5ed)
#   ✅ Б.3 — SQL миграция: cleanup смешанных пресетов (commit bb0d7f9,
#           требует применения в Supabase)
#   ⏳ В.1 — убрать селект Section Structure из обзора альбома
#   ⏳ В.2 — убрать поле «Плотность портретов» из редактора шаблона
#   ⏳ В.3 — убрать блок «Комплектация + Тип печати» из формы альбома
#   ⏳ Г.1 — summary + закрытие фазы
#
# Этап Б (данные) закрыт. Дальше — этап В (UI). Это ДЕСТРУКТИВНАЯ
# часть фазы — каждый подэтап меняет видимое поведение UI.
# После каждого коммита — обязательный ручной тест Сергея.
#
