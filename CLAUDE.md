...
- **Перед каждым коммитом** объяснять простыми словами что сделано и зачем, **называть риски** простым языком, и **спрашивать подтверждение**. Никогда не коммитить молча. После «да» от Сергея — `git commit` и `git push` делаются сразу, одной операцией. Второго OK на push не ждать (Сергей работает один, лишнее подтверждение тормозит).
...
**Commit + push:** перед коммитом объяснить простыми словами что сделано + риски + спросить OK. После «да» — `git add && git commit && git push` одной операцией, второго подтверждения не запрашивать. Если pre-commit hook падает или возникают неожиданные изменения — остановиться, объяснить, дождаться нового OK.
...
Локальная резервная база знаний в `knowledge/` (на диске + в Git, два зеркала).
Структура: `README.md`, `PROJECT_CONTEXT.md` (текущее состояние), `DECISIONS.md`
(ключевые решения), `RECOVERY.md` (восстановление с нуля), `sessions/YYYY-MM-DD.md`
(лог по датам).

**Правило конца сессии.** В конце каждой рабочей сессии Claude Code:
1. Обновляет `knowledge/PROJECT_CONTEXT.md` (актуальное состояние проекта).
2. Дописывает/создаёт `knowledge/sessions/<сегодня YYYY-MM-DD>.md` (что сделано за сессию).
3. Коммитит и пушит в **оба зеркала** ОТДЕЛЬНЫМ коммитом:
   `docs(knowledge): обновление после сессии <дата>`.

Это ручное действие силами Claude Code (не cron) — выполняется только когда идёт
работа. Секреты/токены/ключи/содержимое `.env*` в `knowledge/` НЕ писать (в
`RECOVERY.md` — только названия переменных). Если за сессию менялись архитектурные
решения — отразить в `DECISIONS.md`.
