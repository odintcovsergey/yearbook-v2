...
# Версия: v128
# Обновлено: 21.05.2026 (после корректировки spec + миграции 27.1
# в связи с открытием existing-колонки albums.print_type).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0 (spec) и 27.1 (индекс + документирование) закрыты.
# Следующий — 27.2 (API: разрешить явный print_type).
# Spec: docs/phase-Р27-spec.md v1.0 (с §2.0 после открытия).
# Последний commit фазы: 5a52544 (корректировка).
#
# ✅ Миграция РЭ.27.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026).
#    Проверка: колонка albums.print_type есть, индекс idx_albums_print_type
#    создан, CHECK constraint существует с 8 мая 2026.
#
# ⚠️ ОТКРЫТИЕ ПРИ СТАРТЕ 27.1 (зафиксировано в spec §2.0):
#   • Колонка albums.print_type СУЩЕСТВУЕТ С 8 МАЯ 2026
#     (миграция 1.0 — migrations/2026-05-08-album-config.sql).
#   • CHECK constraint ('layflat'|'soft') УЖЕ есть.
#   • API create_album УЖЕ копирует preset.print_type в albums.print_type
#     при создании (строка 3012 в app/api/tenant/route.ts).
#
#   Распределение значений в проде на старте РЭ.27:
#     layflat: 4 альбома (новые, после копирования)
#     soft:    2 альбома (новые)
#     NULL:    6 альбомов (старые, до появления копирования)
#
#   При этом ENGINE ВСЁ ЕЩЁ ЧИТАЕТ print_type ИЗ ПРЕСЕТА — основная
#   работа фазы (resolvePrintType в 27.3) остаётся в силе.
#
# Что в итоге сделано в 27.1:
#   • ADD COLUMN IF NOT EXISTS — no-op (колонка была).
#   • COMMENT обновлён с привязкой к РЭ.27.
#   • CREATE INDEX IF NOT EXISTS idx_albums_print_type — ✅ полезно,
#     индекса не было, создан.
#   • Миграция не откатывается — она документирует и создаёт индекс.
#
# Что меняется в плане:
#   • 27.2 теперь точнее: разрешить ЯВНО передавать print_type
#     в create_album (сейчас только копируется из пресета) и
#     update_album (сейчас вообще нельзя менять).
#   • 27.7: заполнение 6 NULL-альбомов (число известно).
#
...
#   ⏳ 27.2 — API: явный print_type в create_album и update_album
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v128
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v128 — РЭ.27.0 и РЭ.27.1 закрыты + ОТКРЫТИЕ existing-колонки
#      • docs/phase-Р27-spec.md §2.0 — что уже было в коде до фазы
#      • docs/phase-Р27-spec.md остальное — для общего контекста
#      • docs/phase-Р25-summary.md — последняя закрытая фаза
#   4. Применены в Supabase: все РЭ.22..РЭ.25.1 + РЭ.27.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.2 (API, точнее):
#      Текущее поведение (что УЖЕ есть в коде):
#      • create_album копирует preset.print_type в albums.print_type
#        (app/api/tenant/route.ts строка 3012, переменная presetPrintType).
#      • update_album НЕ принимает print_type (надо добавить в allowedFields).
#      • Engine читает print_type из ПРЕСЕТА, не из альбома (это 27.3).
#
#      Что нужно сделать в 27.2:
#      • create_album: дать партнёру явно передать print_type:
#        - если body.print_type === 'layflat'|'soft' → используем явное
#          значение (приоритет над пресетом).
#        - если не передано → fallback на presetPrintType (текущее
#          поведение, бэк-совместимость).
#        - валидация на уровне БД (CHECK constraint) уже работает.
#      • update_album: добавить 'print_type' в allowedFields.
#        - Если меняется значение и в альбоме есть spread-мастера —
#          warning или confirmation на фронте (это уже 27.6, но
#          API должен быть готов).
#      • SELECT'ы пресетов: print_type становится менее важным,
#        но пока не убираем (engine ещё его читает).
#      • TypeScript типы:
#        - Album.print_type?: 'layflat' | 'soft' | null
#        - В app/app/page.tsx локальные типы тоже расширить.
#
#      Дальше — 27.3 (engine), 27.4 (превью), 27.5 (UI каталога),
#      27.6 (UI формы), 27.7 (миграция данных), 27.8 (summary).
