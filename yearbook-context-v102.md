...
# Версия: v102
# Обновлено: 20.05.2026 (после РЭ.22.8.2 — engine soft семантика).
#
# 🎉 ФАЗА РЭ.22 — ВСЕ СЕКЦИИ ПЕРЕВЕДЕНЫ НА СЕМАНТИЧЕСКИЙ ПОИСК
# ─────────────────────────────────────────────────────────────────
# Двух-осевая модель «режим × параметры» — students секция:
#   - mode='page'   — ✅ РЭ.22.4 (Standard/Universal)
#   - mode='spread' — ✅ РЭ.22.5 (Maximum/Individual)
#   - mode='grid'   — ✅ РЭ.22.6 (Medium/Light/Mini)
# Teachers секция:    ✅ РЭ.22.7.2
# Soft секции:        ✅ РЭ.22.8.2 (intro + final)
#
# ✅ РЭ.22.8.2 (a4c1d51) — engine soft_intro/soft_final семантический.
#    - lib/album-builder/types.ts: PageRole + 'final' (синхронизация
#      TS-типа с CHECK constraint расширенным в РЭ.22.8.1).
#    - lib/rule-engine/master-finder.ts: новая findSoftSectionMaster —
#      простой first-match по page_role + опц. точный photos_full.
#    - lib/rule-engine/sections/soft-intro.ts: семантика(intro,
#      photos_full=1) → fallback 'S-Intro'. decision_trace.rule_id с
#      реальным именем мастера, inputs.semantic.
#    - lib/rule-engine/sections/soft-final.ts: семантика(final,
#      photos_full=1) → fallback 'S-Final' → 'S-Final-Soft-L'.
#    - 11 новых тестов sections-soft-semantic.test.ts.
#    - 360/360 vitest passing (было 349/349).
#
# ⏳ Следующий подэтап: РЭ.22.9 — диагностика недостающих мастеров.
#    После всех семантических переходов в engine стало много типов
#    warning со спецификацией нужных slot_capacity тегов (например
#    students_master_not_found с photos_friend=4, has_quote=true).
#    Идея РЭ.22.9: добавить:
#      - Опциональный UI-просмотр (в /super/presets или /super/templates)
#        который для выбранного пресета сразу показывает «каких мастеров
#        не хватает в template_set». До нажатия «Собрать альбом».
#      - Возможно агрегировать warning'и в более удобный JSON-формат
#        (сейчас текстовые строки).
#    Это финальный содержательный подэтап. После идёт РЭ.22.10 —
#    docs/phase-Р22-summary.md и закрытие фазы.
#
# Скоуп РЭ.22: students (✅) + teachers (✅) + soft_intro (✅) + soft_final (✅).
# common_required / common_additional / transition НЕ трогаем — уже
# работают через эталонную таблицу OkeyBook (РЭ.21.8.9-11).
#
# ─── Скрытые баги фазы (все закрыты по ходу) ─────────────────────────
# • Баг #1 (закрыт РЭ.22.2): loaders.ts не пробрасывал student_* поля
# • Баг #2 (известная коллизия L-Grid-12 vs N-Grid-Page, развилка D.1)
# • Баг #3 (закрыт РЭ.22.7.2): legacy ищет G-Teachers-4x3, в БД G-Teachers-3x4
# • Баг #4 (закрыт РЭ.22.8.1 fix): CHECK constraint не содержал 'final',
#   первая попытка применения миграции откатилась, фикс в коммите 5187361
#
# Все 10 архитектурных развилок решены и зафиксированы в spec §11.
# Подробности: docs/phase-Р22-spec.md (564 строки).
#
...
#   15a3303 (Р.2 — поворот фото ±45° для исправления горизонта),
...
#
#   ⏳ Следующий подэтап: РЭ.22.9 — диагностика недостающих мастеров
#      Опциональный UI-просмотр в /super который для выбранного пресета
#      показывает «каких мастеров не хватает в template_set» до сборки
#      альбома. Возможно агрегация warning'ов в JSON. После придёт
#      РЭ.22.10 (summary + закрытие фазы).
#
#   Полный план (10 подэтапов, см. docs/phase-Р22-spec.md §10):
#     ✅ РЭ.22.1  — миграция БД (student_layout_mode + student_grid_size)
#     ✅ РЭ.22.2  — типы (Preset) + API валидация в rule_preset_update
#     ✅ РЭ.22.3  — UI: двух-осевая модель в PresetEditorModal
#     ✅ РЭ.22.4  — engine semantic для mode='page' (Standard/Universal)
#     ✅ РЭ.22.5  — engine semantic для mode='spread' (Maximum/Individual)
#     ✅ РЭ.22.6.0 — data-миграция grid-мастеров (page_role + slot_capacity)
#     ✅ РЭ.22.6  — engine semantic для mode='grid' (Medium/Light/Mini)
#     ✅ РЭ.22.7.1 — data-миграция teachers-мастеров
#     ✅ РЭ.22.7.2 — engine teachers семантический поиск
#     ✅ РЭ.22.8.1 — data-миграция soft_intro/soft_final мастеров
#     ✅ РЭ.22.8.2 — engine soft_intro/soft_final семантический поиск
#     ⏳ РЭ.22.9  — диагностика недостающих мастеров (warning + опц UI-сводка)
#        РЭ.22.10 — финал, docs/phase-Р22-summary.md, контекст v(N)
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v102
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v102 — что закрыто (ВСЕ 4 секции переведены)
#      • docs/phase-Р22-spec.md v1.0 — §6.9 (диагностика).
#      • Сводный список warning-форматов которые engine генерирует
#        (для понимания что агрегировать):
#          - students_master_not_found (page + spread + grid)
#          - students_grid_tail_padded (если использован null-fallback)
#          - students_grid_size_missing
#          - students_lost_photos
#          - teachers_master_not_found
#          - teachers_right_empty
#          - soft_intro_master_not_found
#          - soft_intro_skipped / soft_final_skipped
#          - soft_final_master_not_found
#   4. Применены в Supabase (все 4 миграции фазы):
#      • Все миграции до v87 ✅
#      • 2026-05-19-presets-student-layout-fields.sql ✅
#      • 2026-05-19-okeybook-default-student-master-tags.sql ✅
#      • 2026-05-20-albums-drop-rules-preset-id.sql ⬜ НЕ применена (не блокер)
#      • 2026-05-20-presets-student-layout-mode.sql ✅ (РЭ.22.1)
#      • 2026-05-20-okeybook-grid-master-tags.sql ✅ (РЭ.22.6.0)
#      • 2026-05-20-okeybook-teacher-master-tags.sql ✅ (РЭ.22.7.1)
#      • 2026-05-20-okeybook-soft-master-tags.sql ✅ (РЭ.22.8.1 + fix)
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.9 — диагностика недостающих мастеров.
#      Идея: для выбранного пресета можно ЗАРАНЕЕ (до сборки альбома)
#      сказать каких мастеров не хватает в template_set. Сейчас warning
#      выдаётся только при попытке сборки.
#      Возможные варианты реализации:
#        Вариант А (минимум): добавить в /super/presets кнопку
#          «Проверить пресет» которая делает dry-run engine'а с
#          фиктивными данными (10 учеников, 5 предметников, 1 фото класса)
#          и показывает список warning'ов из result.warnings.
#        Вариант Б: вычислять список нужных мастеров статически из пресета
#          (без engine) — пройтись по section_structure, для каждой секции
#          вывести требуемые page_role + slot_capacity, сравнить с тем
#          что есть в template_set.
#        Вариант В: продумать только улучшение формата warning'ов (более
#          структурированный JSON, добавить severity), UI оставить на потом.
#      Сначала обсудить с Сергеем какой вариант предпочесть.
#      Скоуп решит сам — может быть один атомарный коммит, может несколько.
