...
# Версия: v119
# Обновлено: 21.05.2026 (после РЭ.25.1 — миграция БД готова и закоммичена).
#
# ⏳ ФАЗА РЭ.25 В РАБОТЕ — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтапы 25.0 (spec) и 25.1 (миграция БД) закрыты.
# Следующий — 25.2 (API + типы).
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: b8f671f (РЭ.25.1 миграция).
#
# ⚠️ ВНИМАНИЕ Сергею: миграция РЭ.25.1 ещё НЕ ПРИМЕНЕНА в Supabase.
#    Применить ДО или ОДНОВРЕМЕННО с деплоем кода 25.2:
#    migrations/2026-05-21-children-is-purchased-and-album-include-non-purchasers.sql
#    Миграция аддитивная (NOT NULL DEFAULT) — старый код не падает,
#    но новый код 25.2 будет ожидать наличия колонок.
#
...
#   ✅ 25.1 — миграция БД (commit b8f671f)
#              ⚠️ ПРИМЕНИТЬ В SUPABASE перед деплоем 25.2
#   ⏳ 25.2 — API + типы (update_child action, расширения create/update_album,
#             set_purchased в /api/select)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v119
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v119 — РЭ.25.0 и РЭ.25.1 закрыты. Фаза РЭ.25 в работе.
...
#      ⚠️ ПРОВЕРИТЬ ПРИМЕНИЛ ЛИ СЕРГЕЙ миграцию РЭ.25.1:
#        migrations/2026-05-21-children-is-purchased-and-album-include-non-purchasers.sql
#      Если не применена — код 25.2 после деплоя упадёт при SELECT/UPDATE
#      колонок is_purchased / include_non_purchasers.
#      Проверочный запрос в Supabase SQL Editor:
...
#      Ожидание: две строки. Если ноль — миграция не применена.
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.2 (API + типы):
#      • Новый action POST /api/tenant: update_child
#        - Принимает { child_id, full_name?, class?, is_purchased? }
#        - Tenant-aware через assertChildAccess(auth, child_id)
#        - Аудит-лог child.update с указанием изменённых полей
#        - Если все patch-поля undefined → 400 'нечего обновлять'
#      • Расширение POST /api/tenant: create_album и update_album
#        - Опциональное поле include_non_purchasers: boolean
#        - В create_album дефолт false если не передано
#        - В update_album если undefined — поле не трогается
#      • Новый action PUT /api/select: action='set_purchased'
#        - Принимает { token, action:'set_purchased', value:boolean }
#        - Token-аутентификация ребёнка (уже работает в /api/select)
#        - Доступно ВСЕГДА, даже при submitted_at !== null
#      • TypeScript типы (если нужно) — обновить запросы children.*
#        чтобы возвращали новые поля
#      Дальше — 25.3 (engine-фильтр), 25.4 (UI фотографа), 25.5 (UI
#      родителя + CSV), 25.6 (счётчик), 25.7 (summary).
