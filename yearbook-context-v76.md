...
# Версия: v76
# Обновлено: 18.05.2026 (РЭ.20.5 — data-миграция: density + sheet_type
#                       для 7 пресетов + создан mini-hard)
#
...
#      • INSERT mini-hard как копия mini-soft с print_type='layflat'
#        и sheet_type='hard'. Sections идентичны mini-soft (Сергей
#        подтвердил: отличие только в начальной/конечной странице).
#      • total_pages везде остался DEFAULT 24 — сезонные изменения,
#        фиксировать рано. Партнёр поставит через UI в РЭ.12.
#      ⚠️ Применить вручную в Supabase SQL Editor:
#      migrations/2026-05-18-presets-density-sheet-type-values.sql
#   ⏳ Дальше: РЭ.12 (UI редактор пресетов), затем РЭ.20.6 (генератор
#      правил + декремент cursor'ов). Осталось ~16-20 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v76
#   3. Открыть docs/phase-Р20-spec.md — это план работы
#   4. РЭ.20.2 (схема БД), РЭ.20.3 (типы), РЭ.20.4 (модуль матрицы),
#      РЭ.20.5 (data-миграция) — готовы.
#   5. Этап А+Б РЭ.20 закрыт. Дальше — выбор:
#      (а) РЭ.12 (UI редактор пресетов /app/presets) — даст партнёрам
#          возможность настраивать total_pages и копировать пресеты.
#      (б) РЭ.20.6 (генератор правил из матрицы) — даст полную работу
#          mandatory_section для 6 пресетов с density.
#      Рекомендация: сначала РЭ.20.6, т.к. без правил mandatory_section
#      в РЭ.20.4 заполняется но не используется build engine'ом.
#
# 📍 КАК ПРОБОВАТЬ КОНТЕНТ-РЕДАКТОР НА ЖИВОМ АЛЬБОМЕ:
#   1. Открой любой собранный альбом → 'Открыть редактор'
#   2. Клик левой кнопкой по любому фото (одиночный, не drag)
...
#          применить вручную; UPDATE + INSERT mini-hard)
...
#   ✅ РЭ.20.2 (715add1, 18.05.2026) — миграция БД. Применена в Supabase.
#   ✅ РЭ.20.3 (45848e2, 18.05.2026) — расширение TypeScript типов.
#   ✅ РЭ.20.4 (f0d9c23, 18.05.2026) — модуль album-structure-matrix.
#   ✅ РЭ.20.5 (37029b3, 18.05.2026) — data-миграция значений + mini-hard.
#      ⚠️ Применить в Supabase SQL Editor:
#      migrations/2026-05-18-presets-density-sheet-type-values.sql
#   1. РЭ.20.6 — генератор правил из матрицы + декремент cursor'ов
#      mandatory_section в advanceCursors. Это активирует mandatory_section
#      в build engine'е (сейчас она заполняется но не читается).
#      Удаление @deprecated common_section_max_spreads после доказательства
#      что новая логика работает.
#   2. РЭ.20.7 — unit-тесты для каждой строки матрицы.
#   3. РЭ.12.A — UI /app/presets список + create form.
#   4. РЭ.12.Б — UI редактор пресета (total_pages — главное! партнёр
#      выставляет под свой альбом, без правок БД).
