...
# Версия: v82
# Обновлено: 18.05.2026 (РЭ.21.5.1 + РЭ.21.5.2:
#   диапазон min_pages/max_pages вместо одиночного total_pages.
#   Миграция БД (nullable колонки) + типы + API + форма создания.
#   Старые 7 пресетов с NULL — фолбэк на total_pages работает.)
#
...
#   ⚠️ Подключение section_structure к build engine — самый главный шаг.
#      Сейчас engine работает по legacy priority-правилам. Когда у пресета
#      есть section_structure → engine должен идти по нему слот за слотом
#      с применением цепочек flex_A/B/C.
#
...
#   1. РЭ.21.5.3 — DROP COLUMN total_pages.
#      Маленький шаг: миграция + удалить упоминания total_pages в коде
#      (типы, loaders, build engine, тесты, API insert/select).
#      Делать только когда Сергей подтвердит что РЭ.21.5.2 работает на
#      проде без регрессий и проставил конкретные диапазоны для 7 встроенных.
#   2. РЭ.21.6 — template_set_id у пресета.
#      Миграция БД + select в форме создания/редактирования.
#      Партнёр выбирает дизайн из доступных template_set (на запуск —
#      только глобальные OkeyBook'овские).
#   3. РЭ.21.7 — редактор структуры пресета.
#      Кнопка «Редактировать» на карточке. Drag-and-drop секций + слотов.
#      POST action rule_preset_update.
#   4. РЭ.21.8 — подключение section_structure к build engine.
...
#   5. РЭ.21.9 — копирование глобального пресета как стартового.
#      Кнопка «Скопировать как мой» на карточке глобального пресета.
#   6. РЭ.21.10 — финальная документация архитектуры.
#
# ## Параллельная задача для Сергея (вне разработки):
#   ⚠️ Проставить диапазоны min_pages/max_pages для 7 встроенных пресетов
#      через Supabase SQL Editor. Пример SQL — в разделе «SQL для дефолтов
#      диапазонов» этого файла (внизу).
#
