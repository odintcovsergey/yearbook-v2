...
# Версия: v89
# Обновлено: 20.05.2026 (после РЭ.21.8.чистка-1: удалён движок 2 buildFromRules.
#   Было 3 движка сборки альбома (legacy buildAlbum, buildFromRules,
#   buildFromSectionStructure), стало 2 (legacy + section_structure).
#   Удалены 12 файлов (~2000 строк), 132 теста движка 2, UI компоненты
#   RulesPresetControl + RulesEnginePreviewBlock + sandbox в /super/templates.
#   Подготовлена миграция БД на удаление albums.rules_preset_id колонки
#   (не применена). vitest: 247/247 passing. См. коммит 9124a96 + секцию
#   "ЧИСТКА ДВИЖКА 2" ниже.
#   Стратегически: подготовка к РЭ.22 — полному семантическому
#   конструктору пресетов с UI "как у фотобота". После чистки осталось
#   только 2 движка чтобы не путаться при больших изменениях в РЭ.22.)
#
...
#   2. Доработка редактора (шрифты, горизонт, автозаполнение)
...
#
# ─── Сессия 2 (СЛЕДУЮЩАЯ) — Доработка редактора ─────────────────────
#   Из озвученного Сергеем:
#   - Работа со шрифтами (цвет, размер) — сейчас нельзя поменять текст
#   - Горизонт фотографии — есть масштабирование/смещение, но не поворот
#   - Автозаполнение при смене мастера: если в редакторе подменить мастер
#     на другой, фотографии должны автоматически перенестись в новые
#     слоты (без ручного перетаскивания)
#   - Возможно ещё что найдём при копании в /app/album/[id]/layout
#   Объём: ~3-4 коммита, 2-3 сессии работы.
#
# ─── Фаза РЭ.22 — Конструктор пресетов (основная работа) ────────────
#   Что входит:
#   - Полный семантический поиск мастера для всех density:
#     • Standard / Universal — переработать buildAlternatingLR на
#       семантический поиск через findStudentMaster
#     • Maximum — расширить master-finder для двух-страничных режимов
#     • Medium / Light / Mini — переработать buildGrid на семантический
#       поиск с slot_capacity (4/6/9/12 учеников на странице + has_quote)
#   - Двух-осевая модель «режим × параметры» в UI PresetEditorModal:
#     • Режим: 1 ученик/страница, 1 ученик/разворот, сетка N учеников
#     • Параметры зависят от режима: фото с друзьями (для page/spread),
#       размер сетки (для grid)
#     • Цитата (has_quote) — зависит от выбранного режима, а не
#       independent поле как сейчас
#   - Аналогичная переработка teachers / soft_intro / soft_final секций
#     (common_required / common_additional / transition оставляем
#     как есть — они уже работают через семантический поиск таблицы OkeyBook)
#   - При недостающих мастерах engine выдаёт спецификацию slot_capacity
#     тегов которые нужны — Сергей рисует мастер в InDesign, загружает,
#     система автоматически его использует
#   - Полноценный UI конструктор «как у фотобота»
#   Объём: оценочно 2-3 недели работы, 6-10 коммитов.
#
...
#   - teachers — переработать на семантический поиск
#   - students — переработать полностью (основная часть РЭ.22)
#   - soft_intro / soft_final — переработать семантически
#   - Конструктор пресетов касается ВСЕХ разделов альбома (вступление,
#     учителя, личный, общий, финал, виньетки, обложка)
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v89
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v89 — что закрыто (РЭ.21.8 + чистка движка 2)
#      • Секция "ФАЗА РЭ.21.8 — ИТОГИ" — что сделано в 7 коммитах
#      • Секция "РЭ.21.8.чистка-1" — про удаление движка 2
#      • Секция "🎯 ЧТО ДАЛЬШЕ" — стратегическая карта 20.05.2026
#      • lib/rule-engine/album-structure-okeybook.ts — эталонная таблица
#        OkeyBook (26 строк × density × sheet_type)
#      • docs/templates/master-cleanup-tz.md раздел H — TZ на комбо-мастера
#   4. Применены в Supabase:
#      • Все миграции до v87 ✅
#      • 2026-05-19-presets-student-layout-fields.sql ✅ (РЭ.21.8.15,
#        с DO-блоком для CONSTRAINT IF NOT EXISTS из-за Postgres)
#      • 2026-05-19-okeybook-default-student-master-tags.sql ✅
#        (теги для 6 E-* мастеров)
#      • 2026-05-20-albums-drop-rules-preset-id.sql ⬜ НЕ применена
#        (не блокер — колонка просто остаётся в БД, не читается)
#   5. UI пресетов работает в /super/presets — суперадмин видит и
#      редактирует все 7 глобальных пресетов.
#   6. Архитектура движков ПОСЛЕ чистки 20.05.2026:
...
#   7. СЛЕДУЮЩАЯ СЕССИЯ: доработка редактора (шрифты, горизонт,
#      автозаполнение при смене мастера).
#      После — большая фаза РЭ.22 (конструктор пресетов).
#   8. ЯЗЫК Сергея (важно для тона):
