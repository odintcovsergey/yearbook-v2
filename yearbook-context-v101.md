...
# Версия: v101
# Обновлено: 20.05.2026 (после РЭ.22.8.1 — data-миграция soft-мастеров).
#
...
# ✅ РЭ.22.7.1+22.7.2 — teachers секция семантический поиск.
# ✅ РЭ.22.8.1 (6039415) — data-миграция soft-мастеров.
#    migrations/2026-05-20-okeybook-soft-master-tags.sql
#    S-Intro → page_role='intro', S-Final-Soft-L → page_role='final',
#    photos_full вычисляется динамически (=1 у обоих, есть classphotoframe).
#    Замечание: в БД нет 'S-Final' без суффикса — есть только
#    'S-Final-Soft-L'. Legacy всегда падает на fallback по имени.
#    ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.22.8.2.
#
# ⏳ Следующий подэтап: РЭ.22.8.2 — engine soft_intro/soft_final семантический.
#    - В master-finder.ts: новая findSoftSectionMaster (или общая утилита).
#      Параметры: pageRole='intro'|'final', опц. photosFull.
#    - В sections/soft-intro.ts: семантический поиск 'intro' → legacy
#      fallback 'S-Intro' → warning.
#    - В sections/soft-final.ts: то же для 'final' → fallback на
#      'S-Final' → 'S-Final-Soft-L' → warning.
#    - Тесты.
#
# Двух-осевая модель «режим × параметры» — students секция готова:
#   - mode='page'   — ✅ РЭ.22.4
#   - mode='spread' — ✅ РЭ.22.5
#   - mode='grid'   — ✅ РЭ.22.6
# Teachers секция готова: ✅ РЭ.22.7.2
# Soft секции (intro/final) — ⏳ РЭ.22.8.2
#
...
#
#   ✅ РЭ.22.8.1 (коммит 6039415) — data-миграция soft-мастеров.
#      migrations/2026-05-20-okeybook-soft-master-tags.sql:
#      S-Intro → page_role='intro', S-Final-Soft-L → page_role='final'.
#      photos_full вычисляется динамически через jsonb_array_elements
#      (по выгрузке: оба мастера имеют 1 classphotoframe → photos_full=1).
#      В БД мастера 'S-Final' без суффикса НЕТ — legacy всегда падает
#      на fallback по имени 'S-Final-Soft-L'. После РЭ.22.8.2 эта
#      гимнастика с двумя именами не нужна.
#      ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.22.8.2.
#
#   ⏳ Следующий подэтап: РЭ.22.8.2 — engine soft_intro/soft_final семантический
#      В master-finder.ts: новая findSoftSectionMaster (или общая утилита).
#      В sections/soft-intro.ts и soft-final.ts: семантика → legacy fallback
#      по имени → warning. Тесты.
#
...
#     ⏳ РЭ.22.8.2 — engine soft_intro/soft_final семантический
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v101
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v101 — что закрыто (включая РЭ.22.8.1 миграция)
#      • docs/phase-Р22-spec.md v1.0 — §6.6 (soft_intro / soft_final).
#      • migrations/2026-05-20-okeybook-soft-master-tags.sql — теги.
#      • lib/rule-engine/sections/soft-intro.ts (79 строк) — жёсткое
#        'S-Intro'. soft-final.ts — 'S-Final' → 'S-Final-Soft-L' fallback.
#      • lib/rule-engine/master-finder.ts — образец findTeacherMaster
#        для новой findSoftSectionMaster.
...
#      • 2026-05-20-okeybook-teacher-master-tags.sql ✅ ПРИМЕНЕНА (РЭ.22.7.1)
#      • 2026-05-20-okeybook-soft-master-tags.sql ⬜ РЭ.22.8.1 —
#        ПРИМЕНИТЬ ДО деплоя РЭ.22.8.2.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.8.2 — engine soft_intro/soft_final.
#      Файлы:
#        - lib/rule-engine/master-finder.ts: новая findSoftSectionMaster.
#          Параметры: pageRole='intro'|'final', опц. photosFull (0/1).
#          Простой точный поиск, без режимов min_fit (один мастер на роль).
#        - lib/rule-engine/sections/soft-intro.ts: переписать через
#          findSoftSectionMaster (pageRole='intro', photosFull=1).
#          Legacy fallback на 'S-Intro' по имени.
#        - lib/rule-engine/sections/soft-final.ts: аналогично
#          ('final' / photosFull=1). Legacy fallback на 'S-Final' →
#          'S-Final-Soft-L' (двойной fallback сохраняется для совместимости).
#        - lib/rule-engine/__tests__/sections-soft-intro-semantic.test.ts
#          и sections-soft-final-semantic.test.ts (новые) — по 3-5 тестов
#          каждый.
#      Что НЕ трогаем: students, teachers (закрыты).
