...
# Версия: v148
# Обновлено: 21.05.2026 (В.2 закрыт — поля «Плотность портретов» и
# «Тип листов» убраны из редактора пресета).
#
# ⏳ ФАЗА РЭ.30 ИДЁТ — чистка путей сборки альбома (5/7 коммитов)
# ──────────────────────────────────────────────────────────────────
# Spec: docs/phase-Р30-spec.md (283 строки, commit 8d5b996)
# Диагностика: docs/phase-Р30-diagnostic.md (commit fe3ce67)
# Следующий шаг: В.3 (убрать legacy блок «Комплектация + Тип печати»
# из формы создания/редактирования альбома — `app/app/page.tsx`).
#
...
#
# После В.2 партнёр в редакторе пресета видит ТОЛЬКО семантический
# выбор: Режим (page/spread/grid) + grid_size/friends + has_quote.
# Initial-хелперы упрощены — больше не fallback'ят по density (после
# Б.1 у всех глобальных layout_mode заполнен, для пустых партнёрских
# берётся 'page' + grid_size=4 как дефолт).
#
