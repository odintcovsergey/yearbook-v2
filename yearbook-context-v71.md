...
# Версия: v71
# Обновлено: 16.05.2026 (КЭ закрыта — контент-редактор фото с scale +
#                       offset работает end-to-end в редакторе и PDF.
#                       Снят главный блокер августовской партнёрки.)
#
...
#
# Главные блокеры закрыты: rule engine end-to-end в проде (РЭ) +
# контент-редактор фото в редакторе и PDF (КЭ). Партнёр МОЖЕТ собрать
# и опубликовать альбом.
#
# Оставшиеся работы (по убыванию приоритета):
#
#   1. Балансировка проверка end-to-end (1-2 коммита)
#      - Проверить __hidden__<label> реально скрывает placeholder
#        в Konva preview + PDF (lib/rule-engine/balance.ts уже есть,
#        нужна интеграционная проверка на боевом альбоме)
#      - Балансировка placeholder_centering: после hide_unfilled
#        видимые placeholder'ы центрируются (видимая часть мастера
#        выравнивается, не остаётся 'дыр' от скрытых слотов)
#
#   2. РЭ.19+РЭ.20 — добивка общего раздела (3-5 коммитов)
#      - common-section-spread-pair (J-Spread для двухстраничных)
#      - common-section-collage-4-pair (J-Collage-4)
#      - Fallback правила для остатков (single J-Full/J-Half/etc
#        когда категории не хватает на полный pair)
#
#   3. РЭ.11 — фильтр TemplatePickerModal по family_id (1-2 коммита)
#      В редакторе при замене мастера partner видит ВСЕ мастера; должен
#      видеть только подходящие по family_id/page_role.
#
#   4. РЭ.12 — UI редактор пресетов /app/presets (5-8 коммитов)
#      Главный пункт для запуска партнёрки. Сейчас правки только через
#      JSON в репо + seed. С UI редактором партнёр сможет копировать
#      глобальный пресет и подстраивать (drag-drop sections, переключение
#      density/has_quote/etc).
#
#   5. Параллельно июнь — переезд с Vercel (1-2 коммита Dockerfile +
#      GitHub Actions deploy на Timeweb / YC App Platform). Без этого
#      okeybook.ru остаётся за VPN.
#
# Реалистичная скорость: 2-3 коммита в неделю. Все 5 блоков влезают
