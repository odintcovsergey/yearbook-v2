...
# Версия: v63
# Обновлено: 16.05.2026 (РЭ.4+РЭ.5+РЭ.6+РЭ.7 закрыты — 36 правил в JSON, все валидны)
#
...
# Rule engine спецификация v1.3 готова — фундамент реализации buildFromRules
# для запуска партнёрки в сентябре.
# Подэтапы РЭ.1+РЭ.2+РЭ.3+РЭ.3.5+РЭ.4+РЭ.5+РЭ.6+РЭ.7 закодированы.
...
#    все валидны (Zod-схема + ссылочная целостность). ПОКА НЕ в БД —
#    UPSERT правил будет в РЭ.8.
# Состав 36 правил:
#   - head-teacher: 13 (subjects=0/1-4/5-8/9/10-12/13-16 × варианты с
#     полкласса/общим/без общих)
#   - student-section: 14 (max/universal/standard pair+tail +
#     light/medium/mini overflow/spread-tail/grid-tail/initial-combined)
#   - common-section: 3 (заполнение висящей правой страницы:
#     J-Half→J-Full→J-Collage-6 по приоритету)
#   - intro: 1 (S-Intro для soft)
#   - final: 2 (S-Final-Soft-L with-photo/text-only)
# Combined-мастера (M/L/N-Combined-Page) применяются ТОЛЬКО для
# изначально маленьких классов (students_remaining ≤ MAX_SLOTS_COMBINED
# при current_student_index=0). Для остатков после overflow висящая
# правая страница заполняется через common-section.
# Стратегия ребалансировки (Light 5+5+3 вместо 6+6+1) НЕ реализована
# в правилах — оставлена как UI-функция редактора фаз 2+.
# Следующий шаг: РЭ.8 (UPSERT правил/пресетов из seed-скрипта в БД),
# затем РЭ.9 (алгоритм buildFromRules — большая работа, lib/rule-engine/
# build.ts + evaluate.ts + apply.ts + balance.ts).
#
...
#    Подэтапы РЭ.1-РЭ.7 ✅ выполнены, РЭ.8-РЭ.13 запланированы.
#
...
# РЭ.8  — UPSERT правил и пресетов в Supabase из seed-скрипта.
#         Сейчас seed --write пишет только семейства (template_families).
#         Надо добавить: UPSERT rules (36 файлов → таблица rules),
#         UPSERT presets (7 файлов → таблица presets).
#         Это короткий подэтап ~30-50 строк изменений seed-скрипта.
...
#         БОЛЬШОЙ ПОДЭТАП. Включает интерпретатор when-clauses,
#         подстановку {i}/$current_student_index в bind, обработку
#         consumes/skip_if, цикл по секциям пресета.
#         Оценка ~1000+ строк, отдельная сессия.
