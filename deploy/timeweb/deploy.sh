...
# 1. Выбираем доступный источник (primary gitflic → fallback origin) и тянем main.
#    Один ls-remote на источник; первый ответивший — ACTIVE_REMOTE. Если ни один
#    не отвечает — честный провал (деплой не едет), а не молчаливый старый код.
# shellcheck source=/dev/null
source "$SCRIPT_DIR/select-source.sh"
ACTIVE_REMOTE="$(select_active_remote "$BRANCH")" || {
  log "НИ ОДИН источник не отвечает ($DEPLOY_REMOTES) — деплой отменён"
  exit 1
}
if [ "$ACTIVE_REMOTE" = "${DEPLOY_REMOTES%% *}" ]; then
  log "deploy source: $ACTIVE_REMOTE (primary)"
else
  log "deploy source: $ACTIVE_REMOTE (fallback)"
fi
git -C "$REPO" fetch --quiet "$ACTIVE_REMOTE" "$BRANCH"
REMOTE_SHA="$(git -C "$REPO" rev-parse --short "$ACTIVE_REMOTE/$BRANCH")"
...
rm -rf "$NEW"
mkdir -p "$NEW"
git -C "$REPO" archive "$ACTIVE_REMOTE/$BRANCH" | tar -x -C "$NEW"
