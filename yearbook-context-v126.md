...
# Версия: v126
# Обновлено: 21.05.2026 (после РЭ.27.0 — spec фазы РЭ.27 написан).
#
# ⏳ ФАЗА РЭ.27 СТАРТОВАЛА — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтап 27.0 (spec) закрыт. Следующий — 27.1 (миграция БД схемы).
# Spec: docs/phase-Р27-spec.md v1.0 (377 строк).
# Финальный commit подэтапа 27.0: 266341a.
#
# Что зафиксировано в spec (7 архитектурных развилок утверждены):
#   A) Постраничный режим редактора — НЕТ. Редактор всегда разворотный
#      (как в InDesign), отличия по print_type только в правилах
#      сборки и экспорта.
#   B) Где живёт print_type — albums.print_type (новое поле БД)
#      + fallback на preset.print_type через resolvePrintType().
#   C) Автозаглушка «Форзац» для soft — водяной знак «Форзац»
#      бледным курсивом по центру страницы.
#   D) Миграция print_type для существующих альбомов — автоматически
#      из preset.print_type.
#   E) Фильтр spread-мастера для soft — защита в ДВУХ местах:
#      engine (пропуск с warning) + UI (visually disabled).
#   F) Слияние дубль-пресетов layflat/soft — Вариант A (агрессивный):
#      перепривязка ссылок → удаление осиротевших → переименование.
#      Колонку presets.print_type оставляем как deprecated.
#   G) Подтверждение в UI при смене типа листов если выбраны
#      spread-мастера. Engine при пересборке сам разрулит.
#
# Ключевое архитектурное отличие от ранней формулировки (Р24-spec §13):
#   ТАМ: двойной редактор (постраничный для soft, разворотный для layflat).
#   ЗДЕСЬ: один разворотный редактор для всех. Объём упал с 10-15
#   до 7-9 коммитов, риск регрессий минимальный.
#
# План подэтапов РЭ.27 (9-10 коммитов):
#   ✅ 27.0 — spec (commit 266341a)
#   ⏳ 27.1 — миграция БД: схема (albums.print_type)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v126
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v126 — РЭ.27.0 закрыт (spec). Фаза РЭ.27 в работе.
#      • docs/phase-Р27-spec.md v1.0 (377 строк) — полная спецификация
#        фазы РЭ.27 с 7 архитектурными развилками и планом подэтапов
#      • docs/phase-Р25-summary.md — последняя закрытая фаза (РЭ.25)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#      НЕ ПРИМЕНЕНА: миграция РЭ.27.1 (ещё не написана).
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.1 (миграция схемы):
#      • SQL добавить колонку:
#        ALTER TABLE albums ADD COLUMN print_type TEXT;
#        (NULL допустим — fallback на preset.print_type)
#      • Индекс idx_albums_print_type (только для NOT NULL).
#      • Файл: migrations/2026-05-21-albums-print-type.sql
#      • Применить в Supabase ДО деплоя кода (аддитивная миграция,
#        zero-downtime — старый код продолжает работать).
#      • Полное описание: docs/phase-Р27-spec.md §2.2.
#
#   ⚠️ ВАЖНО: миграция данных (заполнение albums.print_type из пресета
#      + слияние дубль-пресетов) — это ОТДЕЛЬНЫЙ подэтап 27.7, не 27.1.
#      Сначала сделать всю функциональность с fallback, потом
#      мигрировать данные.
