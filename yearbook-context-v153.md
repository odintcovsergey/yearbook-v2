...
# │   decrement шёл с quarter, cursor sixthUsed не сдвигался → вторая  │
# │   J-Collage-4 брала те же 4 фото из sixth[0..3]. Симптом на Тест2  │
# │   (разворот 8 и 9 — одинаковые collagephoto_1..4).                 │
# │                                                                    │
# │   ФИКС: обобщённое правило 'collageCount > 0 → category sixth,     │
# │   count=collageCount'. Покрывает J-Collage-4, J-Collage-6 и любой  │
# │   будущий J-Collage-N (3, 5, 8) без правок кода. Семантика теперь  │
# │   согласована: «есть collagephoto_N → читай из sixth-пула».        │
# │                                                                    │
...
# │   • Тест2 после РЭ.36 — 8 уникальных фото в J-Collage-4×2          │
# │     (требует ре-сборки альбома для применения)                     │
# │   • Все 30 мастеров в БД совпадают с InDesign                      │
# │   • Template Set ID: 08baf556-7831-44e9-9ba8-4af20f19ee44          │
# │                                                                    │
# │ ЧТО ДАЛЬШЕ (приоритет Сергея):                                     │
# │   1. UI warnings для партнёра — индикатор «есть N предупреждений   │
# │      от автосборки» в редакторе альбома с разворачиваемым списком  │
# │      (common_required_page_skipped, _master_missing, _no_category, │
# │      slot_skipped, _autopack_underflow и т.д.). Маленькая          │
# │      фронт-правка, следующая в очереди.                            │
# │   2. РЭ.37 — конструктор переходного раздела (отдельный чат,       │
# │      spec-работа с требованиями)                                   │
# │   3. Симметризация хвоста students (отдельный чат, может вместе    │
# │      с РЭ.37)                                                      │
# │   4. РЭ.38 — гибридная авто-классификация общих фото (на потом,    │
# │      после 5-10 реальных партнёров на проде; план описан выше)     │
# │   5. Сергей дорисовывает недостающие мастера в InDesign            │
# │      (Приоритет 1 из docs/master-catalog.md)                       │
# │   6. Тестируем белый дизайн на 1-2 реальных альбомах               │
# │   7. Заказ второго дизайна с фоном по designer-brief.md            │
# └────────────────────────────────────────────────────────────────────┘
#
# ┌────────────────────────────────────────────────────────────────────┐
