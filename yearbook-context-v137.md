...
# Версия: v137
# Обновлено: 21.05.2026 (после РЭ.28.1 — миграция БД parent_template_set_id).
#
# ⏳ ФАЗА РЭ.28 В РАБОТЕ — партнёрские дизайны (сценарий A)
# ──────────────────────────────────────────────────────────────────
# Подэтапы 28.0..28.1 закрыты. Следующий — 28.2 (чистые утилиты).
# Spec: docs/phase-Р28-spec.md v1.0.
# Последний commit фазы: 68a7f0a (РЭ.28.1).
#
# Что готово после 28.1:
#   • SQL-миграция создана: migrations/2026-05-21-template-sets-parent.sql
#   • template_sets.parent_template_set_id uuid NULL с FK самоссылкой
#     ON DELETE SET NULL.
#   • Partial индекс idx_template_sets_parent WHERE NOT NULL.
#   • ⚠️ Применить в Supabase (Сергей) ДО или ОДНОВРЕМЕННО с деплоем
#     кода 28.3+.
#
# Все архитектурные развилки A..H зафиксированы в spec §7 (см. v136).
#
# План подэтапов РЭ.28:
#   ✅ 28.0 — spec (commit 1193b5d)
#   ✅ 28.1 — миграция БД (commit 68a7f0a) — ждёт применения в Supabase
#   ⏳ 28.2 — чистые утилиты + unit-тесты (~15-20):
#             • lib/template-set-clone/constants.ts (PRINT_DPI=300, MM_STEP)
#             • lib/template-set-clone/round-to-pixels.ts (roundMmToPx, mmToPx)
#             • lib/template-set-clone/aspect-compatibility.ts (3 уровня)
#             • lib/template-set-clone/resize-placeholder.ts
#             • lib/template-set-clone/prepare-clone.ts (главная функция)
#   ⏳ 28.3 — API: template_set_clone (транзакция INSERT template_sets +
#             N spread_templates) + template_set_my_list + template_set_delete
#   ⏳ 28.4 — UI каталога: «Мои дизайны» + кнопка «Создать на основе...»
#   ⏳ 28.5 — UI модалка: name + page sizes (mm + ≈ px hint) + bleed_mm
#             опционально + предупреждения по уровням совместимости
#   ⏳ 28.6 — summary + закрытие фазы
#
# Тесты на main: 437/437 passing (стабильно после РЭ.27.3).
#
...
# Применено в 28.1 — schema уже выгружена в прошлом ходе (15 колонок,
# parent_template_set_id отсутствует), миграция написана корректно.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v137
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v137 — фаза РЭ.28 в работе, 28.0+28.1 закрыты
#      • docs/phase-Р28-spec.md §3 «Утилиты ресайза» — основной
#        контракт для 28.2
#      • lib/album-builder/print-type-resolver.ts — образец чистого
#        модуля без зависимостей (паттерн из РЭ.27.3)
#      • lib/smart-fill/filter-by-purchase.ts — другой образец
#        чистой функции (РЭ.25.3)
#   4. Применены в Supabase: все миграции до РЭ.27.7b + РЭ.28.1 ✅
#      (Сергей подтвердил применение 28.1 миграции)
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.28 — следующий подэтап 28.2 (утилиты):
#
#      Создать в lib/template-set-clone/:
#
#      a) constants.ts — PRINT_DPI=300, MM_STEP=25.4/PRINT_DPI ≈ 0.0847
#         Хардкод, без параметризации (см. spec §3.1).
#
#      b) round-to-pixels.ts:
#         • roundMmToPx(mm: number): number — округляет до ближайшего
#           целого пикселя (значение остаётся в mm, но кратно MM_STEP).
#         • mmToPx(mm: number): number — конверсия в целые пиксели
#           для UI отображения «≈ NNNN px».
#
#      c) aspect-compatibility.ts:
#         • checkAspectCompatibility(oldW, oldH, newW, newH):
#             { level: 'ok'|'warning'|'blocked',
#               aspect_diff_percent: number, message: string }
#         • <5% ok / 5-10% warning / >10% blocked
#         • Метрика разницы: max(a/b, b/a) - 1 (симметрична)
#
#      d) resize-placeholder.ts:
#         • resizePlaceholder<P>(p, scaleX, scaleY): P
#         • x_mm, y_mm, width_mm, height_mm — умножить на scale +
#           округлить через roundMmToPx.
#         • rotation_deg, label, type, fit и т.д. — копируем как есть.
#         • Возвращает НОВЫЙ объект (не мутирует).
#
#      e) prepare-clone.ts (главный entry point):
#         • prepareTemplateSetClone(request): ClonePlan
#         • Расчёт scale_x, scale_y из page_width/height.
#         • Проверка aspect — если blocked, throw.
#         • Resize всех мастеров + всех placeholder'ов внутри.
#         • Расчёт spread_width/height (учитывая facing_pages).
#         • Округление всех mm-значений.
#
#      Unit-тесты в lib/template-set-clone/__tests__/:
#         • round-to-pixels.test.ts ~5 тестов (вкл. идемпотентность)
#         • aspect-compatibility.test.ts ~5 тестов (вкл. симметрию)
#         • resize-placeholder.test.ts ~5 тестов
#         • prepare-clone.test.ts ~5 тестов
#         Итого ~15-20 тестов. 437/437 → ~455/455.
#
#      Публичный экспорт в lib/template-set-clone/index.ts (по паттерну
#      lib/album-builder/index.ts).
#
#      Дальше — 28.3 (API: clone/my_list/delete), 28.4-5 (UI), 28.6
#      (summary).
