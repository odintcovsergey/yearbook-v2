...
# Обновлено: 17.04.2026 (этап 3.3.a закрыт — фото в /app)
...
### Этап 3.3.b — Экспорт CSV (следующий)
- GET /api/tenant: export_csv — статистика по альбому (по аналогии
  со старым /api/admin?action=export)
  - Колонки: ФИО, класс, статус, телефон, родитель, выбранные портреты,
    выбранные групповые, обложка, доплата, текст ученика
  - Учителя — отдельный лист/секция
- /app UI:
  - Кнопка "Экспорт CSV" в AlbumDetailModal (в шапке или на вкладке Обзор)
  - Скачивание как <album_title>_<date>.csv

### Этап 3.3.c — Управление заявками (referral_leads)
- GET /api/tenant: leads (фильтр по auth.tenantId, с referrer_name/album)
- POST /api/tenant: update_lead_status, delete_lead
- /app UI:
  - Отдельный раздел "Заявки" в главном меню (не в модалке альбома)
  - Карточки заявок: имя, телефон, город, школа, класс, статус, источник
  - Фильтр по статусу (new / contacted / converted / rejected)
  - Изменение статуса inline, удаление с confirm
  - Счётчик новых заявок на главной (уже есть в dashboard)

### Этап 3.3.d — Цитаты
- GET /api/tenant: quotes (свои tenant + глобальные через OR is.null)
- POST /api/tenant: create_quote, update_quote, delete_quote
- /app UI:
  - Раздел "Цитаты" (свои + глобальные, глобальные read-only)
  - Добавить/редактировать свои цитаты
  - Статистика: сколько детей выбрали каждую цитату

### Этап 3.3.e — Напоминания родителям
- GET /api/tenant: reminders — список детей без submitted_at с контактами
- /app UI:
  - Кнопка "Напомнить родителям" в AlbumDetailModal
  - Генерация текста-шаблона (с именами и ссылками)
  - Копирование в буфер обмена (без реальной отправки пока)
  - В будущем — интеграция с SMS/Email провайдерами
