...
# Версия: v132
# Обновлено: 21.05.2026 (после РЭ.27.5 — UI палитры фильтрует spread для soft).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0..27.5 закрыты. Следующий — 27.6 (UI формы альбома:
# селект «Тип листов»).
# Spec: docs/phase-Р27-spec.md v1.0.
# Последний commit фазы: ed380a3 (РЭ.27.5).
#
# ✅ Миграция РЭ.27.1 применена в Supabase.
#
# Что готово после 27.5 (защита spread-мастеров в двух местах):
#   • ENGINE (РЭ.27.3): isMasterAllowedForPrintType фильтрует
#     spread-мастера в build — последний рубеж защиты.
#   • UI (РЭ.27.5): TemplatePickerModal принимает printType пропс.
#     Для soft-альбомов spread-мастера показываются:
#     - opacity-40 (визуально приглушены)
#     - cursor-not-allowed
#     - title с пояснением «недоступно для мягких листов»
#     - подпись «не для мягких листов» под названием мастера
#     - HTML disabled + JS guard в onClick (двойная защита).
#   • Передаётся в обе точки вызова: «Добавить разворот» и
#     «Заменить шаблон». Используется уже подтянутый
#     effective_print_type из РЭ.27.4.
#
# Что ещё НЕ работает:
#   • UI формы альбома — нет селекта «Тип листов» (27.6).
#     Сейчас тип берётся из пресета при создании альбома,
#     явно поменять можно только через API (с 27.2). UI-кнопки нет.
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v132
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v132 — РЭ.27.0..27.5 закрыты. Защита spread-мастеров
#        работает в engine + UI.
#      • docs/phase-Р27-spec.md §4.1 «Форма создания/редактирования
#        альбома» — описание UI селекта «Тип листов».
#   4. Применены в Supabase: все РЭ.22..РЭ.25.1 + РЭ.27.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.6 (UI формы):
#      Цель: добавить селект «Тип листов» в форму создания/редактирования
#      альбома, чтобы партнёр мог менять print_type БЕЗ смены пресета.
#
#      Места правок (app/app/page.tsx — AlbumFormModal):
#
#      a) FormData тип расширить:
#         + print_type: 'layflat' | 'soft' | null
#         (опционально для бэк-совместимости с emptyForm()).
#
#      b) emptyForm() — дефолт null (engine применит fallback на пресет).
#
#      c) Загрузка из БД при редактировании альбома:
#         + print_type из albumData.print_type (поле уже отдаётся API).
#
#      d) UI селект в форме (рядом с/после блока шаблона):
#         <select>
...
#         </select>
#         Подсказка: «На мягких листах не доступен мастер фото на разворот».
#
#      e) Submit: form.print_type → body.print_type
#         (API уже принимает с РЭ.27.2 — приоритет над пресетом).
#
#      f) Опциональная развилка G (см. spec): если меняется тип с
#         layflat на soft и в layout уже есть spread-мастера —
#         confirmation dialog. Можно отложить (это редкость пока).
#
#      Дальше — 27.7 (миграция данных + слияние дубль-пресетов),
#      27.8 (summary).
