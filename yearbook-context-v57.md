...
# Версия: v57
# Обновлено: 15.05.2026 (rule engine — спецификация v1.0 написана)
#
# ⚠️ ВАЖНО: ПРЕЖДЕ ЧЕМ ПРИНИМАТЬСЯ ЗА ЛЮБУЮ ДРУГУЮ РАБОТУ — ПРОЧИТАЙ:
#   docs/rule-engine-spec.md              — 🆕 спецификация rule engine v1.0 (15.05.2026)
...
# Rule engine спецификация v1.0 готова (15.05.2026) — фундамент реализации
...
# 📐 RULE ENGINE — спецификация v1.0 написана (15.05.2026, 1 коммит docs).
#    Подэтапы реализации РЭ.1-РЭ.13 запланированы, ещё не начаты.
#
...
#
# ────────────────────────────────────────────────────────────────────
# 📐 RULE ENGINE — СПЕЦИФИКАЦИЯ v1.0 (15.05.2026)
# ────────────────────────────────────────────────────────────────────
#
...
#   ✓ docs/rule-engine-spec.md v1.0 (65d1408, 15.05.2026, 1518 строк)
...
#
# 2. СЕМЬ АКТИВНЫХ СЕМЕЙСТВ + I-Personal отложен:
...
#    в одном пресете (например «Стандарт + виньетка»).
#
# 4. ПОСТРАНИЧНАЯ МОДЕЛЬ + СЕРИИ МАСТЕРОВ:
#    Все мастера postpage (page-left/page-right/page-any/spread).
#    Из 4 одностраничных можно собрать 8 разворотов.
#    Серия мастеров (series_id) = набор страниц одного дизайна
#    гарантированно сочетающихся.
#
# 5. МЕЖСЕМЕЙСТВЕННЫЙ РАЗВОРОТ — первоклассная концепция:
#    Левая страница из student-section + правая из common-section
#    (одинокий 13-й ученик в Standard, неполная сетка Light с 7 учениками).
#    Не исключение, а нормальный механизм дозаполнения.
#
# 6. ПАРАМЕТРИЧЕСКИЕ МАСТЕРА (variable grid):
#    Для Mini (1..12 учеников на странице), Light (1..6), Medium (1..4),
#    G-Teachers (3x3/4x3/4x4) — ОДИН IDML с диапазоном grid_modes,
#    не N отдельных мастеров. Алгоритм выбирает режим по числу слотов.
#
...
#     А.9 I-Personal → структура закладывается, правил нет
#
# ────────────────────────────────────────────────────────────────────
...
#
# РЭ.1  — Миграция БД (template_families, rules, presets, layout_cache
#         + ALTER spread_templates, album_layouts) — rule-engine-migration.sql
# РЭ.2  — Типы и Zod-схемы (lib/rule-engine/types.ts, schemas.ts)
# РЭ.3  — Каталог JSON в docs/rule-engine-data/ (families, rules,
#         presets) + scripts/seed-rule-engine.ts
...
#         (включая одинокого ученика через межсемейственный разворот)
# РЭ.6  — Правила student-section: medium/light/mini +
#         параметрические мастера (L-Grid-Page, N-Grid-Page, M-Grid-Page)
# РЭ.7  — Правила common-section + intro + final
# РЭ.8  — 6 базовых пресетов OkeyBook (Стандарт, Стандарт+виньетка,
#         Универсал, Максимум, Лайт, Мини в обеих печатях через
#         enabled_when)
# РЭ.9  — buildFromRules алгоритм (lib/rule-engine/build.ts +
#         evaluate.ts + apply.ts + balance.ts) + интеграция в
#         lib/album-builder/index.ts (фолбэк на старый)
...
# РЭ.13 — Документация + designer-tz v1.3 + обновление контекста
#
...
#   docs/rule-engine-spec.md (1518 строк) — главная спека
...
#   docs/templates/designer-tz-2026-05-12.md (v1.2, станет v1.3
#     после реализации rule engine)
#
