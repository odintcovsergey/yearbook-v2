...
# Версия: v100
# Обновлено: 20.05.2026 (после РЭ.22.7.2 — engine teachers семантический).
#
...
# ✅ РЭ.22.7.1 (fea1a75) — data-миграция teachers-мастеров, ✅ применена.
# ✅ РЭ.22.7.2 (8d91953) — engine teachers семантический поиск.
...
#    - В sections/teachers.ts: pickLeftMaster и pickRightMaster переписаны
#      через resolveTeacherMaster (семантика → legacy-fallback по имени →
#      warning). photos_full=0 в левых запросах отсеивает
#      F-Head-WithClassPhoto-L. semantic-флаг в decision_trace.inputs.
#    - Закрыт скрытый баг #3 фазы: legacy искал 'G-Teachers-4x3', в БД
#      'G-Teachers-3x4'. Тесты обновлены на корректное имя.
#    - 8 новых тестов sections-teachers-semantic.test.ts.
#    - Всего тестов в проекте: 349/349 passing (было 341/341).
#
# Двух-осевая модель «режим × параметры» — students секция готова:
#   - mode='page'   — ✅ РЭ.22.4
#   - mode='spread' — ✅ РЭ.22.5
#   - mode='grid'   — ✅ РЭ.22.6
# Teachers секция готова:
#   - ✅ РЭ.22.7.2
#
# ⏳ Следующий подэтап: РЭ.22.8 — soft_intro + soft_final семантический.
#    - Заменить жёсткое S-Intro / S-Final в sections/soft-intro.ts и
#      sections/soft-final.ts на семантический поиск через page_role='intro'
#      или 'final' + slot_capacity (photos_full=1).
#    - Аналогичная архитектура: семантика → legacy fallback по имени.
#    - data-миграция: разметить S-Intro / S-Final тегами (если они есть
#      в БД). Сначала SELECT для выгрузки.
#
...
# • Баг #2 (известная коллизия L-Grid-12 vs N-Grid-Page, develop ↓)
...
#
#   ⏳ Следующий подэтап: РЭ.22.8 — soft_intro + soft_final семантический
#      Перевести жёсткое S-Intro / S-Final в sections/soft-intro.ts и
#      sections/soft-final.ts на семантику через page_role='intro'/'final'
#      + slot_capacity (photos_full=1). data-миграция при необходимости.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v100
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v100 — что закрыто (включая students и teachers)
#      • docs/phase-Р22-spec.md v1.0 — §6.5/6.6 (soft_intro / soft_final).
#      • lib/rule-engine/sections/soft-intro.ts — текущий код ищет жёстко
#        'S-Intro'. Аналогично soft-final.ts ('S-Final').
#      • lib/rule-engine/master-finder.ts — образец findTeacherMaster
#        для аналогичной findIntroMaster / findFinalMaster (или просто
#        переиспользуем findTeacherMaster pattern).
...
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.8 — soft_intro + soft_final семантический.
#      Сначала запросить у Сергея выгрузку:
#        SELECT name, page_role, slot_capacity,
#          (SELECT COUNT(*) FROM jsonb_array_elements(placeholders) e
#           WHERE LOWER(e->>'label') = 'classphotoframe') AS classphoto_count
#        FROM spread_templates
#        WHERE name LIKE 'S-%' OR name LIKE 'Soft%' OR page_role IN ('intro', 'final')
#        ORDER BY name;
#      По выгрузке составить data-миграцию (если есть что размечать) +
#      переписать sections/soft-intro.ts и soft-final.ts через семантику.
#      Аналог архитектуры РЭ.22.7.2 (resolveSoftMaster со fallback на имя).
#      Скорее всего меньший подэтап чем teachers — нет таблицы выбора,
#      один мастер на секцию.
#      Проверки: tsc + next build + vitest зелёные. Один атомарный коммит
#      (или два если нужна data-миграция).
