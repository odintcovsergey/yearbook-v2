...
# Версия: v139
# Обновлено: 21.05.2026 (после РЭ.28.3 — API endpoints).
#
# ⏳ ФАЗА РЭ.28 В РАБОТЕ — партнёрские дизайны (сценарий A)
# ──────────────────────────────────────────────────────────────────
# Подэтапы 28.0..28.3 закрыты. Следующий — 28.4 (UI каталога).
# Spec: docs/phase-Р28-spec.md v1.0.
# Последний commit фазы: 0b8eced (РЭ.28.3).
#
# ✅ Миграция РЭ.28.1 применена в Supabase (Сергей подтвердил).
#
# Что готово после 28.3:
#   • Engine (РЭ.28.2): 5 чистых модулей + 55 unit-тестов
#   • API (РЭ.28.3): 3 новых action'а в app/api/tenant/route.ts
#     - GET 'template_set_my_list' — клоны партнёра (для UI 28.4)
#     - POST 'template_set_clone' — создание клона с resize
#       и ручным rollback при сбое INSERT мастеров (Supabase JS не
#       поддерживает явные транзакции)
#     - POST 'template_set_delete' — с защитой от удаления при
#       0 ссылок из albums/presets (COUNT перед DELETE)
#   • Существующий designs_list (РЭ.24) уже фильтрует «свои +
#     глобальные» по tenant_id — новые клоны автоматически появятся
#     в существующем UI каталога /app/templates без правок.
#
...
#   ⏳ 28.4 — UI каталога: кнопка «Создать на основе...» на карточке
#             глобального дизайна + раздел «Мои дизайны» для клонов
#   ⏳ 28.5 — UI модалка ввода размеров: name + page sizes (mm + ≈ px hint)
#             + bleed_mm опционально + предупреждения по уровням
#             совместимости (ok/warning/blocked)
#   ⏳ 28.6 — summary + закрытие фазы
#
# Тесты на main: 492/492 passing (стабильно после 28.2). API тесты
# не пишем — идут через UI в 28.4-5 (E2E на тестовом дизайне).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v139
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v139 — 28.0..28.3 закрыты, бэкенд готов
#      • docs/phase-Р28-spec.md §5.1 «UI каталога»
#      • app/app/templates/ или app/app/designs/ — каталог дизайнов
#        (где живёт текущий designs_list рендер)
#   4. Применены в Supabase: все миграции до РЭ.28.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.28 — следующий подэтап 28.4 (UI каталога):
#
#      Цель: партнёр в /app/templates (или ином UI каталоге) видит:
#      • Раздел «Глобальные шаблоны OkeyBook» (как сейчас).
#      • Раздел «Мои дизайны» (новый) — если есть хотя бы один клон.
#      • На карточке ГЛОБАЛЬНОГО дизайна — новая кнопка
#        «Создать на основе...» (открывает модалку 28.5).
#      • На карточке СВОЕГО дизайна — кнопки «Открыть» + «Удалить»
#        (последнее с confirm).
#
#      Места правок:
#      a) Найти страницу каталога:
#         find app/app -name 'page.tsx' | xargs grep -l 'designs_list'
#      b) Существующий designs_list уже отдаёт «свои + глобальные»
#         вперемешку — нужно разделить на два раздела по tenant_id.
#         Либо использовать template_set_my_list для своих и
#         designs_list (с фильтром tenant_id IS NULL) для глобальных.
#      c) Кнопка «Создать на основе...» — заглушка на 28.4, реальная
#         модалка приедет в 28.5. Можно временно открывать алерт
#         с template_set_id для тестирования.
#      d) Кнопка «Удалить» на своих — простой confirm() →
#         POST template_set_delete → reload.
#
#      Дальше — 28.5 (UI модалка ввода размеров с mm/px и предупреждениями)
#      → 28.6 (summary + закрытие фазы).
