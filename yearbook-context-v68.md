...
# Версия: v68
# Обновлено: 16.05.2026 (РЭ.14 закрыт — read-only превью rule engine
#                       на реальных альбомах. Кнопка '🧪 Превью через
#                       Rule Engine' во вкладке Обзор AlbumDetailModal.)
#
...
# Rule engine MVP готов end-to-end + ДОСТУПЕН ИЗ БОЕВОГО UI ДЛЯ ПРЕВЬЮ:
#   ✅ Спецификация v1.3 + 7 семейств + 36 правил + 7 пресетов
#   ✅ Алгоритм buildFromRules (РЭ.9, ~1.5K строк)
#   ✅ 77 unit tests (РЭ.10, 132ms)
#   ✅ Sandbox endpoint + UI Build Test в /super/templates (РЭ.13)
#   ✅ Preview endpoint + UI кнопка в AlbumDetailModal (РЭ.14) ← НОВОЕ
# Подэтапы РЭ.1..РЭ.10 + РЭ.13 + РЭ.14 закодированы.
# РЭ.11 (UI фильтрация picker'а) + РЭ.12 (UI редактор пресетов) — отложены.
#
# 📍 КАК ПРОТЕСТИРОВАТЬ RULE ENGINE НА РЕАЛЬНОМ АЛЬБОМЕ:
#   1. Если ещё не делал — залить правила/пресеты в боевой Supabase:
#      npx tsx --env-file=.env.local scripts/seed-rule-engine.ts --write
#      После: 7 families + 36 rules + 7 presets в БД (однократно).
#   2. Открыть кабинет → выбрать ЛЮБОЙ альбом → вкладка 'Обзор'
#   3. Если legacy layout уже собран — под warnings блок появится
#      фиолетовая кнопка '🧪 Превью через Rule Engine'
#   4. Выбрать preset из 7 (standard/universal/maximum/individual/
#      medium/light/mini-soft) → Прогнать
#   5. Смотреть:
#      - status badge (зелёный/жёлтый/красный)
#      - smart-fill warnings (если что-то с данными альбома)
#      - rule engine warnings
#      - decision_trace (какое правило применилось для каждой страницы)
#      - spreads (master_name слева | справа + mixed badge)
#   6. Ничего не сохраняется в БД — это read-only.
#
# 📍 КАК ПРОТЕСТИРОВАТЬ НА СИНТЕТИКЕ (без альбома):
#   1. Зайти как superadmin в /super/templates → 'Открыть' на
#      okeybook-default → внизу под мастерами фиолетовая кнопка
#      'Build Test (Rule Engine)'
#   2. Задать synthetic N учеников/предметников/фото + preset → Build
#
# Следующий шаг (после ручной проверки):
#   - Если rule engine собирает разумно → подключение к боевому
#     /api/layout?action=build_album (ступень Б из плана — миграционный
#     режим: новые альбомы с preset_id из 'presets' идут через rule engine,
#     legacy остаются на старом buildAlbum).
#   - Если найдутся проблемы → правки в правилах JSON + новые unit tests.
#   - В обоих случаях РЭ.11/РЭ.12 (UI редактор пресетов) — после
...
#    Подэтапы РЭ.1-РЭ.10 + РЭ.13 + РЭ.14 ✅ выполнены, РЭ.11-РЭ.12 отложены.
#
...
#            РЭ.14.3 — контекст v68 (текущий) + how-to.
