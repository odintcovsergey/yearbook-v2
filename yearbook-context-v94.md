...
# Версия: v94
# Обновлено: 20.05.2026 (после РЭ.22.3 — UI двух-осевая модель).
#
...
# ✅ РЭ.22.1 (8516419) — миграция БД: student_layout_mode + student_grid_size
#    с CHECK через DO-блоки. ✅ Применена в Supabase 20.05.2026.
# ✅ РЭ.22.2 (335f00e) — типы + API + попутно закрыт скрытый баг РЭ.21.8.15
#    (loader не пробрасывал student_* поля).
# ✅ РЭ.22.3 (4d132ec) — UI двух-осевая модель в PresetEditorModal.
#    - Главный селект 'Режим' (page/spread/grid)
#    - Conditional параметры: page/spread → friend_photos + has_quote;
#      grid → grid_size + has_quote
#    - Fallback: при первом открытии legacy-записи UI вычисляет режим
#      из density/preset.id (показывает amber-warning до сохранения)
#    - Save: новые поля + дублирование в legacy student_pages_per_student
#      (page=1, spread=2, grid=NULL) для отката Vercel
#
# ⏳ Следующий подэтап: РЭ.22.4 — engine semantic для mode='page'.
#    - В sections/students.ts добавить ветку для preset.student_layout_mode='page'
#    - Использовать findStudentMaster (расширение из РЭ.21.8.15) для
#      Standard/Universal — заменить жёстко прошитые E-Standard-Left/Right
#      и E-Universal-Left/Right на семантический поиск.
#    - Fallback на текущую buildAlternatingLR когда student_layout_mode IS NULL
#      (legacy запись) — ничего не ломается.
#    - 5-8 новых тестов в lib/rule-engine/__tests__/
#
...
#
#   ⏳ Следующий подэтап: РЭ.22.4 — engine semantic для mode='page'
#      В sections/students.ts добавить ветку приоритетного чтения
#      student_layout_mode='page' → buildPageSemantic через findStudentMaster.
#      Fallback на buildAlternatingLR когда mode=NULL. Тесты.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v94
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v94 — что закрыто (РЭ.22.0..РЭ.22.3 + миграция применена)
#      • docs/phase-Р22-spec.md v1.0 — особо §6.1-6.2 (Engine контракт
#        для mode='page' + расширение findStudentMaster).
#      • lib/rule-engine/sections/students.ts — функция fillStudentsSection
#        и buildAlternatingLR. В РЭ.22.4 добавляем приоритетную ветку:
#        если preset.student_layout_mode='page' → новая buildPageSemantic
#        вместо buildAlternatingLR.
#      • lib/rule-engine/master-finder.ts — задел из РЭ.21.8.15. Для page
#        режима функция уже работает (фильтр studentsCap===1).
...
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.4 — engine semantic для mode='page'.
#      Файлы:
#        - lib/rule-engine/sections/students.ts — новая buildPageSemantic
#          в начале fillStudentsSection (приоритет над текущей логикой по
#          density). Использует findStudentMaster с pageRole='student_left'/
#          'student_right', photosFriend=preset.student_friend_photos,
#          hasQuote=preset.student_has_quote.
#        - lib/rule-engine/__tests__/sections-students-page-semantic.test.ts
#          (новый файл) — 5-8 тестов: точное совпадение, ближайший меньший,
#          mismatch квоты, fallback на legacy когда mode=NULL.
#      Что НЕ трогаем:
#        - buildAlternatingLR (остаётся как fallback)
#        - buildOnePerSpread / buildOnePerSpreadAdaptive (РЭ.22.5)
#        - buildGrid (РЭ.22.6)
#        - findStudentMaster (для page режима уже работает,
#          расширение только для grid в РЭ.22.6)
