...
# Обновлено: 17.04.2026 (этап 3.6 закрыт — брендинг на странице родителя)
...
### Этап 4 — перенос старой админки (следующий)

Самая деликатная задача: старая админка /admin работает на
ADMIN_SECRET и по ней идут реальные заказы. Миграция — поэтапная,
чтобы не сломать работающих организаторов.

Разбиение:
- **4.a** — добавить requireAuth в /api/admin параллельно с
  x-admin-secret (оба работают). Старая админка продолжает жить
  на ADMIN_SECRET, новая авторизация — для будущих сценариев.
- **4.b** — добавить tenant_id фильтры ко всем запросам админки.
  Сейчас они есть только на уровне delete/update (через MAIN_TENANT_ID),
  но чтения — нет. Для однотенантного режима не критично, но нужно
  перед полноценной миграцией.
- **4.c** — когда все тестовые сценарии пройдены, сделать /app
  основным входом (редирект /admin → /app при JWT), а /admin —
  legacy fallback через x-admin-secret.
- **4.d** — после подтверждения что всё работает через /app —
  удалить app/admin/page.tsx и app/api/admin/route.ts.

Перед началом 4.a стоит провести ручную приёмку /app на production:
убедиться что все сценарии (альбомы, ученики, учителя, фото,
экспорт, заявки, цитаты, напоминания, команда, настройки)
работают так же или лучше, чем в /admin.
