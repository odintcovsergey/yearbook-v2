...
# Что нового в v37: фаза 0 — этапы 0.3...0.7 завершены (11 коммитов на ветке);
# ADMIN_SECRET удалён из Vercel.
...
- **Текущий статус:** фаза 0 — этапы 0.1...0.7 завершены (11 коммитов на ветке `feature/layout-engine`), идёт работа над 0.8
...
### Артефакты в БД (на 5 мая 2026, после 0.7)

- `template_sets`: 1 запись — `slug=okeybook-default`, name «Плотные Мастер Белый», `is_global=true`, `print_type=layflat`.
- `spread_templates`: 39 записей с этим `template_set_id` (`sort_order` 0-38).

### Что осталось до конца фазы 0

- **0.8** — UI detail page `/super/templates/[id]` с **Konva canvas**: все 39 spread_templates с цветовой кодировкой плейсхолдеров (см. `docs/phase-0-spec.md §9`). Использует endpoint 0.5 DETAIL. Перед стартом: `npm i konva react-konva`.
...
- **0.14** — финальное обновление контекста (v38) + чистка legacy `x-admin-secret` кода в `lib/auth.ts`. Текущий v37 — частичное обновление; v38 закроет 0.14 после 0.8-0.13.
