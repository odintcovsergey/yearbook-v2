...
# CRM v1+v2 готов. Свайп в лайтбоксе. Supabase Free.
...
## ИЗМЕНЕНИЯ С 02.05.2026 (v32 → v33)

**CRM фаза 2 — контакты, создание альбома, настройка этапов, DnD (02.05.2026)**
- Контакты в карточке клиента: загрузка при выборе клиента, CRUD (добавить/редактировать/удалить)
- Создание альбома прямо из сделки: кнопка "+ Альбом" на карточке, форма с предзаполнением
- Настройка этапов воронки: новая вкладка ⚙️, переименование, цвет, порядок, флаг "закрыт"
- Drag & Drop карточек через @dnd-kit: перетаскивание между колонками, ghost-карточка, подсветка колонки
- fix: дропдаун "Переместить" через fixed positioning (не обрезался колонкой)

**Свайп фото в лайтбоксе (02.05.2026)**
- Свайп переписан на useRef (был useState-хак — глючил)
- Вертикальный скролл не мешает горизонтальному свайпу
- Зелёная галочка на выбранных миниатюрах (с fix: relative на кнопке)
- Кнопка выбора остаётся на том же фото после нажатия (без автоперехода)

## ИЗМЕНЕНИЯ С 02.05.2026 (v31 → v32)

**CRM v1 — клиенты, воронка, задачи (02.05.2026)**
- app/api/crm/route.ts — новый API endpoint
  GET: clients, client_detail, stages, deals, tasks, albums_list, team_members
  POST: create/update/delete client, create/update/delete contact,
        create/update/move/delete deal, create/complete/delete task, update_stages
- app/app/CRMModal.tsx — полный UI модалки (3 вкладки)
  Воронка: Kanban-доска по этапам, карточки с быстрым перемещением,
           форма создания/редактирования сделки с привязкой к клиенту и альбому
  Клиенты: список школ с поиском, карточка с историей сделок, заметки, теги
  Задачи: создание с дедлайном, привязка к сделке/клиенту,
          группировка Просрочено/Сегодня/Предстоящие, фильтр Мои/Все
- crm-migration.sql — миграция БД (clients, contacts, deal_stages, deals, tasks)
  + unique index на deal_stages(tenant_id, name)
- Кнопка CRM добавлена в toolbar /app
- Этапы воронки создаются автоматически при первом открытии:
  Лид → Переговоры → Договор → Съёмка → Отбор фото → Верстка → Готово → Закрыто
- fix: ensureStages вызывается только из /stages endpoint (не из /deals),
  предотвращает дублирование этапов при параллельных запросах
...
**1. Аналитика альбомов** — полезно уже сейчас в активный сезон:
- Воронка отбора: не начал → в процессе → завершил
- Конверсия по альбомам, динамика по дням
- Финансовая сводка по сделкам CRM

**2. Чистка кода**
- Удалить /api/img/ прокси (все пути yc:, прокси не нужен)
- Логотипы tenant'ов перенести в YC
- Удалить ADMIN_SECRET из Vercel env

**4. Рост платформы**
- Биллинг — ЮKassa, тарифы, автопродление
- SMS/Email рассылка — SMSC.ru, Postmark
- Мобильное приложение фотографа

**5. Переезд с Vercel (осень 2026)**
