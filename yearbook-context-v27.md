...
# Обновлено: 17.04.2026 (этапы 4.a + 4.b закрыты, 4.c следующий)
...
### Этап 4.c — Редирект /admin → /app для JWT-пользователей (следующий)

Обратимый шаг: если что-то сломается, удаление одного блока кода
мгновенно возвращает старое поведение.

- app/admin/page.tsx: на старте проверяет наличие JWT через GET /api/auth.
  - Если залогинен через JWT → router.push('/app')
  - Если только ADMIN_SECRET или нет авторизации → остаётся legacy /admin
- Это делает /app основным входом для мигрировавших пользователей,
  но /admin продолжает работать как fallback через ADMIN_SECRET.
- После 4.c нужно подождать 1-2 недели, посмотреть на production
  чтобы убедиться что никто не жалуется на /app.

### Этап 4.d — Удаление legacy-кода (финал)

НЕОБРАТИМЫЙ шаг. Выполняется ТОЛЬКО после подтверждения что все
пользователи успешно работают через /app без жалоб.
