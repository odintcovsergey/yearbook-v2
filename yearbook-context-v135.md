...
# Версия: v135
# Обновлено: 21.05.2026 (РЭ.27.8 — ФАЗА РЭ.27 ЗАКРЫТА).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v135
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v135 — РЭ.27 ЗАКРЫТА. Все 9 подэтапов + 4 урока зафиксированы.
#      • docs/phase-Р27-summary.md — полный summary фазы (318 строк).
#      • docs/phase-Р24-spec.md §14 — описание будущих фаз РЭ.28/РЭ.29
#        (партнёрские дизайны, сценарии A и B).
#   4. Применены в Supabase: все миграции до РЭ.27.7b включительно ✅
#   5. ДАЛЕЕ — три варианта:
#
#      A) РЭ.28 — партнёрские дизайны (сценарий A: клон template_set
#         с override размеров).
#         - 5-7 коммитов, проще чем РЭ.27.
#         - Engine уже умеет proportional placeholder resize.
#         - После РЭ.27 партнёр копирует ОДИН пресет «Стандарт» вместо
#           двух layflat/soft — модель данных проще.
#         - Pre-condition для партнёров с собственной типографией.
#
#      B) РЭ.29+ — партнёрский IDML (сценарий B: полный custom
#         template_set из IDML-файла партнёра).
#         - Большая фаза без чёткого скоупа.
#         - Открывается по реальному запросу партнёра.
#         - НЕ начинать без чёткой потребности.
#
#      C) Пауза — тестирование РЭ.27 на боевых альбомах.
#         - Создать новый soft-альбом через UI, убедиться что:
#           • Селект «Тип листов» в форме работает
#           • Плашка «📖 Мягкий переплёт» показывается в редакторе
#           • Заглушки «Форзац» слева на первом и справа на последнем
#             развороте
#           • При попытке выбрать spread-мастер (если такие есть)
#             в TemplatePickerModal — серый с тултипом
#         - Создать новый layflat-альбом — поведение как раньше
#           (плашка не показывается, заглушки не рисуются).
#         - Проверить existing 12 альбомов — engine собирает по
#           правильному print_type.
#
#      Рекомендация: вариант C (тестирование) → потом A (РЭ.28).
#
#      ⚠️ ВАЖНОЕ ПРАВИЛО ИЗ УРОКОВ РЭ.27:
#      Перед каждым новым подэтапом, особенно с миграцией данных:
#      1. SELECT column_name, data_type FROM information_schema.columns
#         WHERE table_name = '...';
#      2. Test JOIN на 5 строках.
#      3. Только потом писать код миграции/запрос.
#      Это не «принцип», а обязательное действие.
