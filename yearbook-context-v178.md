...
# │
# │ 7. **ФИНАЛЬНОЕ ПРЕВЬЮ ВЫБОРА** (новая задача, зафиксирована
# │    28.05.2026 по обсуждению с Сергеем):
# │
# │    КОНЦЕПЦИЯ:
# │      После того как родитель завершил отбор фото и нажал
# │      «Подтвердить» — показать ему как именно будет выглядеть его
# │      личный разворот в альбоме. Не абстрактная сводка («вы
# │      выбрали 5 фото»), а РЕАЛЬНЫЙ визуальный превью с подставлен-
# │      ными фото и текстом ребёнка.
# │
# │    ЦЕЛИ:
# │      • Снижение претензий после печати — родитель видел и
# │        подтвердил визуально
# │      • Усиление эмоциональной связи с продуктом — «вау, мой
# │        ребёнок»
# │      • Дифференциация от Wfolio и конкурентов — у них такого нет
# │      • Возможный апсейл — родитель видит что страница «пустая»,
# │        хочет добавить фото
# │
# │    ПРОДУКТОВЫЕ РЕШЕНИЯ ОТ СЕРГЕЯ:
# │      • Показывать ТИПИЧНЫЙ вариант разворота для комплектации
# │        (без учёта left/right — точная сторона определяется при
# │        сборке всего класса). С дисклеймером мелким шрифтом.
# │      • Личный разворот + обложка. Если общего фото класса нет —
# │        плейсхолдер «здесь будет фото класса от фотографа».
# │      • Лимиты пересмотров не нужны — родитель скорее всего
# │        посмотрит 1 раз и подтвердит, бесконечный цикл маловероятен.
# │      • Текущая «сводка выбора» (что выбрал) уже есть. Задача —
# │        ДОБАВИТЬ визуальный рендер разворота, не делать сводку с нуля.
# │
# │    ВАЖНО ПРО ОБЛОЖКУ:
# │      Сергей уточнил: обложка ≠ S-Intro/soft_intro. S-Intro и
# │      soft_final — это первая/последняя страницы ВНУТРИ альбома
# │      на мягких листах. Обложка (физическая обложка книги) — это
# │      отдельный объект, который в текущей системе НЕ ФОРМАЛИЗОВАН
# │      через мастера. Обложка завязана на дизайн (template_set),
# │      содержит либо общее фото класса (которое нигде в системе
# │      не выбирается явно — отдельная архитектурная проблема), либо
# │      портрет ученика, либо элемент в стиле дизайна.
# │
# │      Для первой итерации финального превью можно показывать
# │      ТОЛЬКО личный разворот ученика — это и есть главная ценность.
# │      Обложку отложить — там нужно сначала формализовать как
# │      она хранится и выбирается в системе.
# │
# │    ТЕХНИЧЕСКОЕ РЕШЕНИЕ:
# │      Рендер выполняется в БРАУЗЕРЕ родителя через переиспользование
# │      существующего Konva-компонента AlbumSpreadCanvas.tsx (который
# │      уже работает в редакторе партнёра).
# │
# │      Это решает проблему производительности — серверная нагрузка
# │      нулевая. Никаких Puppeteer, никаких затрат на Vercel
# │      functions, никакого кэширования картинок. Просто read-only
# │      режим существующего канваса.
# │
# │      Минусы: на старых мобильных может тормозить, нет картинки
# │      для шеринга. Для MVP — приемлемо.
# │
# │    ЧТО НУЖНО СДЕЛАТЬ:
# │      1. API /api/preview/[token] (~3-4 часа):
# │         - Достать ученика + альбом + комплектацию
# │         - Выбрать ТИПИЧНЫЙ мастер личного разворота для density
# │           альбома (например E-Universal-Left для max/universal,
# │           N-Grid-Page или N-Grid-Page-9 для mini — то что наиболее
# │           вероятно увидит ученик)
# │         - Подготовить data{} с фото и текстом ребёнка
# │      2. Компонент AlbumSpreadPreview (~6-8 часов):
# │         - Переиспользовать AlbumSpreadCanvas в read-only режиме
# │         - Адаптивность под мобильные (Konva resize)
# │         - Без drag, без редактирования, чисто отображение
# │      3. Финальный экран подтверждения (~3-4 часа):
# │         - Кнопка «Подтвердить» теперь ведёт на новый экран
# │         - Превью разворота + подпись «Точная страница определится
# │           при сборке альбома»
# │         - Кнопки «Подтвердить выбор» / «Изменить»
# │      4. Сохранение подтверждения (~2-3 часа):
# │         - Финальный submit, защита от повторной отправки
# │         - Возврат в редактор при «Изменить»
# │      5. Тестирование на разных комплектациях (~3-4 часа):
# │         - Maximum, Universal, Standard, Medium, Light, Mini
# │         - Когда у комплектации Mini нет личного разворота
# │           (только сетка) — что показывать?
# │
# │    ИТОГО: ~17-23 часа работы в Claude Code (3-4 сессии).
# │
# │    КОГДА ДЕЛАТЬ:
# │      После закрытия J-Combined-Tail (когда Сергей нарисует 5
# │      недостающих мастеров). До коллажей и адаптации форматов —
# │      превью даст более ощутимый UX-эффект для родителей.
# │
# │    РИСК ПРОДУКТОВЫЙ:
# │      Родители увидев превью могут начать просить «можно фото
# │      побольше», «можно другой шрифт», «не нравится композиция».
# │      Решение: мелким шрифтом дисклеймер «дизайн альбома фиксиро-
# │      ванный, ваш выбор — фото и текст». Никаких обещаний что
# │      можно поменять вёрстку.
# │
# │ 8. **АДАПТАЦИЯ ПОД РАЗНЫЕ ФОРМАТЫ ТИПОГРАФИЙ** — новая             │
