...
- ✅ Набор РАСШИРЕН (21.06): к 9 синтетическим (классы фич) добавлены **24
  реальные уникальные формы эмбедов** из рантайм-кода (`app/`+`lib/`) — все
  `.select('…(…)…')` из CRM/детей/учителей/обложек/selections, с привязкой к
  месту в коде. Итого **33 запроса**, в self-test (A vs A на Supabase) 33/33 PASS
  — формы корректны и детерминированы. Боевую сверку A-vs-B прогнать, когда
  поднимется PostgREST на сервере (`TIMEWEB_REST_URL=…`). Источник форм собран
  дедупликацией по строке select; список в `scripts/compare-rest-backends.mjs`
  (`REAL_EMBEDS`).
