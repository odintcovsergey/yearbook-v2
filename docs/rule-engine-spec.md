...
**Версия:** 1.2
**Дата:** 16.05.2026 (после сверки с реальным IDML и решения про комбинированные мастера)
...
- `docs/templates/designer-tz-2026-05-16.md` v1.4 — словарь меток placeholder'ов после сверки с IDML
...
**Связанные документы:**
- `docs/templates/designer-tz-2026-05-16.md` (v1.4) — актуальное ТЗ дизайнеру (включая 3 комбинированных мастера M/L/N-Combined-Page)
...
### А.14. Комбинированные мастера M/L/N-Combined-Page (v1.2)

**Проблема**: для density medium/light/mini при неполном остатке учеников типовая верстка OkeyBook — портреты вверху страницы + одно общее фото внизу той же страницы (пример: Light с 9 учениками → правая страница содержит 3 портрета вверху + общее фото внизу). Postpage-модель v1.1 с разворотами со смешанными страницами эту композицию не покрывала — она могла дать только «L-Grid-Page слева + J-Full справа» (разные страницы), но не «портреты + общее на ОДНОЙ странице».

**Решение**: добавлено подсемейство **комбинированных мастеров** в `student-section`:
- `M-Combined-Page` (medium): 4 портрета вверху + 1 общее фото внизу
- `L-Combined-Page` (light): 6 портретов вверху + 1 общее фото внизу
- `N-Combined-Page` (mini): 12 портретов вверху + 1 общее фото внизу

Все три — `page-any`, параметрические (число верхних портретов меняется от 1 до полного, балансировка скрывает лишние). `classphotoframe` остаётся всегда.

**Где НЕ применяются**: для Standard/Universal/Maximum (density max/universal/standard) комбинированные мастера НЕ добавляются. Одинокий ученик в этих плотностях обрабатывается классическим разворотом со смешанными страницами (E-* слева + J-* справа) — это исторически как верстает OkeyBook.
...
- Правило `student-section` с priority выше базового срабатывает когда: `students_remaining < capacity_per_side` + `common_photos.full_class.count >= 1` → выбирает Combined-вариант, привязывает оставшихся учеников + одно общее фото, увеличивает счётчик потреблённых общих фото
- Базовое правило (без Combined) остаётся для случаев когда общего фото нет

**В UI**: при замене шаблона партнёр в `TemplatePickerModal` для страницы student-section с density=light видит оба варианта — `L-Grid-Page` (только портреты) и `L-Combined-Page` (портреты + общее). Может переключаться вручную.
