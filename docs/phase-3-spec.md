...
Noto Serif + Open Sans + Slimamif) — Konva начинает рисовать как
InDesign.
...
app/app/
  ...
  _components/
    AlbumSpreadCanvas.tsx                 — реальные fontFamily (3.8)
    ExportPanel.tsx                       — 🆕 dropdown профилей + кнопка + история (3.7)
  album/
    [id]/
      layout/
        page.tsx                          — реальные fontFamily (3.8)
public/fonts/
  NotoSerif-Regular.ttf                   — 🆕 (3.2) — основной текстовый
  NotoSerif-Bold.ttf                      — 🆕 (3.2) — заголовки/имена
  OpenSans-Regular.ttf                    — 🆕 (3.2) — описания/подписи
  OpenSans-Italic.ttf                     — 🆕 (3.2) — цитаты
  Slimamif-Medium.ttf                     — 🆕 (3.2) — декоративные надписи
app/globals.css                           — 🆕 @font-face Noto Serif/Open Sans/Slimamif (3.8)
```
...
**Шрифты в репо:** `public/fonts/` — три семейства, 5 файлов
суммарно ~1.5-2 МБ:

- **Noto Serif** (Google Fonts, SIL OFL) — Regular + Bold. Основной
  текстовый шрифт макета (имена учеников, заголовки).
- **Open Sans** (Apache 2.0) — Regular + Italic. Описания и цитаты.
- **Slimamif** (FFC, Dmitri Zdorov, free for commercial use) — Medium.
  Декоративные/handwritten надписи.

Все три имеют полную поддержку кириллицы. Все три open-source — можно
коммитить в репо без лицензионных проблем.
...
@font-face {
  ...
  src: url('/fonts/Slimamif-Medium.ttf') format('truetype');
  font-weight: 500;
  font-display: swap;
}
```

В `AlbumSpreadCanvas.tsx`: `fontFamily="${placeholder.font_family}, serif"` —
fallback на serif если шрифт не загрузился. Document.fonts.ready
...
export async function loadFonts(pdfDoc: PDFDocument): Promise<FontRegistry> {
  ...
  const fonts = {
    'noto-serif-regular': await pdfDoc.embedFont(
      await fs.readFile(path.join(fontDir, 'NotoSerif-Regular.ttf')),
      { subset: true }
    ),
    'noto-serif-bold': await pdfDoc.embedFont(
      await fs.readFile(path.join(fontDir, 'NotoSerif-Bold.ttf')),
      { subset: true }
    ),
    'open-sans-regular': await pdfDoc.embedFont(
      await fs.readFile(path.join(fontDir, 'OpenSans-Regular.ttf')),
      { subset: true }
    ),
    'open-sans-italic': await pdfDoc.embedFont(
      await fs.readFile(path.join(fontDir, 'OpenSans-Italic.ttf')),
      { subset: true }
    ),
    'slimamif-medium': await pdfDoc.embedFont(
      await fs.readFile(path.join(fontDir, 'Slimamif-Medium.ttf')),
      { subset: true }
    ),
  }
  return {
    resolve(family: string, weight: string, italic: boolean): PDFFont {
      const key = resolveKey(family, weight, italic)  // см. ниже
      const font = fonts[key]
      if (font) return font
      // Fallback: запрошенное семейство неизвестно — используем
      // Noto Serif Regular + warning
      ctx.warnings.push({code: 'font_not_found', detail: `${family} ${weight}`})
      return fonts['noto-serif-regular']
    }
  }
}

function resolveKey(family: string, weight: string, italic: boolean): string {
  const f = family.toLowerCase()
  if (f.includes('noto serif')) return weight === 'bold' ? 'noto-serif-bold' : 'noto-serif-regular'
  if (f.includes('open sans')) return italic ? 'open-sans-italic' : 'open-sans-regular'
  if (f.includes('slimamif')) return 'slimamif-medium'
  return 'noto-serif-regular'  // unknown family fallback
}
```

`subset: true` — pdf-lib встраивает только использованные глифы (~50 KB
на семью вместо ~400 KB).

**Дефолт парсера:** в `lib/idml-converter/extract-styles.ts`
`TEXT_STYLE_DEFAULTS.font_family` сейчас `'Geologica'`. Это
defensive-fallback для случая когда AppliedFont в IDML не указан.
В рабочих 39 мастерах okeybook-default парсер должен правильно извлечь
Noto Serif/Open Sans/Slimamif. Если в БД обнаружатся placeholders с
`font_family='Geologica'` — это сигнал что парсер где-то fallback'ает,
и мы либо чиним парсер, либо меняем дефолт. Проверка SQL-запросом
выполняется до старта 3.2.
...
**Нюанс кириллицы:** Все три семейства (Noto Serif, Open Sans, Slimamif)
поддерживают кириллицу полностью. Если будут проблемы — fallback на
Noto Serif Regular как универсальный.
...
| 3.2 | Фундамент `lib/pdf-export/`: типы, units, font-loader, sharp wrapper, pdf-lib обёртка + 5 TTF в public/fonts/ | `lib/pdf-export/{types,units,font-loader,index}.ts`, `public/fonts/{NotoSerif,OpenSans,Slimamif}-*.ttf`, `package.json` (+ pdf-lib, sharp) | `feat(layout/3.2): фундамент lib/pdf-export (pdf-lib + sharp + Noto Serif/Open Sans/Slimamif)` |
...
| 3.8 | Шрифты в Konva: `@font-face` Noto Serif/Open Sans/Slimamif + замена fallback Arial | `app/globals.css`, `app/app/_components/AlbumSpreadCanvas.tsx`, `app/app/album/[id]/layout/page.tsx` | `feat(layout/3.8): реальные шрифты в Konva (редактор + LayoutPreviewStrip)` |
...
- ✅ Клик с профилем `okeybook-client-preview` создаёт PDF 5-10 МБ
  без bleed.
- ✅ В PDF используются реальные шрифты Noto Serif / Open Sans /
  Slimamif (визуально совпадает с InDesign-выводом).
...
- ✅ В Konva-редакторе и в LayoutPreviewStrip текст рисуется реальными
  шрифтами Noto Serif / Open Sans / Slimamif (а не Arial fallback).
