# Инструкция для подэтапа 0.11.2 — overflow Лайт и Мини

> Это временный файл-инструкция. После выполнения подэтапа удалить и закоммитить вместе с основными изменениями.

## Цель подэтапа

Реализовать **overflow-логику** для Лайт (25-32 учеников) и Мини (25-36 учеников). Базовая часть (≤24) уже работает в 0.11.1.
...
## Контекст (что изменилось после 0.11.1.5)

После архитектурного фикса 0.11.1.5 фильтрация по `default_for_configs` гарантирует:
- Для Лайт `pickAdaptiveGrid` видит только L-* мастера
- Для Мини видит только N-* мастера

Поэтому **clamping через `pickAdaptiveGridMaxCapacity`** работает прозрачно: для Лайт max=L-6 (capacity=6), для Мини max=N-12 (capacity=12). Никаких пересечений между комплектациями.

Также после 0.11.1.5 нет нужды в `slot_capacity_min: { students: 3 }` для дискриминации L-Overflow-Row vs N-Overflow-Row — у них теперь разные `default_for_configs` (`['light']` vs `['mini','individual']`). Просто `page_role='student_overflow'` + `applies_to_config: 'light'` или `'mini'` — и фильтр найдёт правильного.

## Логика overflow (из build_album.jsx)

### Лайт (basePages=4, slotsPerPage=6)
- **25-27** (overflow 1-3): 4 базовых L-6 + 1× L-Overflow-Row (с classphotoframe)
- **28-30** (overflow 4-6): 4 базовых L-6 + 1× extra L-6-Left (5-я страница, неполная)
- **31-32** (overflow 7-8): 4 базовых L-6 + 1× полная L-6-Left + 1× L-Overflow-Row-Right

### Мини (basePages=2, slotsPerPage=12)
- **25-28** (overflow 1-4): 2 базовых N-12 + 1× N-Overflow-Row (с classphotoframe)
- **29-36** (overflow 5-12): 2 базовых N-12 + 1× extra N-12-Left (3-я страница, неполная)

### Универсальный алгоритм
```
overflow = total - basePages * slotsPerPage
rowCapacity = overflow_row_master.slot_capacity.students  (3 для L, 4 для N)
...
if overflow ≤ 0:
  ...
  ветка extra_grid (одна обычная сетка с пустыми слотами, leftMaster)
else:
  ветка grid_plus_row (полная сетка + overflow-row-right; только Лайт 31-32)
```

## Состояние БД (миграция уже применена)

**Page roles:**
```
L-Overflow-Row:        page_role=student_overflow,       default_for_configs=['light'],            slot_capacity={students:3, photos_full:1}
L-Overflow-Row-Right:  page_role=student_overflow_right, default_for_configs=['light'],            slot_capacity={students:3, photos_full:1}
N-Overflow-Row:        page_role=student_overflow,       default_for_configs=['mini','individual'], slot_capacity={students:4, photos_full:1}
```

Миграция БД для `student_overflow_right` уже применена в Supabase Studio через первую попытку 0.11.2. Файл миграции **нужно создать** в этом подэтапе (см. шаг 1) для repeatability.
...
Новый файл (миграция уже выполнена в Supabase Studio):
...
И обновить ссылку на миграции: `(миграции 0.8.6.1 + 0.10a.1 + 0.10b.1 + 0.11.1.5 + 0.11.2)`.
...
**Примечание:** благодаря 0.11.1.5 (default_for_configs=['light'] на L-* мастерах), не нужен `slot_capacity_min: { students: 3 }` для дискриминации — фильтр уже видит только L-Overflow-Row (а не N-).
...
### 4.1 Новая функция `pickAdaptiveGridMaxCapacity`
...
### 4.2 Полная переписка `buildAdaptiveGridStudents`
...
### 4.3 Новая функция `buildOverflowRow`
...
### 5.2 Добавить 8 сцен overflow в `SCENES`
...
1. Создать `migrations/2026-05-07-page-role-overflow-right.sql`
2. Внести правки в 4 файла кода (types.ts, scenarios.ts, build.ts, scripts/smoke-album-builder.ts)
3. `cp .env.local .env.local.real-backup`
4. fake `.env.local`
5. `npx tsc --noEmit --project .` → пусто
6. `npx next build` → зелёный
7. `rm -f .env.local && mv .env.local.real-backup .env.local`
8. `set -a && . ./.env.local && set +a && npx tsx scripts/smoke-album-builder.ts`
   → `Result: 44/44 scenes passed`, exit 0
9. Если меньше — НЕ коммить, репортни в чат конкретные сцены
10. Удалить файл-инструкцию: `rm docs/internal/0.11.2-instructions.md`
    `rmdir docs/internal 2>/dev/null || true`
11. `git add lib/album-builder/ scripts/smoke-album-builder.ts migrations/ docs/internal/`
12. `git status` — должно быть:
    ...
    ```
13. `git commit -m "feat(layout/0.11.2): overflow Лайт (25-32) и Мини (25-36) + page_role student_overflow_right"`
14. **НЕ пушить** — дождись OK от Сергея
...
- Никаких костылей с поиском по имени (`findOverflowRowRight` забыть навсегда)
- Никаких полей `base_slots_per_page`, `base_max_capacity` и подобных — clamping через `pickAdaptiveGridMaxCapacity`
