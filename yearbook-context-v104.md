...
# Версия: v104
# Обновлено: 20.05.2026 (после РЭ.23.1 — миграция display_label).
#
# ФАЗА РЭ.23 — Визуальный каталог мастеров (В РАБОТЕ, начата 20.05.2026)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.23.0 (5d6a276) — docs/phase-Р23-spec.md v1.0 (157 строк).
#    Все 5 архитектурных развилок зафиксированы:
#      A: SVG (не PNG), B: новое поле (не jsonb), C: отложить
#      партнёров, D: разворот в одной SVG, E: стартуем сейчас.
# ✅ РЭ.23.1 (e4b7a2f) — миграция БД.
#    migrations/2026-05-20-spread-templates-display-label.sql:
#    ALTER TABLE spread_templates ADD COLUMN display_label TEXT NULL.
#    Идемпотентная, не блокирующая, NULL допустим (UI fallback на name).
#    ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.23.3.
#
# ⏳ Следующий подэтап: РЭ.23.2 — утилита renderPreviewSvg + тесты.
#    Чистая функция в lib/album-builder/render-preview-svg.ts:
#      placeholders + page-size → SVG string
#    Без зависимостей от React/Konva. Тестируется в vitest без браузера.
#    Превью: серые прямоугольники для фото, серые линии для текстов,
#    соблюдены пропорции. Двухстраничные мастера (is_spread=true) —
#    обе страницы рядом + промежуток-сгиб.
#
# Контекст фазы:
# После РЭ.22 engine работает через семантические теги, но партнёр и
# админ не видят свою библиотеку мастеров глазами. РЭ.23 — инфраструктура:
#   - SVG-превью (РЭ.23.2)
#   - API + страница /super/master-catalog (РЭ.23.3-23.4)
#   - Разметка мастеров display_label'ами (РЭ.23.5)
# После РЭ.23 готовы РЭ.24 (готовые пресеты для быстрого старта) и
# РЭ.25 (конструктор с галереей вариантов).
#
# Стартовая линейка пресетов Сергея для РЭ.24 (зафиксирована 20.05.2026):
#   мини, лайт, медиум, стандарт, универсал, максимум, индивидуальный.
# Это его рабочие пресеты которые он продаёт много лет.
#
# Логика РЭ.24-25 (зафиксирована 20.05.2026):
#   Путь A: партнёр заходит, видит 3-4 готовых пресета, жмёт «Использовать».
#           Минимально-валидный способ работы с платформой.
#   Путь B: партнёр хочет кастом — открывает конструктор, видит галерею
#           вариантов на каждую роль (cover, student_*, teacher_*, и т.д.),
#           выбирает альтернативные layout'ы. По образцу фотобота.
#
# 🎉 ФАЗА РЭ.22 ЗАКРЫТА — двух-осевая модель + семантический engine
# во всех 4 секциях. Summary: docs/phase-Р22-summary.md (158 строк).
# 26 коммитов на main, 4 миграции, 4 скрытых бага, +28 тестов
# (332 → 360). 0 регрессий.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v104
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v104 — что закрыто (РЭ.23.0+23.1)
#      • docs/phase-Р23-spec.md v1.0 — полный план фазы (157 строк)
#      • docs/phase-Р22-summary.md — что было сделано в предыдущей фазе
#   4. Применены в Supabase (все 4 миграции РЭ.22 + новая РЭ.23.1):
#      • Все миграции до v87 ✅
...
#      • 2026-05-20-spread-templates-display-label.sql ⬜ РЭ.23.1 —
#        ПРИМЕНИТЬ ДО деплоя РЭ.23.3.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.23.2 — утилита renderPreviewSvg.
#      Файлы:
#        - lib/album-builder/render-preview-svg.ts (новый):
#          Чистая функция renderPreviewSvg(template: SpreadTemplate): string
#          Параметры мастера:
#            template.placeholders — массив Placeholder (x_mm, y_mm,
#              width_mm, height_mm, type='photo'|'text', label)
#            template.width_mm, template.height_mm — размер страницы
#            template.is_spread — двухстраничный мастер? (если да —
#              рендерим оба разворота рядом с промежутком-сгибом 4mm)
#          Выход: SVG-строка готовая для встраивания через
#                 dangerouslySetInnerHTML или прямого вывода.
#          Стили: fill='#d1d5db' для фото, серая линия для текстов
#                 (короткая если auto_fit=false, длинная если true).
#          viewBox должен быть в mm (или мм*scale), фиксированной
#          высоты не задаём — SVG масштабируется через CSS контейнера.
#        - lib/album-builder/__tests__/render-preview-svg.test.ts (новый):
#          5-8 тестов: один портрет, разворот (две страницы), grid
#          из 4 учеников, мастер с classphotoframe, мастер с текстами,
#          edge-case с пустым placeholders.
#      Что НЕ трогаем: engine, API, UI — это РЭ.23.3-4.
