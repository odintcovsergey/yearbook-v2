...
# Версия: v116
# Обновлено: 20.05.2026 (после РЭ.24.6 — модалка выбора шаблона при создании альбома).
#
# ФАЗА РЭ.24 — Готовые шаблоны для быстрого старта партнёра (ПОЧТИ ЗАКРЫТА)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.24.0..24.5 — основная фаза закрыта
# ✅ РЭ.24.5b — multi-design расширение
# ✅ РЭ.24.7 — галка is_recommended в редакторе пресета
# ✅ РЭ.24.6 (3433a13) — модалка выбора шаблона при создании альбома:
#    • UI: новое поле 'Шаблон' в AlbumFormFields рядом со старыми
#      селектами config_type + print_type
#    • Старые селекты затемняются (opacity 50%) когда выбран новый шаблон
#    • TemplatePickerModal: фильтр по дизайну + Мои/Готовые + disabled
#      невалидные + 'Создать без шаблона'
#    • Confirm-диалог при смене/снятии существующего шаблона
#      (предупреждение о drafts и пропорциях)
#    • create_album: section_structure_preset_id приоритетнее legacy
#      preset_slug, template_set_id и print_type из preset'а
#    • update_album: обработка null (снятие) + строки (валидация
#      доступа + auto-resolve template_set_id и print_type)
#
# ⏳ Следующий и финальный подэтап: РЭ.24.8 — summary фазы + закрытие.
#    Файл: docs/phase-Р24-summary.md (новый, ~150-200 строк по образцу
#    docs/phase-Р22-summary.md и Р23-summary.md).
#    Контент: что сделано, ключевые архитектурные решения, нерешённые
#    долги (§13 print_type, §14 партнёрские дизайны), статистика
#    (коммиты, тесты, файлы), известные баги если есть.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v116
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v116 — что закрыто (РЭ.24 почти вся)
#      • docs/phase-Р24-spec.md v1.3 — полная фаза, §13 и §14 — долги
#      • lib/presets/validate.ts + preview-bundle.ts — готовые утилиты
#      • app/app/templates/page.tsx (выбор дизайна) + [designId]/page.tsx
#        (каталог шаблонов внутри дизайна)
#      • app/app/page.tsx — AlbumFormFields + TemplatePickerModal (РЭ.24.6)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1 ✅
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.24.8 — summary + закрытие фазы.
#      Файл: docs/phase-Р24-summary.md (новый, по образцу
#      docs/phase-Р22-summary.md и Р23-summary.md).
#      Что включить:
#        • Что сделано — список 7+1 подэтапов с коммитами
#        • Архитектура multi-design (template_set = дизайн)
#        • Каталог шаблонов (двухуровневая навигация)
#        • Модалка выбора шаблона при создании альбома
#        • Скрытые баги (2) которые закрыты по ходу
#        • Зафиксированные долги: §13 print_type→альбом (РЭ.27),
#          §14 партнёрские дизайны (РЭ.28+), РЭ.25 галка покупки
#        • Статистика: коммиты, тесты, файлы
#        • Известные аномалии данных (L-Grid-12)
#      Длина: ~150-200 строк. Это завершающий документ фазы.
#      После коммита — контекст v117 → следующие фазы (РЭ.25 или РЭ.27).
#   6. План фаз вперёд (зафиксирован):
#      • РЭ.25 — галка покупки альбома ребёнком (children.is_purchased).
#        Маленькая фаза 1-2 дня. Engine не включает не-покупателей в
#        студенческую секцию. Это пункт #4 из наблюдений по фотоботу.
#      • РЭ.27 — print_type в АЛЬБОМЕ, не в пресете. Большая фаза
#        ~10-15 коммитов. Редактор переключается между layflat
#        (разворотный режим) и soft (постраничный). См. spec §13.
#      • РЭ.28 — партнёрские дизайны (сценарий A): копирование
#        глобального template_set с переопределением размеров под
#        свою типографию. ~5-7 коммитов, см. spec §14.
