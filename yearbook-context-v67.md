...
# Версия: v67
# Обновлено: 16.05.2026 (РЭ.13 закрыт — POST /api/layout?action=build_album_test_rules
#                       endpoint + UI Build Test (Rule Engine) в /super/templates/[id].
#                       Rule engine MVP полностью доступен superadmin'у через
#                       UI для тестирования синтетических сценариев.)
#
...
# Rule engine MVP готов end-to-end:
#   ✅ Спецификация v1.3 + 7 семейств + 36 правил + 7 пресетов
#   ✅ Алгоритм buildFromRules (РЭ.9, ~1.5K строк)
#   ✅ 77 unit tests (РЭ.10, 132ms)
#   ✅ Endpoint + UI Build Test (РЭ.13)
# Подэтапы РЭ.1+РЭ.2+РЭ.3+РЭ.3.5+РЭ.4+РЭ.5+РЭ.6+РЭ.7+РЭ.8+РЭ.9+РЭ.10+РЭ.13
# закодированы. РЭ.11+РЭ.12 — UI для редактирования (отложены, не блокеры).
#
# 📍 КАК ПРОТЕСТИРОВАТЬ RULE ENGINE В UI:
#   1. Залить правила/пресеты: npx tsx --env-file=.env.local scripts/seed-rule-engine.ts --write
#      (Сергей: делать на боевой Supabase — однократно)
#   2. Зайти в /super/templates/[id] как superadmin
#   3. Открыть фиолетовый «▼ Build Test (Rule Engine)»
#   4. Выбрать preset, задать N учеников/предметников/общие фото
#   5. Нажать Build → видим:
#      - status (ok/partial/failed)
#      - decision_trace (какое правило когда применилось)
#      - spreads (left+right master_name + bindings)
#      - rules_version (детерминированный хэш)
#
# Следующий шаг: проверить рукой на нескольких сценариях, после чего
# либо РЭ.11 (TemplatePickerModal фильтрация по family_id), либо
# подключение buildAlbumOrFallback к боевому /api/layout?action=build_album
# для реальных альбомов (миграционный режим).
...
#    Подэтапы РЭ.1-РЭ.10 + РЭ.13 ✅ выполнены, РЭ.11-РЭ.12 отложены.
#
