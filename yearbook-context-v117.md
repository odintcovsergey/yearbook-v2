...
# Версия: v117
# Обновлено: 20.05.2026 (после РЭ.24.8 — фаза РЭ.24 ЗАКРЫТА).
#
...
#
# ⏳ Следующая фаза: РЭ.25 — галка покупки альбома ребёнком.
#    Маленькая фаза 1-2 дня. По пункту #4 из обсуждения наблюдений
#    по фотоботу (сессия после деплоя РЭ.24).
#    План:
#      • Миграция БД: children.is_purchased BOOLEAN DEFAULT true
#        (true = по умолчанию — все ученики в альбоме, обратная
#        совместимость со старыми альбомами)
#      • UI: галка 'Заказывает альбом' в форме ученика в /app
#        (видно фотографу/админу при редактировании списка учеников)
#      • Также — галка для родителя в его странице ребёнка
#        (так как покупка решается родителем в момент отбора фото)
#      • Engine: фильтр в студенческой секции — не включать
#        is_purchased=false в personal pages / spread / grid
#      • Возможно — отображение количества в карточке альбома
#        ('15 из 25 заказали')
#    Перед началом РЭ.25 — spec по образцу phase-Р22-spec.md и Р23-spec.md.
#    Сначала фиксируем архитектурные развилки:
#      • Кто проставляет галку — фотограф вручную или родитель при заказе?
#      • Что с учениками без is_purchased — попадают или нет в учительский
#        раздел (где общие классные фото)?
#      • Что с виньетками класса — там должны быть только заказавшие
#        или все?
#      • Как мигрировать существующие альбомы (default true разумно).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v117
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v117 — фаза РЭ.24 ЗАКРЫТА (11 подэтапов, ~25 коммитов)
#      • docs/phase-Р24-summary.md — полное summary фазы (195 строк)
#      • docs/phase-Р24-spec.md v1.4 — спецификация с долгами §13/14/15
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1 ✅
#   5. СТАРТ ФАЗЫ РЭ.25 — галка покупки альбома ребёнком.
#      Подэтап 25.0: написать docs/phase-Р25-spec.md по образцу
#      docs/phase-Р22-spec.md и Р23-spec.md.
#      Перед началом — обсудить с Сергеем архитектурные развилки:
#        • Кто проставляет галку — фотограф вручную или родитель
#          при заказе?
#        • Что с учениками без is_purchased — попадают или нет
