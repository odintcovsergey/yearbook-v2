...
## 🆕 СЕССИЯ 27.06.2026 — РЭ.22 ЗАВЕРШЁН + ФАЗА 3 (проектирование, 5 аудитов, ТОЛЬКО ЧТЕНИЕ)

### ✅ DONE
- **РЭ.22 ЗАВЕРШЁН ПОЛНОСТЬЮ.** Движок раскладки полностью семантичен — выбор мастера по
  **`role` + `capacity` + `page_type`** ВЕЗДЕ в активном пути. Подэтапы:
  - **.9 common / slot-chains** → `findCommonMaster` (by-name → by-type). akvarel **14/14 идентично**.
  - **.10 transition** → `findComboTailMaster` (+ переиспользование `findCommonMaster`). akvarel + belly
    **132/132 идентично**, **риск-точка лево/право закрыта** (стороны через `page_type`).
  - **.11** сняты мёртвые `adaptiveTailNames`; `buildGrid` **оставлен как legacy safety-net** (mode=null,
    0/20 прод-пресетов, удалить после перезаливки дизайнов).
  - **.12 диагностика** — активных fallback-имён, вытесненных семантикой, **НЕ осталось**; правок не
    потребовалось.
  - **Канон (`master_page_types`) в движок НЕ тащили.** **Раскладка на проде не изменилась** (доказано
    сверкой spreads[] до/после на каждом подэтапе).

- **5 узких аудитов (только чтение) — определена продуктовая модель Фазы 3.** Кода не трогали; данные в
  т.ч. из живой прод-БД (Timeweb, таблица `presets`, 20 строк). Решения и выгрузки — в **Notion
  (карточка Фазы 3 + инвентаризация preset)**.

### 🧭 ФАЗА 3 — ПРОЕКТИРОВАНИЕ (модель определена, реализация НЕ начата)

- **Модель ЧЕТЫРЁХ осей (раздельные):**
  - **СТРУКТУРА** — **конкретная заготовка-продукт = «стоимость»**: состав секций + сетки + форзац +
    **правило старта `first_spread`**. Создаётся **выбором мастеров**; **конкретна, не абстрактна**;
    **`grid_size` ОСТАЁТСЯ в структуре**. Сейчас живёт в `presets`.
  - **ДИЗАЙН** — **оформление тех же структурных мастеров** + **плотность сеток** + **семейство
    пропорций** (квадрат / прямоугольник).
  - **ФОРМАТ** — **ось ЗАКАЗА**; адаптер uniform-scale (`adaptTemplateToFormat`) ПОСЛЕ раскладки.
  - **ПЕЧАТЬ** — физика заказа; **на раскладку НЕ влияет**.

- **Ключевые находки (5 аудитов):**
  - **`preset` УЖЕ почти = `structures`.** Дизайн развязан (`template_set_id` **3/20** заполнено —
    реальный дизайн приходит override'ом заказа), печать (`print_type`) живёт на `albums`. →
    **отдельная таблица может НЕ понадобиться**: Фаза 3 = **чистка + переименование + переосмысление
    `sheet_type`→`first_spread`**, а НЕ миграция в новую сущность.
  - **Переключение движка локализовано в `loadBundle`** (`lib/rule-engine/loaders.ts:49`). Ядро не трогаем.
  - **Боль «два выбора»** = убрать перезатирание `sheet_type` из `album.print_type` в
    **`layout/route.ts:1015-1020`** → выбор структуры станет единственным выбором.
  - **Правило старта живёт в структуре:** Булгак — **double** (двустраничный старт/конец),
    Окейкнига + Фабрика — **single** (слева форзац, справа 1-я стр.).
  - **Движок раскладки НЕ трогаем.**
  - **Мёртвое к чистке** (в новую модель не тянуть): `density` (0/20), `transition_scenario` (0/20),
    `pages_per_spread` (все = 2).

### 🚩 ЧТО ДАЛЬШЕ
- **Фаза 3 готова к написанию ФИНАЛЬНОГО ПРОЕКТА СХЕМЫ** (свежим заходом):
  1. `structures` vs чистка `presets` на месте;
  2. `sheet_type` → `first_spread`;
  3. убрать перезатирание (`layout/route.ts:1015-1020`);
  4. UI: индикатор старта (single/double) + предупреждение про сетку;
  5. механика создания / клонирования структур;
  6. порядок миграции **со сверкой раскладки**.
- **Backlog:**
  ...
  - **Полная типовая чистка legacy (Вариант B)** — `RuleEngineBundle.rules/.families`, `Rule`,
    `TemplateFamily`, `Preset.sections` + **~25 тест-фикстур**.
  - **Предусловие belly:** перезалить (2 NULL-мастера `J-Collage-5` / `J-J-Combined-Tail-2-Right`).
