...
# Версия: v208
# Обновлено: 27.06.2026 — Фаза 3: модель раскола preset определена через 5 аудитов (проектирование, БЕЗ кода).
#            Ранее (26.06 вечер): РЭ.22.12 закрыт без правок → РЭ.22 ЗАВЕРШЁН (движок полностью семантичен).
# HEAD: a30287c (main) на момент записи (5 аудитов Фазы 3 — только чтение, кода не меняли).
#
# Предыдущий контекст: yearbook-context-v207.md (история не переписывается).

---

## 🆕 СЕССИЯ 27.06.2026 — ФАЗА 3: модель раскола preset определена (5 аудитов, ТОЛЬКО ЧТЕНИЕ)

### ✅ DONE
- **РЭ.22 подтверждён ЗАВЕРШЁННЫМ.** Движок раскладки семантичен на ВСЁМ активном пути выбора
  мастера (`page_role` + `slot_capacity` + `page_type`); fallback-имён, которые семантика вытеснила,
  в активных путях нет. (.12 — диагностика без правок, см. секцию 26.06 ниже.)

- **Проведено 5 узких аудитов (только чтение) — определена продуктовая модель Фазы 3.** Кода не
  трогали; данные брал в т.ч. из живой прод-БД (Timeweb, таблица `presets`, 20 строк). Решения и
  выгрузки сложены в **карточки Notion (Фаза 3 + инвентаризация preset)**.

- **ТРИ ОСИ модели (раздельные, ортогональные):**
  - **СТРУКТУРА** (что/сколько/порядок секций + форзац + **правило старта `first_spread`
    single/double**) — **конкретна, это «стоимость продукта»**, выбор партнёра в заказе. Сейчас живёт
    в `presets`.
  - **ДИЗАЙН** (оформление = `template_set` + **плотность сеток** 12/6/4 + **семейство пропорций**
    квадрат/прямоугольник). Размер сетки и формат-семейство — свойства ДИЗАЙНА, не структуры.
  - **ФОРМАТ / ПЕЧАТЬ** — **ось ЗАКАЗА** (целевой формат типографии + физика печати). На логику
    раскладки НЕ влияет, только на масштаб готовой раскладки и экспорт.

- **Ключевые находки аудитов:**
  - **`preset` УЖЕ почти = `structures`.** Дизайн развязан (`template_set_id` в `presets` почти всегда
    NULL — реальный дизайн приходит override'ом заказа в `loadBundle`), печать (`print_type`) живёт на
    `albums`. Из preset «уезжают» только МЕТА-поля → **отдельная таблица `structures` может не
    понадобиться** (вариант: переименовать/почистить `presets` + перенацелить FK
    `albums.section_structure_preset_id`).
  - **Переключение движка локализовано в `loadBundle`** (`lib/rule-engine/loaders.ts:49`,
    `.from('presets')` + `presetRowToPreset`). Движок читает структурные поля из `bundle.preset` —
    если они переедут, меняется один загрузчик, само ядро не трогаем.
  - **Боль «два выбора» (форзац = отдельный выбор переплёта в заказе)** = единственная подмена
    `bundle.preset.sheet_type` из `album.print_type` в **`layout/route.ts:1015-1020`**. Убрать
    перезатирание → структура несёт правило старта сама, выбор структуры = единственный выбор.
  - **Правило старта живёт в структуре:** Булгак — **double** (полноценный двустраничный старт/конец),
    Окейкнига + Фабрика — **single** (слева форзац, справа 1-я стр.). Это разная логика раскладки
    (сдвиг чётности `softOffset` + гейт форзац-секций), но она ПРОИЗВОДНА от одного бита `first_spread`,
    который структура уже несёт (колонка `presets.sheet_type`). Дублирование «плотная/мягкая
    структура» Сергей считает приемлемым.
  - **Формат = ось заказа, НЕ часть структуры.** Движок раскладки формат-агностичен (не читает
    размеры/пропорции); `adaptTemplateToFormat` делает чистый uniform-scale уже выбранных мастеров
    ПОСЛЕ раскладки. Внутри семейства — одна структура под любой формат; квадрат↔прямоугольник —
    разные ДИЗАЙНЫ (разный `template_set.format_family`), но та же структура.
  - **Плотность сетки (12/6/4) — кандидат в дизайн.** Сейчас `student_grid_size` в структуре жёстко
    диктует ёмкость (`buildGridSemantic` ищет мастер `match:'exact'`; нет мастера → раздел молча не
    строится). Инфраструктура «взять что есть в дизайне» уже написана (`findStudentGridMaster` умеет
    `min_fit` + `emptySlots`), просто не задействована. При допущении «один дизайн = одна плотность»
    структура несёт только `mode:'grid'`, плотность читается из `slot_capacity.students` дизайна.
  - **`student_layout_mode` (page/spread/grid/multi_spread) = ТИП раздела (абстрактно, структура);
    `student_grid_size` = плотность (в дизайн).** Не путать.
  - **Мёртвые колонки `presets`** (в новую модель НЕ тянуть): `density` (0/20 заполнено),
    `transition_scenario` (0/20), `pages_per_spread` (все = 2, константа).

- **ГРАНИЦА: движок раскладки НЕ трогаем.** Все 5 аудитов — карты для проектирования, не реализация.

### 🚩 ЧТО ДАЛЬШЕ
- **Фаза 3 готова к написанию ФИНАЛЬНОГО ПРОЕКТА СХЕМЫ** (начинать СВЕЖИМ заходом). Решить на основе
  карт:
  1. таблица `structures` vs чистка `presets` на месте (модель показала — отдельная таблица может не
     понадобиться);
  2. `sheet_type` → `first_spread` (single/double) как собственный признак структуры;
  3. **убрать перезатирание** `sheet_type` в `layout/route.ts:1015-1020`;
  4. UI: индикатор старта (single/double) при выборе структуры + предупреждение про сетку (если в
     дизайне нет мастера нужной плотности);
  5. механика создания/клонирования структур;
  6. порядок миграции (перенацелить FK, бэкфилл, чистка мёртвых колонок).
- **Backlog:**
  - **Удаление `buildGrid`** + 3 legacy-switch ветки + ~17 legacy-path тестов — **после перезаливки
    дизайнов**. Карточка в Notion.
  - **Полная типовая чистка legacy (Вариант B)** — `RuleEngineBundle.rules/.families`, `Rule`,
    `TemplateFamily`, `Preset.sections` + ~25 фикстур.
  - **Предусловие боевого на belly:** перезалить belly (2 NULL-мастера `J-Collage-5` /
    `J-J-Combined-Tail-2-Right` получат разметку).

---

## 🆕 СЕССИЯ 26.06.2026 (вечер) — РЭ.22.12 (диагностика, без правок) → РЭ.22 ЗАВЕРШЁН

### ✅ DONE
- **РЭ.22.12 — финальная уборка fallback-имён: закрыт БЕЗ ПРАВОК.** Диагностика (только чтение, как
  в .11) прошлась по всем строковым литералам имён мастеров в `lib/rule-engine/` и показала: **в
  активных путях ВЫБОРА мастера fallback-имён, которые семантика вытеснила и которые можно убрать с
  нулевым риском, НЕ осталось.** Весь активный движок уже semantic-first (каждая секция сначала зовёт
  семантический finder, имя — только запасной путь). Классификация оставшихся имён:
  - **A. Legacy safety-net fallback** (после семантики, только для неразмеченных дизайнов — тот же
    класс, что `buildGrid` в .11): `teachers.ts:228` F-Head-*/G-* (после `findTeacherMaster`),
    `soft-intro.ts:87` `'S-Intro'` / `soft-final.ts:79-80` `'S-Final'`/`'S-Final-Soft-L'` (после
    `findSoftSectionMaster`). **Оставляем — уйдут синхронно с buildGrid после перезаливки дизайнов.**
  - **B. Партнёрские явные имена** (не fallback, легитимная фича): `section.master_name` в soft,
    transition custom-mode. **Оставляем.**
  - **C. Внутренние токены slot-chains** ('J-Half'/'J-Quarter-Left' и т.п.): выбор by-type в
    `common.ts` (`findCommonMaster`), имя только для стороны через `pageTypeFromName`. .9 намеренно
    не трогал форму цепочек. **Оставляем.**
  - **D. Статическая таблица стандарта OkeyBook** (`album-structure-okeybook.ts`) + её by-name резолв
    (`common-additional.ts`, `common-required.ts` FALLBACK_CHAIN, валидируется `analyzeMasterCapability`)
    — это **Фаза 3** (структура-объект на канон), не type-fallback выбора. **Оставляем.**
  - **E. students legacy** (вся `mode='legacy'` ветка: buildGrid + buildAlternatingLR + buildOnePerSpread)
    — safety-net .11. **Оставляем.**
  - **F. Комментарии / типы / примеры** (ноль поведения).
  - **G. 🟡 Единственный вестигиальный** — `transition-cases.ts` `combo_master_base` ЗНАЧЕНИЯ
    ('J-Combined-Tail-4/3/2'): после .10 `transition.ts` читает их только как `!== null`/`=== null`
    (флаг «combo применим»), само значение для выбора не используется. Но убрать = поменять форму
    возврата `classifyTransitionLayout` на boolean → упирается в запрет «классификаторы не трогать».
    **Оставлено (пользы ~ноль, риск ненулевой).**
  - **Канон в движок не тащили.** Раскладка на проде не изменилась (диагностика без правок кода).

- **🎉 ИТОГ: РЭ.22 ЗАВЕРШЁН.** Движок раскладки **полностью семантичен** — все живые секции
  (students/teachers/soft/common/transition) выбирают мастер по **`page_role` + `slot_capacity` +
  `page_type`**, а не по имени. Дизайн-имена мастеров больше не зашиты в активные пути выбора.
  Подэтапы: **.9** common/slot-chains (`findCommonMaster`), **.10** transition (`findComboTailMaster` +
  переиспользование `findCommonMaster`, риск-точка лево/право пройдена), **.11** сняты мёртвые
  `adaptiveTailNames`, **.12** диагностика — остаток только намеренные safety-net'ы / партнёрские
  имена / токены / Фаза-3-территория / комментарии. **Раскладка на проде не изменилась** — доказано
  сверкой spreads[] до/после на КАЖДОМ подэтапе (akvarel 14/14 в .9, 132/132 в .10, rule-engine 501
  без правок ассертов в .11). Канон `master_page_types` в движок ни разу не затащили (читает только
  структура, не движок).

### 🚩 ЧТО ДАЛЬШЕ / хвосты (НЕ горит)
- **РЭ.22 закрыт → РАЗБЛОКИРОВАНА ФАЗА 3.** Структура как **переиспользуемый объект-заготовка**,
  ссылающийся на канон (`master_page_types`); `preset` **раскалывается на структуру + дизайн + печать**.
  Структура = `section_structure` + student/transition config, **формат-независима**. Крупный новый
  пласт — **начинать со свежей головы**, диагностика уже есть (`audit-phase3-structure.md`,
  `tz-struct-design-split.md` v2). Дальше Фаза 4 (дизайн↔структура m:m + расщепление placeholder),
  Фаза 5 (UI «Дизайны» + канон-диалог + slug-панель), Фаза 6 (уборка дублирующих полей spread_templates).
- **Предусловие боевого на belly:** перезалить belly (2 NULL-мастера `J-Collage-5` /
  `J-J-Combined-Tail-2-Right` получат разметку).
- **Backlog:**
  - **Удаление `buildGrid`** + 3 legacy-switch ветки + ~17 legacy-path тестов + legacy fallback-имена
    из категории A (teachers F-Head/G-, soft S-Intro/S-Final) — **после перезаливки дизайнов** (когда
    все старые пресеты/мастера гарантированно размечены). Карточка в Notion.
  - **Полная типовая чистка legacy (Вариант B)** — удалить поля/типы `RuleEngineBundle.rules/.families`,
    `Rule`, `TemplateFamily`, `Preset.sections` + ~25 тест-фикстур.
- **Проекты/доки:** `tz-re22-binding.md`, `tz-phase2-canon-match.md`, `tz-master-library-schema.md`,
  `tz-struct-design-split.md` (v2), `audit-phase3-structure.md`.
