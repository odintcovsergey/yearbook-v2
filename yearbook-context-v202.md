...
# Версия: v202
# Обновлено: 26.06.2026 (Библиотека мастер-страниц Фаза 1 — канон типов; + отказоустойчивый деплой GitFlic; + аудит legacy + fail-fast STORAGE_BACKEND)
# HEAD: f1ece1b (main) на момент записи
#
# Предыдущий контекст: yearbook-context-v201.md (история не переписывается).
...
- **2 дефектных belly-мастера** (`J-Collage-5`, `J-J-Combined-Tail-2-Right`: null role/capacity
  ...
  (Фаза 1 «Сделано»).
- **Инициатива «Разделение структуры и дизайна» (Путь Б): Фаза 1 готова.** Дальше:
  **Фаза 2** — сверка дизайна с каноном при загрузке IDML (лекарство от «каждый дизайн заново»);
  затем перевод движка с биндинга **by-name → by-type** (семантика, РЭ.22);
  **Фаза 3** — структура как объект; **Фаза 4** — дизайн↔структура m:m + расщепление placeholder
  (label-скелет ⊗ скин); **Фаза 5** — UI + раздел «Дизайны»; **Фаза 6** — уборка (удаление
  дублирующих полей spread_templates). Проектные доки: `tz-struct-design-split.md` (v2),
