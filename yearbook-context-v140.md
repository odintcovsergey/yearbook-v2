...
# Версия: v140
# Обновлено: 21.05.2026 (после РЭ.28.4 — UI каталога с двумя разделами).
#
# ⏳ ФАЗА РЭ.28 В РАБОТЕ — партнёрские дизайны (сценарий A)
# ──────────────────────────────────────────────────────────────────
# Подэтапы 28.0..28.4 закрыты. Следующий — 28.5 (модалка ввода размеров).
# Spec: docs/phase-Р28-spec.md v1.0.
# Последний commit фазы: e22ceae (РЭ.28.4).
#
# ✅ Миграция РЭ.28.1 применена в Supabase (Сергей подтвердил).
#
# Что готово после 28.4:
#   • Engine (РЭ.28.2): 5 чистых модулей + 55 unit-тестов
#   • API (РЭ.28.3): 3 новых action'а в app/api/tenant/route.ts
#   • UI каталога (РЭ.28.4):
#     - Раздел «Мои дизайны» (показывается если есть клоны)
#     - Раздел «Глобальные шаблоны OkeyBook»
#     - Кнопка «Создать на основе...» на глобальных (заглушка alert)
#     - Кнопка «Удалить» на своих с confirm + API call + reload
#     - Карточка преобразована из <button> в <div> для поддержки
#       вложенных action-кнопок
#     - Размеры (мм) теперь видны на карточке
#
...
#   ⏳ 28.5 — UI модалка ввода размеров: name + page sizes
#             (mm + ≈ px hint) + bleed_mm опционально + предупреждения
#             по уровням совместимости (ok/warning/blocked)
#   ⏳ 28.6 — summary + закрытие фазы
#
# Тесты на main: 492/492 passing.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v140
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v140 — 28.0..28.4 закрыты, осталась только модалка + summary
#      • docs/phase-Р28-spec.md §5.2 «UI модалка»
#      • app/app/templates/page.tsx — где сейчас handleCloneClick stub
#      • lib/template-set-clone/aspect-compatibility.ts — для оценки
#        пропорций в реальном времени при вводе размеров
#      • lib/template-set-clone/round-to-pixels.ts — mmToPx для подсказки
#   4. Применены в Supabase: все миграции до РЭ.28.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.28 — следующий подэтап 28.5 (UI модалка):
#
#      Цель: заменить заглушку handleCloneClick (alert) на полноценную
#      модалку ввода параметров клона.
#
#      Компонент: app/app/templates/_components/CloneModal.tsx (новый)
#      или inline в page.tsx если небольшой (~150-200 строк ок).
#
#      Поля формы:
#      • Название (text input, required, default «<source.name> (копия)»)
#      • Ширина страницы, мм (number input) + рядом подсказка «≈ NNNN px»
#        (через mmToPx из lib/template-set-clone)
#      • Высота страницы, мм (number input) + подсказка «≈ NNNN px»
#      • Припуск под обрез, мм (number input, опциональный) +
#        подсказка «≈ NNNN px»
#
#      Defaults:
#      - Размеры — копируются из исходного дизайна.
#      - bleed_mm — копируется из исходного или 3 мм если NULL.
#
#      Real-time check совместимости пропорций (при вводе размеров):
#      • Импорт checkAspectCompatibility из lib/template-set-clone
#      • На каждое изменение page_width/height — пересчёт уровня
#      • Рамка модалки/кнопки меняет цвет:
#        ok      — нейтрально, без блока
#        warning — жёлтый блок с message из API
#        blocked — красный блок, кнопка «Создать» disabled
#
#      Submit:
#      - POST /api/tenant action='template_set_clone'
#      - При ошибке (400 blocked / 500 / 409) — показ из API
#      - При успехе — закрыть модалку, reload designs_list, toast
#        «Дизайн создан» и можно скролл к новой карточке.
#
#      После этого → 28.6 (summary + закрытие фазы).
