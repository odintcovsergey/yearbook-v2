...
# Версия: v79
# Обновлено: 18.05.2026 (РЭ.20.6.3 — автогенератор правил общего раздела
#                       из дизайнерской матрицы, 183 правила в generated/)
#
...
#
# Главный оставшийся блок — реализация полной матрицы общего раздела
# из дизайнерской спецификации (РЭ.20) + UI редактор пресетов (РЭ.12).
# Без них партнёры не смогут получать правильную структуру альбома.
#
...
#
# Контент-редактор фото (КЭ) — end-to-end в проде:
#   ✅ КЭ.1..КЭ.8 — scale + offset + PDF + Konva (16.05.2026)
#   Хранение через __scale__<label> / __offset__<label> в data
#
# Балансировка end-to-end (БТ.1) — end-to-end в проде 18.05.2026:
#   ✅ БТ.1.1 (bb46711) — Canvas автоматически парсит __hidden__/__pos__
#   ✅ БТ.1.2 (dda28cd) — PDF-экспорт уважает __hidden__/__pos__
#   ✅ БТ.1.4 (c008133) — lib/balance-overrides shared модуль (22 теста)
#
...
#   ✅ РЭ.20.6.2 (a8d82cb, 18.05.2026) — первое правило-эталон + tests:
#      • RuleContext.preset_density + preset_sheet_type — читаются
#        прямо из preset, доступны во WHERE любого правила.
#      • 7 JSON-пресетов docs/rule-engine-data/presets/*.json получили
#        density/sheet_type/total_pages на верхнем уровне (sync с БД).
#      • 3 integration-теста в build-edge.test.ts доказывают что
#        новая инфраструктура работает.
#   ✅ РЭ.20.6.3 (d50f8ec, 18.05.2026) — автогенератор правил:
#      • scripts/generate-rules-from-matrix.ts читает 28 entries матрицы
#        и генерирует 183 правила в docs/rule-engine-data/rules/
#        common-section/generated/.
#      • Эталон РЭ.20.6.2 удалён — его заместили generated правила
#        (для mini-soft × page 0 = 3 шт: e3/e4/e5 для разных students).
#      • Тесты переписаны на generated id с суффиксом -e3.
#      • 22 entries обработаны, 4 пропущены (Standard/Universal parity —
#        нет WHEN-оператора для чётности; будут в РЭ.20.6.4).
#      • personal_final ячейки НЕ генерируются (отдельная семантика).
#      • 203 / 203 теста проходят, tsc и next build чистые.
#   ⏳ Дальше: РЭ.20.6.4 — применение seed-rule-engine.ts к Supabase
#      (загрузка 183 generated правил в БД). Затем РЭ.20.6.5 (parity
#      entries + personal_final). РЭ.20.7 (unit-тесты по строкам).
#      РЭ.12 (UI). Осталось ~8-12 часов.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v79
#   3. Применены и в Supabase (если ещё не применены):
...
#   4. Код: РЭ.20.2/3/4/5/6.1/6.2/6.3 готовы. 183 generated правил
#      в репозитории.
#   5. Следующий шаг — РЭ.20.6.4: запустить seed-rule-engine.ts в
#      write-режиме на Supabase, чтобы все 183 generated правила
#      попали в БД:
#         npx tsx --env-file=.env.local scripts/seed-rule-engine.ts --write
#      Ожидание: 7 families, 40 + 183 = 223 rules, 7 presets UPSERTed.
#      После этого rule engine начинает использовать новые правила
#      из матрицы при сборке альбомов с density/sheet_type пресетами.
#
# 📍 РЕГЕНЕРАЦИЯ ПРАВИЛ ПРИ ИЗМЕНЕНИИ МАТРИЦЫ:
#   Если docs/templates/album-structure-matrix.json меняется (или
#   меняется логика generator):
#      npx tsx scripts/generate-rules-from-matrix.ts           # dry-run
#      npx tsx scripts/generate-rules-from-matrix.ts --write   # apply
#   Скрипт ОЧИЩАЕТ docs/rule-engine-data/rules/common-section/generated/
#   перед записью — это чистый regen, без накопления старых файлов.
#
...
#   ✅ РЭ.20.2 (715add1) — миграция БД.
#   ✅ РЭ.20.3 (45848e2) — типы.
#   ✅ РЭ.20.4 (f0d9c23) — модуль album-structure-matrix.
#   ✅ РЭ.20.5 (37029b3) — data-миграция density/sheet_type.
#   ✅ ROLLBACK mini-hard (127f50f) — лишний пресет удалён.
#   ✅ РЭ.20.6.1 (6be5ab5) — инфраструктура consumes.
#   ✅ РЭ.20.6.2 (a8d82cb) — RuleContext.preset_* + первое правило-эталон.
#   ✅ РЭ.20.6.3 (d50f8ec) — автогенератор: 183 правила в generated/.
#   1. РЭ.20.6.4 — применить seed-rule-engine.ts --write для загрузки
#      183 правил в Supabase. Это активирует matrix-логику в проде.
#      После применения проверить пересборку реального альбома
#      (mini-soft / light с фотоматериалом).
#   2. РЭ.20.6.5 — обработка parity entries (Standard/Universal) +
#      personal_final ячеек (отдельная семантика).
#   3. РЭ.20.7 — unit-тесты по каждой entry матрицы (28 шт).
#   4. РЭ.12.A — UI /app/presets + ALTER TABLE albums ADD sheet_type
#      (sheet_type теперь живёт у альбома, не у пресета).
#   5. РЭ.12.Б — UI редактор пресета (total_pages — главное!).
#   6. РЭ.20.8 — финальная документация.
#
