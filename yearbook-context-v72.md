...
# Версия: v72
# Обновлено: 18.05.2026 (БТ.1 балансировка end-to-end + РЭ.19.1 final-разворотов
#                       + РЭ.20 ТЗ полной матрицы структуры от дизайнера)
#
...
#   ⏳ Реализация: ~26-35 часов работы (взаимозависима с РЭ.12)
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v72
#   3. Открыть docs/phase-Р20-spec.md — это план работы
#   4. Решить с чего начать: РЭ.20.А (миграция) или РЭ.12 (UI)
#      Рекомендуется РЭ.20.А первой — добавляет total_pages в БД,
#      потом РЭ.12 уже сможет это поле редактировать.
#
# 📍 КАК ПРОБОВАТЬ КОНТЕНТ-РЕДАКТОР НА ЖИВОМ АЛЬБОМЕ:
#   1. Открой любой собранный альбом → 'Открыть редактор'
#   2. Клик левой кнопкой по любому фото (одиночный, не drag)
...
#   1. РЭ.20.А — миграция БД: total_pages в presets, удалить
#      common_section_max_spreads из albums (или сделать deprecated)
#   2. РЭ.20.Б — расширение Preset/RuleContext типов
#   3. РЭ.12.A — UI /app/presets список + create form
#   4. РЭ.12.Б — UI редактор пресета (total_pages, sections, sheet_type)
#   5. РЭ.20.В — генератор правил из xlsx-матрицы
#   6. РЭ.20.Г — unit-тесты для каждой строки матрицы
#   7. РЭ.20.Д — финальная документация
#
