...
# Версия: v108
# Обновлено: 20.05.2026 (после РЭ.23.6 — закрытие фазы РЭ.23, старт РЭ.24).
#
# 🎉 ФАЗА РЭ.23 ЗАКРЫТА (5 подэтапов + summary, 11 коммитов на main)
# ──────────────────────────────────────────────────────────────────
# Визуальный каталог мастеров + автогенерация SVG-превью.
# Полное summary фазы: docs/phase-Р23-summary.md (116 строк).
# Финальный commit: 7779309 (РЭ.23.6).
#
# Что готово после фазы:
#   • Чистая функция renderPreviewSvg для генерации SVG-схемы мастера
#   • API template_set_list_with_previews + _update_display_label
#   • Страница /super/master-catalog для разметки display_label
#   • Все мастера OkeyBook доступны через каталог для визуального осмотра
#
# Что НЕ сделано в фазе (намеренно отложено):
#   • РЭ.23.5 (массовая разметка display_label) — отложена. Сергей
#     решил сначала дорисовать недостающих мастеров для стартовой
#     линейки пресетов, потом разметить всё в один присест.
#     Не блокер — UI везде делает fallback на name. Каталог работает,
#     можно размечать в любой момент.
#
# Известные аномалии данных:
#   • L-Grid-12: is_spread=true, но placeholders только в левой
#     половине разворота. Исправить при пересохранении из InDesign.
#
# ⏳ Следующая фаза: РЭ.24 — готовые пресеты для быстрого старта партнёра.
#    План в общих чертах:
#      • Пометить часть пресетов как «рекомендованные» (новое поле
#        presets.is_recommended или enum/jsonb)
#      • На главной странице партнёра показать карточки рекомендованных
#        пресетов с SVG-превью ключевых разворотов (через
#        renderPreviewSvg из РЭ.23)
#      • Кнопка «Использовать» клонирует пресет в личный список партнёра
#    Перед началом — spec по образцу docs/phase-Р22-spec.md и Р23-spec.md.
#    Сначала фиксируем архитектурные развилки.
#
...
#   Путь B (РЭ.25): партнёр хочет кастом — открывает конструктор, видит
#                   галерею вариантов на каждую роль.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v108
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v108 — фазы РЭ.22 + РЭ.23 закрыты
#      • docs/phase-Р22-summary.md — итоги engine-семантики
#      • docs/phase-Р23-summary.md — что готово для РЭ.24
#   4. Применены в Supabase (все миграции РЭ.22 + РЭ.23):
#      см. spec'и фаз, повторять не буду.
#   5. СТАРТ ФАЗЫ РЭ.24 — готовые пресеты для быстрого старта партнёра.
#      Подэтап 24.0: написать docs/phase-Р24-spec.md по образцу
#      docs/phase-Р22-spec.md и Р23-spec.md.
#      Перед началом — обсудить с Сергеем архитектурные развилки:
#        • Хранение «рекомендованности» — новое поле presets.is_recommended
#          (bool) или sort_order или enum?
#        • Что показывать в превью пресета: 1 ключевой разворот?
#          3-4? Все ключевые? Через какой механизм выбирать?
#        • Что значит «клонировать» — копия пресета с tenant_id партнёра?
#          Или ссылка на global + override-слой?
#        • Где партнёр это видит — главная /app? Отдельная вкладка
#          «Шаблоны»? Модальное окно при создании альбома?
#        • Что с разметкой пресетов которые у Сергея не доделаны —
#          показывать как есть или скрывать?
