...
# Версия: v121
# Обновлено: 21.05.2026 (после РЭ.25.3 — engine-фильтр готов).
#
# ⏳ ФАЗА РЭ.25 В РАБОТЕ — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтапы 25.0 (spec), 25.1 (миграция), 25.2 (API), 25.3 (engine-фильтр)
# закрыты. Следующий — 25.4 (UI фотографа: форма ученика + форма альбома).
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: 07cac78 (РЭ.25.3 engine-фильтр).
#
# ✅ Миграция РЭ.25.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026).
#
# Что готово после 25.3 (бэкенд закрыт целиком):
#   • API /api/tenant POST: update_child, create_album, update_album
#     с поддержкой is_purchased и include_non_purchasers.
#   • API /api/tenant GET action='children' возвращает is_purchased.
#   • API /api/select PUT action='set_purchased' для родителя
#     (доступен всегда, даже после submitted_at).
#   • API /api/child GET возвращает is_purchased в данных ребёнка.
#   • Engine-фильтр в lib/smart-fill/filter-by-purchase.ts:
#     - Чистая функция filterChildrenByPurchase<C>(children, includeAll)
#     - Применяется в buildAlbumInput ДО формирования AlbumInput.students[]
#     - Engine остаётся чистым (не знает про is_purchased)
#     - Бэк-совместимость: undefined/null трактуются как true
#   • Warning non_purchasers_filtered эмитится при реальном отсечении.
#   • 12 unit-тестов на фильтр — все ✅.
#
# Что ещё НЕ работает (UI):
#   • Фотограф пока не видит галку в UI (это 25.4).
#   • Родитель пока не видит галку в /[token] (это 25.5).
#   • CSV-экспорт пока без колонки 'Заказ' (это 25.5).
#   • Карточка альбома без счётчика 'N из M' (это 25.6).
#
# Что зафиксировано в spec (4 архитектурные развилки утверждены):
#   a) Кто проставляет галку — ОБА (фотограф в /app + родитель в /[token]),
#      одно поле children.is_purchased, last-write-wins.
#   b) Не-покупатели в общих фото (common) и виньетках — ПОПАДАЮТ.
#      Виньетка — это состав класса, не список заказчиков.
#   c) Не-покупатели в личных страницах (students) — НЕ попадают
#      по умолчанию. Альбом получает флаг include_non_purchasers
#      (default false = строгое). При true — все ученики получают
#      личную страницу независимо от is_purchased.
#      Per-child переопределение НЕ вводим — поднимай is_purchased
#      обратно.
#   d) Дефолт миграции:
#      children.is_purchased DEFAULT true (бэк-совместимость).
#      albums.include_non_purchasers DEFAULT false (новые альбомы
#      строгие; старые работают как раньше — у всех is_purchased=true).
#
# Архитектурное место фильтра:
#   В lib/smart-fill/filter-by-purchase.ts — чистая функция без
#   зависимостей от Supabase. Применяется в buildAlbumInput ДО
#   входа в buildAlbum. Engine остаётся чистым, не знает про
#   is_purchased. Один источник истины, легко отлаживать.
#
# План подэтапов РЭ.25 (8-9 коммитов):
#   ✅ 25.0 — spec (commit 5ee16e6)
#   ✅ 25.1 — миграция БД (commit b8f671f, применена в Supabase)
#   ✅ 25.2 — API + типы (commit b0d76ca)
#   ✅ 25.3 — engine-фильтр + 12 тестов (commit 07cac78)
#   ⏳ 25.4 — UI фотографа (форма ученика + форма альбома)
#   ⏳ 25.5 — UI родителя (/[token] чекбокс) + колонка 'Заказ' в CSV
#   ⏳ 25.6 — счётчик 'N из M заказали' в карточке альбома
#   ⏳ 25.7 — summary + закрытие фазы
#
# Тесты на main: 407/407 passing (было 395 до РЭ.25, +12 в 25.3).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v121
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v121 — РЭ.25.0..25.3 закрыты (бэкенд готов). Фаза в работе.
#      • docs/phase-Р25-spec.md v1.0 (334 строки) — спецификация фазы,
#        особенно §5 (UI контракт фотографа)
#      • docs/phase-Р24-summary.md — последняя закрытая фаза (контекст)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.4 (UI фотографа):
#      Два места правки в /app:
#      a) ФОРМА АЛЬБОМА (AlbumFormFields в app/app/page.tsx):
#         - Добавить чекбокс 'Включить в личный раздел всех учеников,
#           даже не заказавших альбом' ниже блока выбора шаблона.
#         - По умолчанию снят (include_non_purchasers=false).
#         - Подсказка: 'если выключено — личные страницы получают
#           только те ученики у которых проставлена галка
#           «Заказывает альбом». Включи если хочешь дать страницу
#           всему классу независимо от заказа.'
#         - Сохраняется через существующий create_album/update_album
#           (поле уже в allowedFields после 25.2).
#      b) ФОРМА УЧЕНИКА (в списке учеников альбома):
#         - Чекбокс 'Заказывает альбом' рядом с ФИО.
#         - По умолчанию проставлен (is_purchased=true).
#         - Сохраняется через update_child action (POST /api/tenant
#           с { action:'update_child', child_id, is_purchased }).
#         - Визуальный маркер в списке для is_purchased=false:
#           приглушённый стиль (серый фон или иконка-метка).
#      Найти точки правок: смотри в app/app/page.tsx
#        - AlbumFormFields (форма создания/редактирования альбома)
#        - Component'ы списка учеников (искать по 'children.map' или
#          фразам типа 'Список учеников')
#      Дальше — 25.5 (UI родителя + CSV), 25.6 (счётчик), 25.7 (summary).
#      Все подэтапы расписаны в docs/phase-Р25-spec.md §5 и §7.
