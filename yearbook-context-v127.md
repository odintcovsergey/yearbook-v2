...
# Версия: v127
# Обновлено: 21.05.2026 (после РЭ.27.1 — миграция схемы готова).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0 (spec) и 27.1 (миграция схемы) закрыты.
# Следующий — 27.2 (API + типы).
# Spec: docs/phase-Р27-spec.md v1.0 (377 строк).
# Последний commit фазы: d4ff79e (РЭ.27.1).
#
# ⚠️ ВНИМАНИЕ Сергею: миграция РЭ.27.1 ещё НЕ ПРИМЕНЕНА в Supabase.
#    Применить ДО или ОДНОВРЕМЕННО с деплоем кода 27.2:
#    migrations/2026-05-21-albums-print-type.sql
#    Миграция аддитивная (TEXT NULL) — старый код продолжает работать
#    через fallback на preset.print_type.
#
...
#   ✅ 27.1 — миграция схемы (commit d4ff79e)
#              ⚠️ ПРИМЕНИТЬ В SUPABASE перед деплоем 27.2
#   ⏳ 27.2 — API + типы (create_album/update_album принимают print_type,
#             SELECT'ы возвращают)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v127
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v127 — РЭ.27.0 и РЭ.27.1 закрыты. Фаза в работе.
#      • docs/phase-Р27-spec.md v1.0 (377 строк) — спецификация фазы
#      • docs/phase-Р25-summary.md — последняя закрытая фаза (контекст)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#      ⚠️ ПРОВЕРИТЬ ПРИМЕНИЛ ЛИ СЕРГЕЙ миграцию РЭ.27.1:
#        migrations/2026-05-21-albums-print-type.sql
#      Если не применена — код 27.2 после деплоя упадёт при SELECT/UPDATE
#      колонки albums.print_type.
#      Проверочный запрос в Supabase SQL Editor:
#        SELECT column_name FROM information_schema.columns
#        WHERE table_name='albums' AND column_name='print_type';
#      Ожидание: одна строка. Если ноль — миграция не применена.
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.2 (API + типы):
#      • Расширение action POST /api/tenant: create_album
#        - Принимать опциональное поле print_type: 'layflat' | 'soft' | undefined
#        - Если не передано — INSERT NULL (fallback на пресет работает).
#      • Расширение action POST /api/tenant: update_album
#        - Добавить 'print_type' в allowedFields.
#      • SELECT'ы албумов (action='dashboard', action='album', и т.д.):
#        - Колонка SELECT * уже подтянет print_type, но в типах
#          надо явно отметить опциональность.
#      • TypeScript типы:
#        - Album.print_type?: 'layflat' | 'soft' | null
#        - В app/app/page.tsx локальные типы тоже расширить.
#      • Engine ещё НЕ читает albums.print_type (это 27.3).
#        Цель 27.2 — чтобы поле можно было сохранять и загружать
#        через API, остальная логика приедет в 27.3.
