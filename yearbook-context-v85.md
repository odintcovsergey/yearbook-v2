...
# Версия: v85
# Обновлено: 18.05.2026 (РЭ.21.7 закрыт целиком — 4 коммита:
#   API rule_preset_update (partial patch) → UI кнопки 'Редактировать'
#   и обобщение PresetCreateForm в PresetForm → DnD редактор секций →
#   DnD редактор слотов внутри common.)
#
...
#   1. РЭ.21.7.5 — параметры внутри секций (архитектурный шаг).
#      Сейчас preset.density / sheet_type живут на уровне ВСЕГО пресета.
#      Это ограничение: один пресет = одна плотность, нельзя сделать
#      'первые 4 ученика отдельно, остальные в сетке'. Правильно:
#      density перенести в section.params (для секций типа 'students').
#      Обсуждение 18.05.2026: Сергей предложил вариант B-стиль на старте —
#      UI настроек в секции (dropdown плотности при клике на секцию
#      students), под капотом БД пока остаётся как есть (preset.density).
#      Потом аккуратный переезд: расширить SectionStructureEntry новым
#      вариантом для students с params, переезд buildMatrixContext.
#      Объём: ~3-4 коммита.
#   2. РЭ.21.8 — подключение section_structure к build engine.
...
#   3. РЭ.21.9 — копирование глобального пресета как стартового.
#      Кнопка «Скопировать как мой» на карточке глобального пресета.
#   4. РЭ.21.10 — финальная документация архитектуры.
#
