...
# Версия: v60
# Обновлено: 16.05.2026 (сверка с реальным IDML — spec v1.2, ТЗ v1.4, +3 комбинированных мастера)
#
# ⚠️ ВАЖНО: ПРЕЖДЕ ЧЕМ ПРИНИМАТЬСЯ ЗА ЛЮБУЮ ДРУГУЮ РАБОТУ — ПРОЧИТАЙ:
#   docs/rule-engine-spec.md              — ✅ спецификация rule engine v1.1 (15.05.2026, после ревью с Сергеем)
...
# Rule engine спецификация v1.2 готова (16.05.2026, после сверки с реальным IDML) — фундамент реализации
# buildFromRules для запуска партнёрки в сентябре.
# Подэтапы РЭ.1+РЭ.2+РЭ.3 закодированы (миграция БД + типы/Zod + JSON-каталог).
# ⚠️ Сергей должен применить rule-engine-migration.sql в Supabase SQL Editor вручную (как обычно).
# Сергей прислал IDML «Белый плотные разворотами» — 27 мастеров, парсер прошёл чисто.
# После сверки IDML обнаружены 11 правок в spec и ТЗ, плюс новое подсемейство:
# КОМБИНИРОВАННЫЕ МАСТЕРА M/L/N-Combined-Page (портреты вверху + общее фото внизу
# на ОДНОЙ странице) для плотностей medium/light/mini. Для E-* (max/universal/standard)
# комбинированные НЕ нужны — остаток через разворот со смешанными страницами как раньше.
#
...
#    Подэтапы реализации РЭ.1-РЭ.3 ✅ выполнены, РЭ.4-РЭ.13 запланированы.
#
...
#   ✓ docs/rule-engine-spec.md v1.2 (16.05.2026, 1650 строк) — после сверки IDML
#     +3 комбинированных мастера M/L/N-Combined-Page +11 правок меток
#   • docs/rule-engine-spec.md v1.1 (7f3647f, 15.05.2026, 1594 строки) — для истории
...
#            ⚠️ ТРЕБУЕТСЯ: Сергей применяет миграцию в Supabase SQL Editor
#            (как и предыдущие миграции template-sets-*).
#            Проверочные SELECT в конце файла.
#
...
#            Результат текущий: 7/7 семейств + 7/7 пресетов OK.
#            UPSERT в Supabase будет добавлен в РЭ.8.
#
# РЭ.4  — Правила head-teacher + subject-teachers + class-photo
#         (с variants для subjects=10..24) + vitest тесты.
#         Включая новое правило t-class-0-classphoto-and-halfs
#         (priority=110) для мастера F-Head-WithClassPhoto-L.
# РЭ.5  — Правила student-section: maximum/universal/standard
#         (включая одинокого ученика через разворот со смешанными
#         страницами). С поддержкой параметров секции:
#         has_quote / has_friend_photos / friend_photos_max.
# РЭ.6  — Правила student-section: medium/light/mini +
#         параметрические мастера. Поддержка ОБОИХ путей:
#         (А) L-Grid-Page / N-Grid-Page / M-Grid-Page параметрические
#         (Б) L-Grid-1..6 / N-Grid-1..12 / M-Grid-1..4 отдельные.
#         Алгоритм выбирает доступный.
# РЭ.7  — Правила common-section + intro + final
# РЭ.8  — Базовые пресеты + добавление UPSERT в seed-скрипт.
#         JSON-файлы пресетов уже созданы в РЭ.3.
#         Здесь: научить seed-скрипт делать INSERT/UPDATE в Supabase
#         для всех 7 базовых пресетов + правил.
# РЭ.9  — buildFromRules алгоритм (lib/rule-engine/build.ts +
#         evaluate.ts + apply.ts + balance.ts) + интеграция в
#         lib/album-builder/index.ts (фолбэк на старый buildAlbum)
# РЭ.10 — vitest тесты алгоритма (~50 тестов, smoke + edge cases +
#         разворот со смешанными страницами)
# РЭ.11 — UI: TemplatePickerModal фильтрация по family_id
#         (партнёр видит только подходящие мастера при замене)
# РЭ.12 — UI: Редактор пресетов /app/presets (опционально, после запуска)
# РЭ.13 — Документация (designer-tz v1.4 УЖЕ есть — designer-tz-2026-05-16.md)
#         + финальное обновление контекста
#
...
#   docs/rule-engine-spec.md v1.2 (1650 строк, актуальная — после сверки IDML 16.05.2026) — главная спека
...
#   docs/templates/designer-tz-2026-05-16.md (v1.4 — актуальная, после
#     сверки с реальным IDML и добавления комбинированных мастеров)
