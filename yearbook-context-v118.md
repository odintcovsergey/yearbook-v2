...
# Версия: v118
# Обновлено: 21.05.2026 (после РЭ.25.0 — spec фазы РЭ.25 написан и закоммичен).
#
# ⏳ ФАЗА РЭ.25 СТАРТОВАЛА — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтап 25.0 (spec) закрыт. Следующий — 25.1 (миграция БД).
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Финальный commit подэтапа 25.0: 5ee16e6.
#
...
#   ⏳ 25.1 — миграция БД (children.is_purchased + albums.include_non_purchasers)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v118
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v118 — РЭ.25.0 закрыт (spec). Фаза РЭ.25 в работе.
#      • docs/phase-Р25-spec.md v1.0 (334 строки) — полная спецификация
#        фазы РЭ.25 с архитектурой, развилками, планом подэтапов
#      • docs/phase-Р24-summary.md — последняя закрытая фаза (РЭ.24)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1 ✅
#      НЕ ПРИМЕНЕНА: миграция РЭ.25.1 (ещё не написана).
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.1 (миграция БД):
#      • SQL добавить две колонки:
#        - children.is_purchased BOOLEAN NOT NULL DEFAULT true
#        - albums.include_non_purchasers BOOLEAN NOT NULL DEFAULT false
#      • Индекс idx_children_is_purchased_album (album_id, is_purchased)
#      • Файл: migrations/2026-05-21-children-is-purchased-and-album-include-non-purchasers.sql
#      • Применить в Supabase ДО деплоя кода (аддитивная миграция,
#        старый код колонок не видит — zero-downtime).
#      • Параллельно подготовить TypeScript типы для children и albums,
#        чтобы 25.2 (API) был готов к работе.
#      Дальше — 25.2 (API), 25.3 (engine-фильтр), 25.4 (UI фотографа),
#      25.5 (UI родителя + CSV), 25.6 (счётчик), 25.7 (summary).
#      Все подэтапы расписаны в docs/phase-Р25-spec.md §7.
