...
# Версия: v142
# Обновлено: 21.05.2026 (РЭ.28.6 — ФАЗА РЭ.28 ЗАКРЫТА).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v142
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v142 — РЭ.27 и РЭ.28 ЗАКРЫТЫ. Все 14 подэтапов
#        двух фаз + 4 урока из РЭ.27 + успех применения уроков в РЭ.28.
#      • docs/phase-Р27-summary.md (318 строк)
#      • docs/phase-Р28-summary.md (310 строк)
#   4. Применены в Supabase: ВСЕ миграции до РЭ.28.1 включительно ✅
#   5. ДАЛЕЕ — три варианта:
#
#      A) Пауза — тестирование РЭ.28 на боевых сценариях.
#         - Создать клон через UI с разными размерами:
#           • 226×288 (как источник, ok)
#           • 220×290 (warning)
#           • 200×200 (blocked)
#         - Проверить что resize мастеров корректный.
#         - Создать альбом на клоне.
#         - Удалить клон когда он в использовании (должно дать 409).
#         - Удалить клон когда он не используется (должно ok).
#
#      B) РЭ.29+ — сценарий B (партнёрский IDML).
#         - Большая фаза без чёткого скоупа.
#         - Открывается ТОЛЬКО по реальному запросу первого партнёра
#           с собственной типографией и желанием своих мастеров.
#         - НЕ начинать без чёткой потребности.
#
#      C) Долг РЭ.27/28 — несрочные улучшения.
#         Из summary'ев:
#         - Удаление колонки config_presets.print_type (РЭ.27 долг)
#         - Confirmation при смене типа листов если есть spread-мастера
#         - PDF-экспорт с водяным знаком «Форзац»
#         - Расширенные роли page_role
#         - Принудительная парность страниц для soft
#         - Партнёрский cover_preview_url у клонов
#         - Update-of-clone при изменении оригинала
#
#      Рекомендация: вариант A (пауза + тестирование).
#
#      ⚠️ ВАЖНОЕ ПРАВИЛО ИЗ УРОКОВ РЭ.27 (подтверждено успехом в РЭ.28):
