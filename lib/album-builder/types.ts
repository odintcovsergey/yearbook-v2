...
export type SpreadInstance = {
  ...
  section_type?: string;
  /**
   * ТЗ экспорта 20.06.2026 «нарезка per-student»: метка принадлежности этой
   * страницы ЛИЧНОЙ книге ученика. Пробрасывается adapter'ом из
   * PageInstance.personal (см. rule-engine/types.ts). По ней типографская
   * выгрузка (lib/export-typography) разносит страницы по книгам 00X; общие
   * страницы (без метки) уходят в книгу 000. Вёрстку/рендер НЕ меняет — чистая
   * метаинформация. Появляется в сохранённой вёрстке у альбомов, собранных
   * ПОСЛЕ этой правки; старым нужна пересборка, чтобы получить личные книги.
   */
