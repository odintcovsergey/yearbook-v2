 ...
 * ставить блок системой. Дизайнер рисует контент как угодно — система гарантирует
 * у КОРЕШКА зазор.
 *
 * Правка 17.06.2026 (по идее Сергея): `spineMarginMm` — это МИНИМАЛЬНЫЙ зазор у
 * корешка, а НЕ точный. Двигаем блок ТОЛЬКО если он ближе к корешку, чем отступ
 * (увеличиваем зазор, отодвигая от корешка). Если зазор уже больше отступа —
 * НЕ трогаем. Иначе хорошо скомпонованную парадную страницу (крупный портрет,
 * нарисованный с запасом от корешка) система подтягивала бы к корешку и портили
 * композицию. При этом исходная проблема (контент нарисован впритык/за корешок)
 * по-прежнему чинится — такой блок отодвигается до зазора `spineMarginMm`.
 *   - левая страница (корешок справа): зазор = W − maxX; если < margin → сдвиг влево;
 *   - правая страница (корешок слева): зазор = minX; если < margin → сдвиг вправо.
...
function positionBlockBySpine(
  ...
  // Отступ = МИНИМУМ, не точное значение: двигаем только В СТОРОНУ ОТ корешка
  // (увеличить зазор). Сдвиг К корешку (зазор уже достаточный) — обнуляем.
  //   left: shift>0 = сдвиг вправо к корешку → не двигаем.
  //   right: shift<0 = сдвиг влево к корешку → не двигаем.
  if (side === 'left' && shift > 0) shift = 0;
  if (side === 'right' && shift < 0) shift = 0;
