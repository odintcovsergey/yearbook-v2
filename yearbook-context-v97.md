...
# Версия: v97
# Обновлено: 20.05.2026 (после РЭ.22.6.0 — data-миграция grid-мастеров).
#
# ФАЗА РЭ.22 — Конструктор пресетов с двух-осевой моделью (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.22.0 (de27d00) — docs/phase-Р22-spec.md v1.0 (564 строки).
# ✅ РЭ.22.1 (8516419) — миграция БД, ✅ применена 20.05.2026.
# ✅ РЭ.22.2 (335f00e) — типы + API + попутно закрыт скрытый баг РЭ.21.8.15.
# ✅ РЭ.22.3 (4d132ec) — UI двух-осевая модель в PresetEditorModal.
# ✅ РЭ.22.4 (a554a68) — engine semantic для mode='page'.
# ✅ РЭ.22.5 (7e6ac50) — engine semantic для mode='spread'.
# ✅ РЭ.22.6.0 (5cbd51a) — data-миграция grid-мастеров.
#    migrations/2026-05-20-okeybook-grid-master-tags.sql
#    UPDATE для M/L/N-Grid-Page + L-Grid-12 + *-Combined-Page:
#    page_role='student_grid', slot_capacity вычисляется динамически
#    через jsonb_array_elements (students=COUNT portraits, photos_full=
#    COUNT classphotoframe, has_quote по правилу spec §6.4).
#    Адаптивные L-2/3/4, N-4/6/9 в БД отсутствуют — отложены.
#    ⬜ Применить в Supabase SQL Editor.
#
# ⏳ Следующий подэтап: РЭ.22.6 — engine semantic для mode='grid'.
#    - В sections/students.ts добавить третью приоритетную ветку:
#      preset.student_layout_mode='grid' → buildGridSemantic.
#    - Расширить findStudentMaster — снять ограничение studentsCap===1.
#    - Новая функция findStudentGridTailMaster для адаптивного хвоста
#      (поиск мастера с slot_capacity.students=remainder).
#    - Combined-tail через мастер с students+photos_full=1.
#    - Fallback на base-master с null-заполнением.
#    - Тесты.
#
# Двух-осевая модель «режим × параметры»:
#   - mode='page'   — 1 ученик/страница (Standard/Universal) — ✅ РЭ.22.4
#   - mode='spread' — 1 ученик/разворот (Maximum/Individual) — ✅ РЭ.22.5
#   - mode='grid'   — сетка N учеников (Medium/Light/Mini) — ⏳ РЭ.22.6
#
...
#      их нарисует.
#      ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.22.6.
#
#   ⏳ Следующий подэтап: РЭ.22.6 — engine semantic для mode='grid'
#      Третья приоритетная ветка в fillStudentsSection. Расширить
#      findStudentMaster для studentsCount>1. Новая функция
#      findStudentGridTailMaster для адаптивного хвоста.
#
...
#        РЭ.22.7  — teachers секция: семантический поиск
#        РЭ.22.8  — soft_intro + soft_final: семантический поиск
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v97
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v97 — что закрыто (РЭ.22.0..РЭ.22.6.0)
#      • docs/phase-Р22-spec.md v1.0 — §6.2 (расширение findStudentMaster)
#        и §6.3 (findStudentGridTailMaster).
#      • migrations/2026-05-20-okeybook-grid-master-tags.sql — какие
#        теги теперь у grid-мастеров (нужно для понимания что искать).
#      • lib/rule-engine/sections/students.ts — buildGrid + pushGridPage
#        + pickAdaptiveTail. В РЭ.22.6 заменим жёсткие имена на
#        семантический поиск, но логика хвоста (combined / adaptive /
#        padded) сохраняется.
#      • lib/rule-engine/master-finder.ts — функция требует расширения:
#        снять ограничение studentsCap===1, чтобы для grid тоже работала.
#   4. Применены в Supabase:
#      • Все миграции до v87 ✅
#      • 2026-05-19-presets-student-layout-fields.sql ✅
#      • 2026-05-19-okeybook-default-student-master-tags.sql ✅
#      • 2026-05-20-albums-drop-rules-preset-id.sql ⬜ НЕ применена
#        (не блокер — колонка просто остаётся в БД, не читается)
#      • 2026-05-20-presets-student-layout-mode.sql ✅ ПРИМЕНЕНА (РЭ.22.1)
#      • 2026-05-20-okeybook-grid-master-tags.sql ⬜ РЭ.22.6.0 —
#        ПРИМЕНИТЬ ДО деплоя РЭ.22.6.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.6 — engine semantic для mode='grid'.
#      Файлы:
#        - lib/rule-engine/master-finder.ts: расширить findStudentMaster
#          — сейчас фильтр studentsCap===1 хардкодит page-режим. Сделать
#          studentsCount параметром StudentLayoutRequest. Для grid поиска
#          studentsCount=N, photos_friend обычно не нужен (опционально).
#        - lib/rule-engine/sections/students.ts:
#          • В fillStudentsSection третья приоритетная ветка:
#            student_layout_mode='grid' → buildGridSemantic.
#          • Новая buildGridSemantic — аналог buildGrid но через семантику:
#            base-мастер ищется через findStudentMaster(pageRole='student_grid',
#            studentsCount=preset.student_grid_size, hasQuote=preset.has_quote).
#          • Адаптивный хвост: новая findStudentGridTailMaster — поиск
#            мастера с slot_capacity.students=remainder, photos_full=0.
#          • Combined-tail: тот же поиск но photos_full=1.
#          • Fallback на base-master с null-заполнением если ничего не
#            подходит (как сейчас в buildGrid).
#        - lib/rule-engine/__tests__/sections-students-grid-semantic.test.ts
#          (новый) — 8-10 тестов: базовая сетка, точное совпадение, combined-
#          tail, padded-tail, fallback на legacy.
#      Что НЕ трогаем:
#        - buildPageSemantic / buildSpreadSemantic — работают как есть
#        - buildAlternatingLR / buildOnePerSpread / buildGrid (legacy)
#        - buildOnePerSpreadAdaptive (legacy для Individual)
