...
# Версия: v52
# Обновлено: 11.05.2026 (поздний вечер — финал большого продуктивного дня)
#
# ⚠️ ВАЖНО: ПОСЛЕ ЗАКРЫТИЯ ФАЗЫ 3 (PDF-экспорт) ВЫЯСНИЛОСЬ ЧТО СИСТЕМА
# НЕ ГОТОВА К ЗАПУСКУ ПАРТНЁРСКОЙ ПРОГРАММЫ. ПРЕЖДЕ ЧЕМ ПРИНИМАТЬСЯ
# ЗА ЛЮБУЮ ДРУГУЮ РАБОТУ — ПРОЧИТАЙ ТРИ ДОКУМЕНТА:
#   docs/roadmap-after-phase-3.md         — план фаз А-И до боеготовности
#   docs/designer-questions-2026-05-10.md — вопросы дизайнеру + полученные ответы
#   docs/templates/master-cleanup-tz.md   — раздел F5 (виртуальные страницы) от 11.05
# В них зафиксированы все договорённости от 10-11.05.2026 после
# обсуждения с Сергеем.
#
...
#
# СОСТОЯНИЕ ФАЗ 11.05.2026 (финал большого дня — 33+ коммитов):
# ✓ А.1 ЗАКРЫТА (4 коммита) — загрузка фото общего раздела
# ✓ А.2 ЗАКРЫТА (5 коммитов) — builder использует общий раздел
# ✓ А.3 ЗАКРЫТА (5 коммитов) — виньетки во всех комплектациях
# ✓ А.4 ЗАКРЫТА (3 коммита) — UI настройки лимита общего раздела
# ✓ Б ЗАКРЫТА минимум (5 коммитов) — оригиналы для печати
# ✓ В ЗАКРЫТА (3 коммита) — cleanup endpoint + виджет статистики
#
# 🎉 ВСЯ ФАЗА А ЗАКРЫТА ПОЛНОСТЬЮ (А.1 + А.2 + А.3 + А.4 = 17 коммитов).
#
# ⚠️ ACTION ITEMS для Сергея после деплоя:
#   1. Применить миграцию А.4.1 в Supabase:
#        ALTER TABLE albums
#          ADD COLUMN IF NOT EXISTS common_section_max_spreads int;
#   2. Добавить CLEANUP_SECRET в Vercel env (если не сделано):
#        openssl rand -hex 32  (или любая случайная строка)
#   3. Опционально: запустить /api/cleanup?dry_run=1 для проверки
#   4. Открыть /super → 'Обновить статистику YC' для baseline
#
# Что дальше — выбирает Сергей. Кандидаты по roadmap:
#   - Применение ответов дизайнера когда придут (блок 1, 11, 15, 17/18)
#   - Г (Печать в типографию — ждёт ответы дизайнера 1-4, 11)
#   - Е (Обложка — ждёт ответ 15)
#   - Д (Размеры — желательно, не критично)
#   - Биллинг партнёров (фаза 5 продукта B) — план на июль 2026
#
# 🔵 БИЛЛИНГ — БУДУЩАЯ ЗАДАЧА (обсуждение 11.05.2026):
# Партнёры будут платить за сервис, но модель оплаты не определена.
# Возможные модели (см. обсуждение в чате):
#   A. Подписка на платформу (₽X/мес флэт)
#   B. Per-album (₽X за альбом)
#   C. Per-student (₽X за ученика)
#   D. Комиссия с вёрстки OkeyBook (текущая бизнес-модель)
#   E. Гибрид (подписка + per-album/per-student)
# Точки списания: создание альбома / завершение отбора /
#   экспорт PDF / передача в OkeyBook.
# Инфраструктура (когда модель выбрана):
#   - billing_events таблица
#   - tenants.balance или интеграция с биллинг-сервисом
#   - Lock-механизм блокировки действий при пустом балансе
#   - Интеграция с ЮKassa/Robokassa/CloudPayments
#   - UI: личный кабинет, история, пополнение
# План: запуск партнёрки в июле начать с ручной оплаты (партнёр
# платит на карту, OkeyBook вручную даёт доступ). Реальную
...
#
# Что нового в v52 относительно v51:
# - Фаза А.4 (UI настройки лимита общего раздела) закрыта
#   3 коммитами:
#   - c0991eb (А.4.1) — миграция БД: ALTER TABLE albums
#     ADD COLUMN common_section_max_spreads int (nullable).
#     NULL = без лимита, 0 = раздел отключён, >0 = лимит N разворотов.
#   - 4c729a5 (А.4.2) — builder в buildCommonSection и
#     buildCommonPair учитывает лимит:
#     * initialCounter фиксирует ctx.spreadCounter.value на входе
#     * limitReached() сравнивает добавленное с лимитом
#     * Перед buildCommonPair проверка — пропуск категории
#     * Внутри buildCommonPair break после левой страницы если
#       лимит достиг = 1 (warning common_right_page_empty)
#     * Финал: warning common_section_truncated если лимит сработал
#     * Новый BuildWarningCode common_section_truncated
#     * AlbumInput.common_section_max_spreads (optional)
#     * smart-fill читает из БД и передаёт в input
#   - 40117c0 (А.4.3) — UI CommonSectionLimitControl:
#     * Числовой input под VignettesControl в Обзоре альбома
#     * Локальный state как string (можно временно очистить)
#     * onBlur парсит: пусто → null, отрицательное → 0
#     * Оптимистичный update через POST update_album
#     * Описание под полем меняется динамически:
#       - пусто: «Без ограничения — builder вставит все...»
#       - 0: «Общий раздел не будет создан.»
#       - >0: «Не более N. Приоритет: крупные...»
#
# Приоритет фото при лимите: spread → full_class → half →
# quarter → sixth. Крупное приоритетнее мелкого, что логично
# для общего раздела (одно большое фото класса важнее коллажа
# из 12 миниатюр).
#
# Применённые миграции БД в v52 (в дополнение к v51):
#   - 2026-05-11-albums-common-section-max-spreads.sql (А.4.1)
#
# 🎉 ВСЯ ФАЗА А ЗАКРЫТА ПОЛНОСТЬЮ:
#   А.1 (загрузка фото общего раздела)  — 4 коммита
#   А.2 (builder использует) — 5 коммитов
#   А.3 (виньетки во всех комплектациях) — 5 коммитов
#   А.4 (UI лимита общего раздела) — 3 коммита
#   ИТОГО: 17 коммитов на фазу А.
#
# Итог дня 11.05.2026 (финальный):
#   33+ коммитов на main, 6 миграций БД применено, 7 фаз закрыто
#   (А.1, А.2, А.3, А.4, Б минимум, В).
#   Все главные блокеры запуска партнёрки которые не зависят от
#   ответов дизайнера — закрыты.
#   Осталось до запуска: Г (печать в типографию, ждёт ответы),
#   Е (обложка, ждёт ответ 15), Д (размеры — желательно).
#
# Что нового в v51 относительно v50:
# - Фаза А.3 (виньетки в комплектациях) закрыта 5 коммитами:
#   - 1f27f27 (А.3.1) — fix: заглушка для правой страницы учительского
#     разворота через G-FullClass с пустым фрейм. Исправляет смещение
#     альбома на 1 страницу когда фотограф ещё не загрузил common_full/
#     common_half. Warning class_photo_missing остаётся как сигнал.
#   - da8e70e (А.3.2) — миграция БД: albums.vignettes_enabled boolean
#     nullable. Override настройки виньеток на уровне альбома.
#     NULL = дефолт пресета, true/false = принудительно.
#   - 72dc59d (А.3.3) — backend: endpoint build_album перезаписывает
#     preset.thumbnails_section перед buildAlbum на основании
#     album.vignettes_enabled.
#   - 2654141 (А.3.4) — UI: dropdown 'Виньетки класса' (Авто/Включить/
#     Выключить) в Обзоре альбома, inline-обновление через update_album.
#     Компонент VignettesControl с оптимистичным state.
#   - 09b29ce (А.3.5) — fix: расширение default_for_configs для
#     N-12-Left/Right на все 7 комплектаций. Раньше Универсал/Стандарт/
#     Максимум не могли использовать виньеточные мастера автоматически
#     (был только Мини+Индивидуальная). Теперь работают везде.
#
# Реализация — Вариант 3 (Гибрид):
#   thumbnails_section в config_presets.config задаёт ДЕФОЛТ:
#     - individual-layflat/soft: {enabled: true, preferred_grid_size: 12}
#     - все остальные: null
#   albums.vignettes_enabled делает OVERRIDE:
#     - NULL → используется дефолт пресета (большинство случаев)
#     - true → принудительно включает виньетки даже в Стандарте
#     - false → принудительно отключает виньетки в Индивидуальной
#   Endpoint /api/layout?action=build_album перезаписывает
#   preset.thumbnails_section перед buildAlbum.
#
# Применённые миграции БД в v51 (в дополнение к v50):
#   - 2026-05-11-albums-vignettes-enabled.sql (А.3.2)
#   - 2026-05-11-n12-default-for-all-configs.sql (А.3.5)
#
# Виньетки УЖЕ работали в Индивидуальной до этой фазы (через
# thumbnails_section в пресете). Сергей подтвердил тестом
# Красночетайской СОШ что builder корректно создаёт виньеточные
# развороты. Эта фаза добавляет ГИБКОСТЬ (override в любой
# комплектации) + фикс смещения, обнаруженный при тесте.
#
# Зафиксирован вопрос БИЛЛИНГА как future work (см. секцию выше).
# Партнёры будут платить — модель оплаты обсуждается. Решено:
# не закладывать инфраструктуру сейчас, ручная оплата на старте
# партнёрки в июле 2026, реальная инфраструктура когда модель
# определится по реальному поведению партнёров.
#
# Итог дня 11.05.2026:
#   ~30 коммитов на main, 5 миграций БД применено, 6 фаз закрыто
#   (А.1, А.2, А.3, Б минимум, В).
#   Все главные блокеры запуска партнёрки которые не зависят от
#   ответов дизайнера — закрыты.
#   Осталось до запуска: Г (печать в типографию, ждёт ответы дизайнера),
#   Е (обложка, ждёт ответы), Д (размеры — желательно но не критично).
#   Опционально: А.4 (UI настройки общего раздела ~0.5 дня).
#
# Что нового в v50 относительно v49:
# - Фаза В (lifecycle YC + хранилище) закрыта 3 коммитами:
#   - 71c11a6 (В.1) — POST /api/cleanup endpoint:
#     удаляет файлы из YC + записи из БД где expires_at < now()
#     для album_exports и delivery_files. Авторизация через
#     CLEANUP_SECRET env (Bearer token или ?token=). Опция
#     ?dry_run=1 показывает что было бы удалено без реального
#     удаления. ycDelete сам игнорирует 'файл не найден'
#     (что произойдёт если YC lifecycle уже удалил физически).
#   - d71311e (В.2) — виджет 'Yandex Cloud Storage' в /super:
#     * GET /api/yc-stats (только superadmin): ListObjectsV2
#       с пагинацией, MAX_PAGES=100 (= 100К объектов лимит),
#       группировка по 9 категориям (originals/exports/delivery/
#       personal/portrait/group/teacher/common/tenants + other)
#     * UI YcStorageWidget между статистикой и табами — большая
#       цифра ГБ, прогноз ₽total_gb × 1.95, progress-bars по
#       категориям с русскими лейблами. Лоадер при сканировании
#       (тяжёлая операция 30-60 сек).
#   - 044adec (В.3) — docs/yc-lifecycle.md:
#     инструкция по настройке lifecycle policy в YC + ЧЕСТНАЯ
#     фиксация ограничения: настроить ПОКА НЕЛЬЗЯ из-за
#     структуры путей <album_id>/<category>/... — S3 lifecycle
#     prefix filter не поддерживает regex. Три возможных решения
#     (path-based prefix не работает, object tags ~1 час кода,
#     реструктура путей — отдельная фаза). НИ ОДНО не делается
#     сейчас. Стратегия: cleanup endpoint работает и без
#     lifecycle, виджет мониторит размер, к задаче возвращаемся
#     когда превысит ~50 ГБ.
#
# ACTION ITEMS для Сергея после деплоя (зафиксированы и в шапке):
#   1. Добавить CLEANUP_SECRET в Vercel env (openssl rand -hex 32)
#   2. Запустить /api/cleanup?dry_run=1 для проверки
#   3. Открыть /super → 'Обновить статистику YC' для baseline
#   4. (Опционально) подключить cleanup к cron-планировщику
#      (UptimeRobot, GitHub Actions, или Vercel Cron Pro)
#
# Применённые миграции БД в v50: НЕТ (всё чистый код).
#
# В работе ничего — Сергей выбирает следующее.
#
# Что нового в v49 относительно v48:
# - Фаза Б (оригиналы для печати) закрыта минимум 5 коммитами:
#   - d2af6c8 (Б.1.0) — миграция БД: ALTER TABLE photos ADD COLUMN
#     original_path text. Идемпотентная (IF NOT EXISTS). Применена
#     в Supabase 11.05.2026.
#   - 1cb600b (Б.1.1+1.2) — backend endpoints:
#     * app/api/upload-url: cleanup filename (popытный fix bug'а
#       для delivery + критично для originals где сохраняется
#       расширение jpg/jpeg/png). Endpoint уже умел upload_type=
#       'originals' с миграции 2026-05-08.
#     * app/api/upload: ответ теперь возвращает photo_id (для
#       клиентского register_original). Backward-compat сохранён.
#     * app/api/tenant: новый action 'register_original' —
#       UPDATE photos SET original_path WHERE id. Защита: путь
#       должен начинаться с {album_id}/originals/.
#   - 6f10e2f (Б.1.3) — клиент: двойная параллельная загрузка
#     в uploadFilesParallel (app/app/page.tsx). После успешной
#     загрузки WebP в фоне (void) запускается uploadOriginalBackground:
#       1. POST /api/upload-url upload_type='originals'
#       2. PUT оригинала прямо в YC по presigned URL
#       3. POST /api/tenant action=register_original
#     Прогресс инкрементится после WebP (UX: фотограф видит фото
#     быстро, оригинал докачивается фоном). Ошибки оригинала
#     идут в onFileError со суффиксом '(оригинал)' — не критично,
#     PDF-export сделает fallback на WebP.
#   - 7df43e5 (Б.2) — pdf-export читает оригинал из
#     photos.original_path с приоритетом перед legacy original_photos:
#     В app/api/layout/route.ts при сборке AlbumExportInput теперь
#     originals[] = [...inlineOriginals, ...legacyOriginals].
#     inlineOriginals формируются из photos.original_path !== NULL.
#     При коллизии filename выигрывает inlineOriginals (он первый
#     в массиве, .find() в photo-embed.ts найдёт его первым).
#
# Архитектурное решение по двум механизмам оригиналов:
#   - NEW (Б.1, основной): photos.original_path — автоматическая
#     загрузка при upload в PhotosTab.
#   - LEGACY: original_photos таблица + UI вкладки 'Производство →
#     Загрузка оригиналов'. Эта вкладка потеряла актуальность с
#     переходом системы от инструмента отбора к полноценной
#     автовёрстке (комментарий Сергея 11.05.2026). UI можно
#     удалить отдельной задачей в backlog (не делается сейчас).
#     Backend оставлен работать для backward-compat.
#
# Backfill старых альбомов НЕ делается. Подтверждено Сергеем
# 11.05.2026: 'для всех текущих заказов не нужно решать этот
# вопрос, поскольку я их сдам гораздо быстрее в ручном режиме
# (дизайнер сам сверстает), чем уже будет готова к боевым
# действиям данная система. Изменения нужны для новых заказов,
# старые не трогаем'.
#
# Применённые миграции БД в v49 (в дополнение к v48):
#   - 2026-05-11-photos-original-path.sql (Б.1.0)
#
# Backlog задача: удалить из UI вкладку 'Производство → Загрузка
# оригиналов' как deprecated (backend оставить). Не блокирует
# работу системы.
#
# В работе ничего — Сергей выбирает следующее.
#
# Что нового в v48 относительно v47:
# - Фаза А.2 (builder использует общий раздел при smart-fill)
#   закрыта целиком 5 коммитами:
#   - a2213ae (А.2.1) — smart-fill читает common_* фото из БД.
#     Расширен тип CommonPhotos полем spread, добавлен 4-й
#     запрос в Promise.all для photos с type IN ('common_*'),
#     распределение по полям spread/full_class/half/quarter/sixth.
#     Поле collage помечено DEPRECATED как alias для sixth.
#   - edaf692 (А.2.2.a) — задача A5 «Мастер J-Spread» зафиксирована
#     в master-cleanup-tz.md как зависимость от дизайнера. SQL-аудит
#     8 мастеров page_role='common' показал что ни один не подходит
#     под семантику 'одно фото на весь разворот' для common_spread.
#   - c41463d (А.2.2.b) — buildCommonSection алгоритм Вариант 1
#     (только одностраничные мастера парами):
#       common_spread  → warning no_master_for_common_spread (A5)
#       common_full    → J-ClassPhoto + J-ClassPhoto-Right
#       common_half    → J-Half × 2 симметричный
#       common_quarter → J-Quarter × 2 симметричный
#       common_sixth   → J-Collage × 2 симметричный
#     Двухстраничные комбинированные мастера (J-HalfSixth/SixthFull/
#     SixthSixth) НЕ используются — это А.2.2.c опциональное
#     расширение если партнёры пожалуются на длинный общий раздел.
#   - 30bd8ab (А.2.2.b1) — фикс после первого теста на проде:
#     все J-* мастера имели default_for_configs='{}' (намеренно
#     с миграции 07.05 — общий раздел не генерировался). Миграция
#     2026-05-11-j-masters-default-for-configs.sql установила
#     default_for_configs = applies_to_configs для всех 8 J-*
#     мастеров. Также в buildCommonPair добавлен is_spread:false
#     фильтр для защиты от случайного выбора двухстраничного
#     мастера когда А.2.2.c будет реализована.
#   - 7527941 (А.2.2.b2) — UI группировка SpreadInstance'ов в
#     визуальные развороты в LayoutPreviewStrip. Новый тип
#     VisualSpread (full_spread | pair с left/right SpreadInstance'ом
#     или null). Функция groupIntoVisualSpreads с правилами:
#     is_spread=true → full_spread, S-Intro → pair с пустой левой
#     (правая страница intro в мягких листах), пары обычных
#     одностраничных склеиваются. Терминология в UI: 'Превью
#     разворотов (N)' где N теперь визуальные развороты,
#     'Layout собран X элементов' (было 'X разворотов', чтобы не
#     врать про физические SpreadInstance'ы на которых держится
#     PDF-экспорт).
#
# Применённые миграции БД в v48:
#   - 2026-05-11-common-photos-categories.sql (А.1.1) — расширение
#     CHECK constraint photos.type с 3 до 8 значений
#   - 2026-05-11-j-masters-default-for-configs.sql (А.2.2.b1) —
#     UPDATE default_for_configs для 8 J-* мастеров
#
# Зависимости на дизайнера зафиксированные в v48:
#   - A5: мастер J-Spread (одно фото на весь разворот). До его
#     готовности common_spread фото загружаются и хранятся, но в
#     layout не попадают. Warning no_master_for_common_spread.
#
# В работе ничего — Сергей выбирает следующее.
#
# Что нового в v47 относительно v46:
# - Фаза А.1 (общий раздел: расширение photos.type + UI загрузки)
#   закрыта целиком 4 коммитами:
#   - 50cd88a (А.1.1) — миграция БД: CHECK constraint photos.type
#     расширен с 3 до 8 значений + COMMENT. Применена в Supabase.
#   - 706fd0f (А.1.2) — backend: ALLOWED_TYPES в upload route +
#     фильтры upload_photo/register_photo в tenant route +
#     TypeScript union в list_photos response.
#   - 4e29c66 (А.1.3) — UI: PhotoKind расширен на 8 значений +
#     PHOTO_KINDS_PERSONAL/COMMON/ALL константы, photoKindLabel
#     switch вместо тернарника, 5 новых блоков загрузки с
#     разделителем 'ОБЩИЙ РАЗДЕЛ АЛЬБОМА' между ними и личными,
#     8 табов галереи через flex-wrap.
#   - 9fc3222 (А.1.3.1) — UX фидбек после реального теста:
#     новые лейблы 'Общее фото на разворот' / 'Общие фото класса' /
#     'Фото по полкласса' / 'Фото 1/4 класса' / 'Фото 1/6 класса'
#     (вместо префикса 'Общий:'), убран невнятный border-l/pl-4
#     разделитель в табах галереи, min-h-[420px] на карточке
#     галереи против jump к верху при переключении на пустой таб.
# - Сергей загрузил первые фото в категорию common_spread на проде
#   (11.05 ~09:13 МСК), всё работает end-to-end.
#
# В работе фаза А.2 — builder использует фото общего раздела
# при smart-fill. Это основная мясная задача фазы А (~1.5 дня).
# Требует погружения в lib/album-builder/ для понимания текущего
# состояния и точек интеграции.
#
# Что нового в v46 относительно v45:
# - Hot-fix orphan photo_locks с TTL 15 минут (коммит 16603c5,
#   10.05 поздно вечером — попал в v45 фактически, но в шапке
#   v45 не выделен отдельно).
# - Зафиксирован roadmap фаз А-И + designer-questions (коммит
#   582d441, 10.05 поздно вечером).
# - 11.05: добавлен раздел F5 «Виртуальные страницы» в
#   master-cleanup-tz.md (коммит 0968326). Решение от 08.05.2026
#   (история чата 6ebba862) не было зафиксировано в документе —
#   закрыли упущение. Суть: все НОВЫЕ мастера делаются
#   разворотами, builder извлекает левую/правую половинку через
#   page_index у placeholder'ов. Существующие 39 мастеров не
#   трогаем. Применить к новым мастерам раздела A после ответов
#   дизайнера. B1/B2 в master-cleanup-tz помечены как
#   пересматриваемые в свете F5.
# - 11.05: поправка А.3 в roadmap (коммит 0968326). Виньеточные
#   мастера уже существуют как page_role='student_grid' (N-12,
#   L-6 + overflow), новых мастеров от дизайнера не требуется.
# - 11.05: фаза А.1 (общий раздел) в работе — стартует с миграции
#   БД (расширение photos.type на 5 новых категорий
#   common_spread/full/half/quarter/sixth).
#
# 3 коммита поверх v45: 16603c5 (photo_locks TTL) +
# 582d441 (roadmap+designer-questions) + 0968326 (F5+А.3 docs).
