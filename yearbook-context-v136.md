...
# Версия: v136
# Обновлено: 21.05.2026 (после РЭ.28.0 — spec фазы РЭ.28 написан).
#
# ⏳ ФАЗА РЭ.28 СТАРТОВАЛА — партнёрские дизайны (сценарий A)
# ──────────────────────────────────────────────────────────────────
# Подэтап 28.0 (spec) закрыт. Следующий — 28.1 (миграция БД).
# Spec: docs/phase-Р28-spec.md v1.0 (553 строки).
# Последний commit фазы: 1193b5d.
#
# Что фаза даёт:
#   Партнёр клонирует глобальный template_set с изменёнными размерами
#   под свою типографию. Мастера и placeholder'ы пересчитываются
#   пропорционально с округлением до целых пикселей при 300 DPI.
#
# Архитектура (8 утверждённых развилок A..H — см. spec §7):
#   A) Глубокая копия (template_set + все spread_templates)
#   B) <5% ok / 5-10% warning / >10% blocked по аспекту
#   C) print_type клона копируется из источника (РЭ.27 на уровне альбома)
#   D) Удаление клона только если 0 ссылок из albums/presets
#   E) slug у клона = NULL
#   F) is_global у клона = false
#   G) Партнёр редактирует name, page_*, bleed_mm
#   H) Округление до целых пикселей при 300 DPI (хардкод)
#
# Реальная схема БД на старте фазы (выгружено через information_schema):
#   • template_sets — 15 колонок, есть tenant_id (NULL=глобальный)
#   • spread_templates — 25 колонок, placeholders jsonb
#   • 3 template_set'а в проде, все tenant_id=NULL
#   • НЕТ поля parent_template_set_id (добавим в 28.1)
#
# План подэтапов РЭ.28 (7 коммитов + 1 миграция):
#   ✅ 28.0 — spec (commit 1193b5d)
#   ⏳ 28.1 — миграция БД: parent_template_set_id + индекс
#   ⏳ 28.2 — чистые утилиты (roundMmToPx, checkAspectCompatibility,
#             resizePlaceholder, prepareTemplateSetClone) + ~15-20 тестов
#   ⏳ 28.3 — API: clone / my_list / delete
#   ⏳ 28.4 — UI каталога: «Мои дизайны» + кнопка «Создать на основе»
#   ⏳ 28.5 — UI модалка ввода размеров + предупреждения + mm/px
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v136
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v136 — фаза РЭ.28 стартовала, подэтап 28.0 закрыт
#      • docs/phase-Р28-spec.md (553 строки) — спецификация фазы
#        с 8 архитектурными развилками
#      • docs/phase-Р27-summary.md §«Уроки фазы» — 4 расхождения
#        с реальностью БД, правило «information_schema перед миграцией»
#   4. Применены в Supabase: все миграции до РЭ.27.7b включительно ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.28 — следующий подэтап 28.1 (миграция БД):
#
#      ⚠️ ПЕРЕД написанием миграции:
#      1. SELECT column_name FROM information_schema.columns
#         WHERE table_name = 'template_sets'
#           AND column_name = 'parent_template_set_id';
#         Ожидание: 0 строк (колонки ещё нет).
#
#      2. Подготовить миграционный файл:
#         migrations/2026-05-2N-template-sets-parent.sql
#         С содержимым (см. spec §2.2):
...
#         + COMMENT с привязкой к РЭ.28.
#
#      3. Закоммитить + сказать Сергею применить в Supabase
#         ДО или ОДНОВРЕМЕННО с деплоем кода 28.2+.
#
#      Дальше — 28.2 (чистые утилиты + unit-тесты), 28.3 (API),
#      28.4-28.5 (UI), 28.6 (summary).
#
#      ⚠️ Не забывать: перед каждым подэтапом 28.X — выгрузка реальной
#      схемы через information_schema, а не предположения из spec'а.
