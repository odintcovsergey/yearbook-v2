...
# Версия: v93
# Обновлено: 20.05.2026 (после РЭ.22.2 — типы + API для двух-осевой модели).
#
# ФАЗА РЭ.22 — Конструктор пресетов с двух-осевой моделью (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.22.0 (de27d00) — docs/phase-Р22-spec.md v1.0 (spec 564 строки).
# ✅ РЭ.22.1 (8516419) — миграция БД: ALTER TABLE presets ADD COLUMN
#    student_layout_mode TEXT, student_grid_size INT + два CHECK
#    constraint'а через DO-блоки. ✅ Применена в Supabase 20.05.2026.
# ✅ РЭ.22.2 (335f00e) — типы + API.
#    - Preset interface расширен в lib/rule-engine/types.ts и в
#      app/super/presets/_components/PresetEditorModal.tsx.
#    - rule_presets_list SELECT расширен новыми колонками.
#    - rule_preset_update валидация для обоих новых полей.
#    - Попутно закрыт скрытый баг РЭ.21.8.15: presetRowToPreset в
#      lib/rule-engine/loaders.ts не пробрасывал legacy student_*
#      поля, из-за чего buildOnePerSpreadAdaptive фактически не
#      работал в проде (только в тестах напрямую). Это блокировало
#      и РЭ.22.4-6 (engine ещё не подключён к новым полям, но через
#      тот же путь).
#
# ⏳ Следующий подэтап: РЭ.22.3 — UI двух-осевая модель.
#    - В PresetEditorModal.tsx заменить три независимых селекта
#      (student_pages_per_student / friend_photos / has_quote) на
#      блок «Личный раздел» с селектом режима (page/spread/grid) и
#      параметрами под него.
#    - Параметры зависят от режима:
#      * page/spread — friend_photos (0..10), has_quote (bool)
#      * grid — grid_size (2..12), has_quote (bool)
#    - При сохранении писать новые поля + дублировать в legacy для
#      отката Vercel (см. spec §5).
#    - Engine в этом подэтапе НЕ ТРОГАЕМ.
#
...
#   - mode='grid'   — сетка N учеников (Medium/Light/Mini)
# Параметры зависят от режима: friend_photos для page/spread,
# grid_size для grid, has_quote для всех режимов.
#
...
#
#   ⏳ Следующий подэтап: РЭ.22.3 — UI двух-осевая модель
#      В PresetEditorModal заменить 3 независимых селекта на блок
#      «Личный раздел» с селектом режима. Параметры зависят от режима.
#      Engine не трогаем.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v93
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v93 — что закрыто (РЭ.22.0..РЭ.22.2 + миграция применена)
#      • docs/phase-Р22-spec.md v1.0 (564 строки) — ПОЛНЫЙ план фазы.
#        Особо §5 (UI контракт двух-осевой модели) для РЭ.22.3.
#      • app/super/presets/_components/PresetEditorModal.tsx — текущий UI.
#        В РЭ.22.3 заменяем секцию «Личный раздел (семантический поиск)»
#        (строки ~290-350 — три независимых селекта) на блок с двух-осевой
#        моделью «режим × параметры».
#      • lib/rule-engine/loaders.ts presetRowToPreset — теперь
#        пробрасывает все student_* поля (legacy + новые).
...
#      • 2026-05-20-presets-student-layout-mode.sql ✅ ПРИМЕНЕНА 20.05.2026
#        (РЭ.22.1, обе колонки + оба CHECK на месте)
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.3 — UI двух-осевая модель.
#      Файл: app/super/presets/_components/PresetEditorModal.tsx
#      Что делаем:
#        - Заменить секцию «Личный раздел (семантический поиск мастера)»
#          с 3 независимыми селектами (student_pages_per_student /
#          friend_photos / has_quote) на новый блок с двух-осевой моделью.
#        - Главный селект — «Режим личного раздела» (page / spread / grid).
#        - Под селектом — параметры в зависимости от режима:
#          • page/spread → friend_photos (0..10), has_quote (bool)
#          • grid → grid_size (свободный input 2..12), has_quote (bool)
#        - При первом открытии без student_layout_mode — fallback по
#          legacy: density='medium'/'light'/'mini' → mode='grid';
#          density='standard'/'universal' → mode='page'; preset.id='maximum'
#          или 'individual' → mode='spread' (см. spec §5).
#        - Сохранение пишет в новые поля + дублирует в legacy
#          student_pages_per_student для отката Vercel:
#          * mode='page' → student_pages_per_student=1
#          * mode='spread' → student_pages_per_student=2
#          * mode='grid' → student_pages_per_student=NULL
#      Что НЕ трогаем: engine (РЭ.22.4-6), карточку пресета (PresetCard
#      в page.tsx — можем добавить отображение нового поля, но это
#      опционально).
