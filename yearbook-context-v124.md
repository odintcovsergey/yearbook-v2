...
# Версия: v124
# Обновлено: 21.05.2026 (после РЭ.25.6 — счётчик в карточке готов).
#
# ⏳ ФАЗА РЭ.25 ПОЧТИ ЗАКРЫТА — осталось только summary (25.7)
# ──────────────────────────────────────────────────────────────────
# Все функциональные подэтапы (25.0..25.6) закрыты. Остался 25.7:
# написать docs/phase-Р25-summary.md и закрыть фазу.
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: 5d87079 (РЭ.25.6).
#
# ✅ Миграция РЭ.25.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026).
#
# Что готово после 25.6 (вся функциональность фазы):
#   • Бэкенд закрыт полностью (миграция + API + engine-фильтр).
#   • UI фотографа в /app:
#     - Чекбокс «Включить в личный раздел всех учеников» в форме альбома.
#     - Колонка «Заказ» в таблице учеников с чекбоксами.
#     - Бейдж «не заказывает» + приглушение строки.
#     - Счётчик «Заказали: N из M» в карточке альбома (показывается
#       только когда есть не-заказчики и не мягкий режим).
#     - Аналогичный блок на детальной странице альбома.
#   • UI родителя в /[token]:
#     - Чекбокс «Мы заказываем альбом» на основном flow.
#     - Тот же чекбокс на экране «Выбор уже сделан».
#   • CSV-экспорт: колонка «Заказ» (да/нет/—).
#   • API dashboard: stats.purchased для каждого альбома.
#   • API album_stats: stats.purchased для детальной страницы.
#
...
#   ✅ 25.6 — счётчик 'N из M заказали' (commit 5d87079)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v124
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v124 — РЭ.25.0..25.6 закрыты. Осталось ТОЛЬКО summary (25.7).
#      • docs/phase-Р25-spec.md v1.0 — спецификация фазы (для сверки скоупа)
#      • docs/phase-Р24-summary.md — образец summary для предыдущей фазы
#      • docs/phase-Р22-summary.md и Р23-summary.md — тоже образцы
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#   5. ФИНАЛЬНЫЙ ПОДЭТАП РЭ.25.7 — закрытие фазы:
#      a) Написать docs/phase-Р25-summary.md по образцу phase-Р24-summary.md:
#         - Что было до фазы
#         - Что сделано (по подэтапам 25.0..25.6, со ссылками на коммиты)
#         - Архитектура фильтра (lib/smart-fill/filter-by-purchase.ts)
#         - Ключевые решения (4 развилки)
#         - Что НЕ сделано (зафиксировано как долг — уведомление
#           фотографу о смене статуса родителем, например)
#         - Статистика (коммиты, тесты, миграции)
#         - Ключевые файлы фазы
#      b) Обновить README или другой index если нужно.
#      c) Закоммитить отдельно: docs(РЭ.25.7): summary фазы РЭ.25 — ...
#      d) Создать yearbook-context-v125.md со статусом «фаза РЭ.25 закрыта,
#         готовность к РЭ.27 (print_type в альбом)».
#
#      После 25.7 — фаза РЭ.25 закрыта. Следующая фаза РЭ.27 — большая
#      (~10-15 коммитов, print_type в альбом + двойной режим редактора).
#      См. docs/phase-Р24-spec.md §13.
