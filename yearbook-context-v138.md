...
# Версия: v138
# Обновлено: 21.05.2026 (после РЭ.28.2 — чистые утилиты resize'а + 55 тестов).
#
# ⏳ ФАЗА РЭ.28 В РАБОТЕ — партнёрские дизайны (сценарий A)
# ──────────────────────────────────────────────────────────────────
# Подэтапы 28.0..28.2 закрыты. Следующий — 28.3 (API endpoints).
# Spec: docs/phase-Р28-spec.md v1.0.
# Последний commit фазы: 888f58c (РЭ.28.2).
#
# ✅ Миграция РЭ.28.1 применена в Supabase (Сергей подтвердил).
#
# Что готово после 28.2 (главная инженерная часть фазы):
#   • lib/template-set-clone/ — 5 файлов чистого кода без Supabase:
#     - constants.ts (PRINT_DPI=300, MM_STEP)
#     - round-to-pixels.ts (roundMmToPx, mmToPx)
#     - aspect-compatibility.ts (три уровня <5%/5-10%/>10%)
#     - resize-placeholder.ts (resizePlaceholder)
#     - prepare-clone.ts (главная функция prepareTemplateSetClone)
#   • Публичный API в lib/template-set-clone/index.ts
#   • 55 новых unit-тестов: 437 → 492 passing на main
#
# Главная функция prepareTemplateSetClone:
#   - Принимает source_template_set + source_masters + new_*
#   - Возвращает ClonePlan со всеми готовыми записями для INSERT
#   - НЕ выполняет операций в БД (только готовит данные)
#   - Throws если aspect_check.level === 'blocked'
#   - parent_template_set_id заполняется ID источника
#   - is_global=false, slug=null у клона
#   - Все mm-значения округляются через roundMmToPx (целые пиксели)
#
# План подэтапов РЭ.28:
#   ✅ 28.0 — spec (commit 1193b5d)
#   ✅ 28.1 — миграция БД (commit 68a7f0a, применена в Supabase)
...
#
# Тесты на main: 492/492 passing (437 + 55 в 28.2).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v138
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v138 — 28.0..28.2 закрыты, чистая инженерия готова
#      • docs/phase-Р28-spec.md §4 «API» — основной контракт
#      • lib/template-set-clone/index.ts — публичный API готовых функций
#      • app/api/tenant/route.ts — место добавления action'ов
#   4. Применены в Supabase: все миграции до РЭ.28.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.28 — следующий подэтап 28.3 (API):
#
#      Три новых action'а в app/api/tenant/route.ts:
#
#      a) template_set_clone (POST):
#         - body: source_template_set_id, new_name, new_page_width_mm,
#           new_page_height_mm, new_bleed_mm?
#         - assertAuth → партнёр или superadmin
#         - SELECT template_sets + spread_templates по source_id
#         - Проверка доступа: source должен быть глобальным
#           (tenant_id IS NULL) или принадлежать партнёру
#         - prepareTemplateSetClone(...) → ClonePlan
#         - Если plan.resize_info.aspect_check.level === 'blocked' → 400
#         - ТРАНЗАКЦИЯ:
#           • INSERT template_sets → новый id
#           • INSERT spread_templates (все мастера, по N штук)
#         - Audit log: 'template_set.clone'
#         - Response: { template_set_id, warnings: [...] }
#         - ⚠️ Supabase не поддерживает явные транзакции через JS клиент,
#           если что-то упадёт после INSERT template_sets — нужен rollback
#           вручную (DELETE template_set). Защита: validate всё что
#           можно валидировать на стороне utility, до INSERT'ов.
#
#      b) template_set_my_list (GET):
#         - SELECT template_sets WHERE tenant_id = auth.tenantId
#         - ORDER BY created_at DESC
#         - Возвращает только КЛОНЫ партнёра (глобальные не включает)
#
#      c) template_set_delete (POST):
#         - body: template_set_id
#         - Проверка: tenant_id === auth.tenantId (партнёр свой удаляет)
#         - Глобальные (tenant_id IS NULL) — никогда не удаляются
#         - COUNT references из albums.template_set_id + presets.template_set_id
#         - Если ≥ 1 → 409 + сообщение с count'ами
#         - DELETE spread_templates (вручную, если нет ON DELETE CASCADE)
#         - DELETE template_sets
#         - Audit log
#
#      Дальше — 28.4 (UI каталога: «Мои дизайны»), 28.5 (UI модалка
#      ввода размеров), 28.6 (summary + закрытие фазы).
