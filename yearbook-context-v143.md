...
# Версия: v143
# Обновлено: 21.05.2026 (spec фазы Р30 готов, начинать с Б.1).
#
# ⏳ ФАЗА РЭ.30 СТАРТОВАЛА — чистка путей сборки альбома
# ──────────────────────────────────────────────────────────────────
# Spec: docs/phase-Р30-spec.md (283 строки, commit 8d5b996)
# Диагностика: docs/phase-Р30-diagnostic.md (commit fe3ce67)
# Следующий шаг: Б.1 (SQL миграция глобальных пресетов на семантику).
#
# Цель фазы:
...
#
# Состояние данных (на момент написания spec):
#   • 6 глобальных пресетов уже мигрированы вручную через SQL
#     UPDATE'ы (см. spec §«Состояние данных»)
#   • individual ещё не мигрирован (density=NULL, layout_mode=NULL)
#   • 2 партнёрских пресета Сергея — смешанные (density + layout_mode)
#   • applies_to_configs у E-* мастеров ещё не снят
#
# План: 7 коммитов в 3 этапа (по spec'у).
#   Этап Б (данные): Б.1 SQL миграция + Б.2 fix PresetEditorModal
#                    + Б.3 cleanup смешанных
#   Этап В (UI): В.1 убрать Section Structure + В.2 убрать density
#                + В.3 убрать комплектацию из формы альбома
#   Этап Г (закрытие): summary + контекст v144
#
