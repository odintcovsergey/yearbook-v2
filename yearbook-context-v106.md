...
# Версия: v106
# Обновлено: 20.05.2026 (после РЭ.23.3 — API endpoints).
#
...
# ✅ РЭ.23.3 (5e672f1) — API endpoints в app/api/tenant/route.ts.
#    • GET action template_set_list_with_previews:
#      выгружает все мастера template_set'а с готовыми SVG-превью.
#      Tenant-aware. Доступ admin/superadmin (не viewer).
#    • POST action template_set_update_display_label:
#      обновляет display_label мастера. Tenant-aware (глобальный
#      template_set → только superadmin).
#    Импорт renderPreviewSvg добавлен в шапку route.ts.
#    369/369 vitest без изменений (API не покрыт unit-тестами,
#    протестируется вручную через UI в РЭ.23.4).
#
# ⏳ Следующий подэтап: РЭ.23.4 — страница /super/master-catalog (UI).
#    Файлы:
#      • app/super/master-catalog/page.tsx (новый) — серверный
#        компонент с auth-проверкой, fetch template_set_list_with_previews.
#      • app/super/master-catalog/_components/MasterCard.tsx —
#        клиентский компонент карточки мастера: SVG-превью,
#        inline-редактор display_label, отображение page_role + slot_capacity.
#      • Возможно фильтр по page_role (select сверху страницы).
#    После применения OkeyBook'ом (РЭ.23.5) и закрытия фазы (РЭ.23.6)
#    переходим к РЭ.24 (готовые пресеты).
#
# Контекст фазы:
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v106
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v106 — что закрыто (РЭ.23.0..23.3)
#      • docs/phase-Р23-spec.md v1.0 — план фазы
#      • app/api/tenant/route.ts — два новых action'а уже готовы
#        (template_set_list_with_previews + template_set_update_display_label)
#   4. Применены в Supabase (все 4 миграции РЭ.22 + РЭ.23.1):
#      • Все миграции до v87 ✅
#      • 2026-05-20-spread-templates-display-label.sql ✅ (РЭ.23.1)
#      (остальные см. в spec'е, тут не повторяю)
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.23.4 — страница /super/master-catalog.
#      Файлы:
#        a) app/super/master-catalog/page.tsx (новый):
#           Серверный компонент. Auth-проверка через requireAuth +
#           проверка role (admin/superadmin). Fetch
#           /api/tenant?action=template_set_list_with_previews напрямую
#           (как в других /super страницах — через relative URL).
#           Заголовок «Каталог мастеров», передаёт masters в client.
#        b) app/super/master-catalog/_components/MasterCatalogClient.tsx
#           (или MasterCardList.tsx — посмотреть существующий паттерн):
#           Клиентский компонент. Список карточек мастеров. Фильтр
#           по page_role (select). Каждая карточка:
#             • SVG-превью (через dangerouslySetInnerHTML)
#             • name (технический, серым шрифтом)
#             • display_label — input для inline-редактирования
#               (placeholder=name). Save через POST
#               template_set_update_display_label.
#             • page_role (badge)
#             • slot_capacity (компактно — students=4, photos_full=1, etc.)
#      Стиль: Tailwind, по образцу существующих /super/* страниц
#      (посмотреть /super/presets для образца). Карточки в grid 3-4
#      колонки. Минималистично, admin-tool.
#      Что НЕ трогаем: engine, существующие /super страницы.
