...
#
# ТЕКУЩАЯ РАБОТА: фаза А (общий раздел альбома).
# А.1 ЗАКРЫТА 11.05.2026 (4 коммита):
#   50cd88a — миграция БД photos.type на 8 значений
#   706fd0f — backend (upload + tenant route фильтры)
#   4e29c66 — UI 5 новых блоков загрузки + 8 табов галереи
#   9fc3222 — UX фидбек (лейблы, разделитель, anti-jump min-h)
#
# А.2 ЗАКРЫТА 11.05.2026 (5 коммитов):
#   a2213ae (А.2.1)   — smart-fill читает common_* фото из БД
#   edaf692 (А.2.2.a) — задача A5 (J-Spread мастер) для дизайнера
#   c41463d (А.2.2.b) — buildCommonSection алгоритм Вариант 1
#   30bd8ab (А.2.2.b1)— фикс: J-* мастера в default_for_configs
#                       + is_spread:false фильтр
#   7527941 (А.2.2.b2)— UI: группировка SpreadInstance'ов в
#                       визуальные развороты в превью альбома
#
# Что дальше — выбирает Сергей. Кандидаты по roadmap:
#   - А.4 (UI настройки разворотов общего раздела, ~0.5 дня)
#   - В (lifecycle YC, ~0.5 дня)
#   - Б (оригиналы для печати, ~2-3 дня)
#   - А.3 (виньетки в Индивидуальной комплектации, ~1 день)
#   - Применение ответов дизайнера когда придут
#
