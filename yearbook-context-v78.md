...
# Версия: v78
# Обновлено: 18.05.2026 (РЭ.20.6.2 — первое mandatory-правило-эталон
#                       + RuleContext.preset_density/preset_sheet_type
#                       + JSON-пресеты синхронизированы с БД)
#
...
#      • Новое правило common-mandatory-mini-soft-page-0-half-pair
#        (priority 230, выше legacy 190) — соответствует первой ячейке
#        матрицы mini × soft × 1-24 учеников = '2 по 1/2 класса'.
#      • 7 JSON-пресетов docs/rule-engine-data/presets/*.json получили
#        density/sheet_type/total_pages на верхнем уровне (sync с БД).
#      • 3 integration-теста в build-edge.test.ts доказывают:
#        правило срабатывает на mini-soft, не повторяется (курсор
#        инкрементируется), не срабатывает на standard.
#      • 200 → 203 теста проходят, tsc + next build чистые.
#   ⏳ Дальше: РЭ.20.6.3 — scripts/generate-rules-from-matrix.ts:
#      автогенерация остальных ~25-30 правил из 28 entries матрицы.
#      Затем РЭ.20.6.4 (seed-rule-engine.ts грузит generated/ в БД).
#      Осталось ~10-14 часов работы.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v78
...
#   4. Код: РЭ.20.2/3/4/5/6.1/6.2 готовы.
#   5. Следующий шаг — РЭ.20.6.3: scripts/generate-rules-from-matrix.ts.
#      Скрипт читает docs/templates/album-structure-matrix.json и
#      генерит JSON-правила в docs/rule-engine-data/rules/common-section/
#      по шаблону common-mandatory-mini-soft-page-0-half-pair.json.
#      ~25-30 правил для 28 entries × ~6 страниц mandatory + ~5 additional.
#
...
#   ✅ РЭ.20.6.2 (a8d82cb) — первое правило-эталон + RuleContext fields
#      + sync JSON-пресетов с БД. 203/203 тестов.
#   1. РЭ.20.6.3 — scripts/generate-rules-from-matrix.ts:
#      Скрипт генерирует JSON-правила по шаблону из РЭ.20.6.2 для всех
#      28 entries матрицы. ~25-30 правил. Записывает в
#      docs/rule-engine-data/rules/common-section/generated/.
#   2. РЭ.20.6.4 — seed-rule-engine.ts грузит generated/ в БД,
#      деактивация legacy common-section-*-pair для пресетов с density
#      (приоритет 230 правил перебивает их сам, но deprecate для чистоты).
#   3. РЭ.20.7 — unit-тесты по каждой строке матрицы.
