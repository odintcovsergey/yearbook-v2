...
#
# Параллельно: продукт B (движок автовёрстки) — фаза 0 закрыта,
# фаза 0.5.3 закрыта (refactor builder под пресеты в БД).
#
# Что нового в v41 относительно v40:
# - Фаза 0.5.3 продукта B полностью закрыта (4 подэтапа: 0.5.3.1 → 0.5.3.4).
# - buildAlbum работает только с Preset из БД (config_presets).
# - Удалены lib/album-builder/build.ts (старый, 1241 строк) и scenarios.ts
#   (806 строк). Единственный builder — lib/album-builder/build-from-preset.ts.
# - Endpoint /api/layout?action=build_album_test переключён на preset_slug.
# - UI Build Test (страница /super/templates/[id]) обновлён.
# - Smoke 58/58 на новом builder'е — поведение эквивалентно старому
#   (главный критерий безрегрессионного рефакторинга).
# - Семантический discovery: last_spread (Медиум), overflow (Лайт/Мини),
#   capacity-pool (Максимум/Индивидуальный) — без декларативных filter'ов.
# - Унификация: 5 ученических функций → 3 обобщённые
#   (buildSinglePagePerStudent / buildSpreadPerStudent / buildGridStudents).
# - НЕ сделано: 0.5.6 (UI выбор пресета per-child в /app, опционально).
#
# Подробности фазы 0.5.3:
#   - docs/phase-0.5-spec.md (v4, финальная спека)
#   - docs/internal/0.5.3.{1,2,3,4}-instructions.md (детальные инструкции)
...
### Продукт B — фаза 0.5.3 (refactor builder под пресеты) — ЗАВЕРШЕНА ✅
...
**Что сделано в фазе 0.5.3:** builder работает только с Preset из БД.
Старые scenarios.ts и build.ts удалены, единственный builder теперь
build-from-preset.ts. Поведение полностью эквивалентно старому
(smoke 58/58).

**Архитектурный путь:** Г1 — прямое переписывание под Preset, без адаптера
(выбран в начале фазы). Старые и новые builder сосуществовали только
во время разработки 0.5.3.1-0.5.3.3, в 0.5.3.4 старые удалены.
...
**Не сделано (опциональное, отложено):**
- 0.5.6 — UI выбор пресета per-child в /app (поле children.config_preset_id
  существует с 0.5.1, dropdown в карточке ученика — UX-улучшение)
...
- master-cleanup-tz §B3 — двухстраничный E-Student-Default для Универсала
  (решение b отложено в master-cleanup-tz)

Подробности: `docs/phase-0.5-spec.md` (v4) +
`docs/internal/0.5.3.{1,2,3,4}-instructions.md`.
...
| ✅ 0.5.3 | Refactor под пресеты в БД (4 подэтапа) | Сделано |
| 🔵 0.5.6 | UI выбор пресета per-child (опционально) | ~1.5 ч |
...
## ПРОДУКТ B — ФАЗА 0.5.3 ЗАВЕРШЕНА ✅
...
перенести структуру в БД. Фаза 0.5.3 решила эту задачу через прямое
переписывание builder'а под Preset (путь Г1, без адаптера).
...
- **Богатая БД, простая логика.** Структура config в JSONB готова под все
  ...
  cover_section.cover_type, personal_spread_addon), но builder в фазе 0.5.3
  читает только подмножество полей.
- **Семантический discovery.** Builder ищет мастера через `page_role` +
  `default_for_configs` + `slot_capacity` без декларативных filter'ов:
...
- 0.5.6 — UI выбор пресета per-child в `/app` (опционально, поле есть в БД)
- Расширение smoke до 65+ сцен с edge-cases для grid (опционально)
- master-cleanup-tz §B3 — двухстраничный мастер для Универсала (P2)

### Связь с текущим состоянием

После 0.5.3.4 фаза 0.5.3 закрыта. Дальше — фаза 1 (Smart-fill)
без блокеров. UI per-child (0.5.6) — параллельно или позже.
