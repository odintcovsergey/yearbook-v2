...
# Версия: v110
# Обновлено: 20.05.2026 (после РЭ.24.2 — утилита validatePreset).
#
# ФАЗА РЭ.24 — Готовые шаблоны для быстрого старта партнёра (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.24.0 (614e6a7) — docs/phase-Р24-spec.md v1.0 (227 строк).
# ✅ РЭ.24.1 (037f9ff) — миграция БД (is_recommended + parent_preset_id),
#    ✅ применена в Supabase 20.05.2026.
# ✅ РЭ.24.2 (e526965) — утилита validatePreset + 18 тестов.
#    lib/presets/validate.ts: чистая функция
#      validatePreset(preset) → { valid, errors[] }
#    6 правил: display_name, print_type, template_set_id,
#    section_structure, student_layout_mode (если есть students),
#    student_grid_size (если mode='grid', в диапазоне 2..12).
#    errors — массив человеко-читаемых строк на русском.
#    387/387 vitest passing (было 369/369).
#
# ⏳ Следующий подэтап: РЭ.24.3 — buildPresetPreviewBundle.
#    lib/presets/preview-bundle.ts: чистая функция которая делает
#    dry-run сборку альбома с фиктивными данными (10 учеников,
#    5 предметников, 1 фото класса) → собирает ключевые развороты
#    → каждый рендерится через renderPreviewSvg → возвращает 4 SVG:
#      { students: SVG | null, cover: SVG | null,
#        teachers: SVG | null, soft: SVG | null }
#    Будет использоваться в API РЭ.24.4 для отдачи превью карточек.
#    Тесты на типичные пресеты (grid, page, без cover, без teachers).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v110
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v110 — что закрыто (РЭ.24.0..24.2)
#      • docs/phase-Р24-spec.md v1.0 — полный план фазы (227 строк)
#      • lib/presets/validate.ts — образец чистой функции
#      • lib/album-builder/render-preview-svg.ts — переиспользуем в .3
#   4. Применены в Supabase:
#      • Все миграции до РЭ.23.1 ✅
#      • 2026-05-20-presets-recommended-and-parent.sql ✅ (РЭ.24.1)
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.24.3 — buildPresetPreviewBundle.
#      Файл: lib/presets/preview-bundle.ts (новый)
#      Сигнатура:
#        buildPresetPreviewBundle(
#          preset: Preset,
#          bundle: RuleEngineBundle,
#        ): {
#          students: string | null,  // SVG главного превью (личный)
#          cover: string | null,     // SVG обложки (или null если нет)
#          teachers: string | null,  // SVG учительского (или null)
#          soft: string | null,      // SVG soft_intro/final (или null)
#        }
#      Алгоритм:
#        1) Сформировать фиктивный RulesAlbumInput (10 учеников,
#           5 предметников, 1 фото класса, head_teacher).
#        2) Вызвать buildFromSectionStructure(bundle, fakeInput)
#           — получить массив pageInstances.
#        3) Для каждого pageInstance.master_id найти SpreadTemplate
#           в bundle.mastersByName.
#        4) Сгруппировать по семантике секции (cover / students /
#           teachers / soft_intro_final). Взять первый мастер каждой
#           группы — это «ключевой» разворот.
#        5) Вызвать renderPreviewSvg для каждого, вернуть 4 SVG.
#      Если какой-то секции нет — соответствующее поле = null.
#      Тесты (~6-8): валидные пресеты разных типов, отсутствие cover,
#      отсутствие soft (layflat), grid vs page vs spread.
#      Будет использоваться в API РЭ.24.4 для отдачи превью.
#      Что НЕ трогаем: API, UI — РЭ.24.4-5.
