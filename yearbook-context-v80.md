...
# Версия: v80
# Обновлено: 18.05.2026 (РОЛЛБЭК 183 правил + генератора —
#                       неверная семантика «ячейки»; уточнена
#                       архитектура: слоты в БД + UI редактор)
#
...
#   ⏳ Дальше: РЭ.20.6 переосмысленно. Архитектура (подтверждена Сергеем):
#      • Структура общего раздела (массив слотов) хранится в БД,
#        attached к пресету. Слоты типов H/Q/FULL/flex_A/flex_B/flex_C
#        + (опционально) кастомные.
#      • UI редактор пресета позволяет добавлять/удалять/переставлять
#        слоты, менять цепочки приоритетов внутри flex-слотов.
#      • Правила в rule engine генерируются из этой БД-структуры
#        автоматически (на лету или через регенерацию).
#      Это вливается в РЭ.12 (UI редактор пресетов), которая теперь
#      становится приоритетом. Дефолтные слоты для каждой комплектации —
#      берутся из build_album.jsx mandSlots/addSlots (Сергей загрузил
#      скрипт в чат 18.05.2026).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v80
#   3. ОБЯЗАТЕЛЬНО прочитать пользовательские скрипты InDesign
#      (Сергей прислал в чате 18.05.2026):
#      • build_album.jsx — основная сборка, функция pushCommonSlot
#        (строки 913-991) — эталон логики слотов H/Q/FULL/flex_A/B/C.
#        Каждый слот = ОДНА страница. Не разворот.
#      • build_cover.jsx — обложка.
#      • build_tryumo.jsx — трюмо.
#   4. Применены и в Supabase:
...
#   5. Состояние кода: РЭ.20.2/3/4/5/6.1/6.2 готовы, .6.3 откачен.
#   6. Следующий шаг — НЕ генерировать новые правила в файлы. Перейти к
#      архитектуре «слоты в БД + UI редактор». Это часть РЭ.12.
#      Конкретные шаги — обсудить с Сергеем перед началом.
#
...
#
# Порядок реализации (обновлено после rollback 18.05.2026):
...
#      Неверная семантика. Будет переделан в РЭ.12 через БД-структуру.
#
# Новый план (на согласовании):
#   1. РЭ.12.А — миграция БД: presets.section_structure jsonb
#      (массив слотов H/Q/FULL/flex_A/flex_B/flex_C с их параметрами).
#      Дефолты для каждой комплектации берутся из build_album.jsx
#      mandSlots/addSlots.
#   2. РЭ.12.Б — UI редактор пресетов /app/presets:
#      • Список + copy глобального пресета.
#      • Редактор total_pages.
#      • Редактор section_structure (drag&drop слотов).
#      • Редактор сheet_type (тип листов на уровне альбома, не пресета).
#   3. РЭ.20.6 — переписать build.ts чтобы он использовал
#      preset.section_structure напрямую при сборке общего раздела
#      (без отдельных правил per-cell). Логика слота — как в
#      pushCommonSlot из build_album.jsx.
#   4. РЭ.20.7 — unit-тесты на каждый слот + smoke по комплектациям.
#   5. РЭ.20.8 — финальная документация.
#
