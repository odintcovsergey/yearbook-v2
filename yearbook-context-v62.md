...
# Версия: v62
# Обновлено: 16.05.2026 (РЭ.3.5 закрыт — IDML v2 в БД, 7 семейств в template_families)
#
...
# Rule engine спецификация v1.3 готова (16.05.2026, после получения IDML v2) — фундамент реализации
# buildFromRules для запуска партнёрки в сентябре.
# Подэтапы РЭ.1+РЭ.2+РЭ.3+РЭ.3.5 закодированы и применены.
# ✅ Миграция rule-engine-migration.sql применена в Supabase Сергеем 16.05.2026.
...
# ✅ 7 семейств (head-teacher, subject-teachers, class-photo, student-section,
#    common-section, intro, final) UPSERTнуты в template_families через
#    seed-rule-engine.ts --write.
# Семантика Combined уточнена: M-Combined=2 портрета, L-Combined=3, N-Combined=4.
# Это отдельный продуктовый вид страницы для маленького остатка учеников
# (1..N) + общее фото, не «обрезанная полная сетка».
# Правило применения: students_remaining <= MAX_SLOTS_COMBINED[density] + есть общее фото.
# Следующий шаг: РЭ.4 — правила head-teacher + subject-teachers + class-photo
# (включая F-Head-WithClassPhoto-L) + vitest тесты.
#
...
#    Подэтапы реализации РЭ.1-РЭ.3.5 ✅ выполнены, РЭ.4-РЭ.13 запланированы.
#
...
# РЭ.13 — Документация (designer-tz v1.5 УЖЕ есть — designer-tz-2026-05-16-v1.5.md)
