...
# Версия: v122
# Обновлено: 21.05.2026 (после РЭ.25.4 — UI фотографа готов).
#
# ⏳ ФАЗА РЭ.25 В РАБОТЕ — Галка покупки альбома ребёнком
# ──────────────────────────────────────────────────────────────────
# Подэтапы 25.0..25.4 закрыты. Следующий — 25.5 (UI родителя в
# /[token] + колонка 'Заказ' в CSV).
# Spec: docs/phase-Р25-spec.md v1.0 (334 строки).
# Последний commit фазы: f3cdbf4 (РЭ.25.4 UI фотографа).
#
# ✅ Миграция РЭ.25.1 ПРИМЕНЕНА в Supabase (Сергей подтвердил 21.05.2026).
#
# Что готово после 25.4:
#   • Бэкенд закрыт полностью (миграция + API + engine-фильтр).
#   • UI фотографа в /app:
#     - Чекбокс «Включить в личный раздел всех учеников» в форме
#       альбома (между блоком шаблона и legacy-пресетом). По умолчанию
#       выключен.
#     - Бейдж «не заказывает» рядом с ФИО ученика в списке + строка
#       приглушена (opacity-60). Видно с первого взгляда кто без альбома.
#     - В expanded-секции ученика — чекбокс «Заказывает альбом» рядом
#       с пресет-селектом. Сохраняется через update_child.
#
...
#   ✅ 25.4 — UI фотографа (commit f3cdbf4)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v122
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v122 — РЭ.25.0..25.4 закрыты. Фаза в работе.
#      • docs/phase-Р25-spec.md v1.0 (334 строки) — спецификация фазы,
#        особенно §5.3 (UI родителя) и §6.4 (CSV-экспорт)
#      • docs/phase-Р24-summary.md — последняя закрытая фаза (контекст)
#   4. Применены в Supabase: все РЭ.22, РЭ.23.1, РЭ.24.1, РЭ.25.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.25 — следующий подэтап 25.5:
#      a) UI РОДИТЕЛЯ — app/[token]/page.tsx:
#         - Добавить чекбокс 'Мы заказываем альбом' (по умолчанию
#           проставлен, чтение из data.child.is_purchased).
#         - При смене → PUT /api/select { token, action:'set_purchased',
#           value:boolean } (endpoint уже работает из 25.2).
#         - Доступно ВСЕГДА, даже при alreadySubmitted=true.
#         - Подсказка: 'Если не покупаете альбом, снимите галочку.
#           Личной страницы для ребёнка в альбоме не будет, но он
#           останется на общих классных фото и в виньетке класса.'
#         - ⚠️ НЕ ТРОГАТЬ ДЕСТРУКТИВНО логику отбора/draft/submit
#           (см. правило безопасности проекта). Только аддитивная правка.
#      b) CSV-ЭКСПОРТ — app/api/tenant/route.ts (action=export_csv):
#         - Добавить колонку 'Заказ' (значения 'да' / 'нет').
#         - Позиция: сразу после 'Класс' / 'Ученик' / 'Портрет_страница',
#           перед остальными. Конкретная позиция не критична.
#         - SELECT children расширить полем is_purchased (если ещё нет).
#         - Для учительских строк колонка пустая или 'учитель'.
#      Дальше — 25.6 (счётчик 'N из M' в карточке альбома), 25.7
#      (summary + закрытие фазы).
