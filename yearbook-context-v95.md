...
# Версия: v95
# Обновлено: 20.05.2026 (после РЭ.22.4 — engine semantic для mode='page').
#
...
#    В sections/students.ts добавлена приоритетная ветка: если
#    preset.student_layout_mode='page' → новая buildPageSemantic через
#    findStudentMaster. Семантический поиск мастера по pageRole +
#    photos_friend + has_quote + has_portrait. Fallback на legacy
#    buildAlternatingLR когда mode=NULL.
#    +8 тестов в sections-students-page-semantic.test.ts.
#    Всего тестов в проекте: 324/324 passing (было 316/316).
#
# ⏳ Следующий подэтап: РЭ.22.5 — engine semantic для mode='spread'.
#    - Расширить buildOnePerSpreadAdaptive (РЭ.21.8.15) на любой пресет
#      где student_layout_mode='spread' (не только preset.id='individual').
#    - Сейчас функция активна только для Individual через два условия
#      (preset.id='individual' + student_pages_per_student=2 + student_has_quote
#      != null). После РЭ.22.5 — активируется через student_layout_mode='spread'
#      и работает для maximum / individual / любых партнёрских spread-пресетов.
#    - Тесты для нового пути активации.
#    - Engine для mode='page' остаётся как есть (РЭ.22.4).
#
...
#
#   ⏳ Следующий подэтап: РЭ.22.5 — engine semantic для mode='spread'
#      Расширить активацию buildOnePerSpreadAdaptive на любой пресет где
#      student_layout_mode='spread' (сейчас активна только для Individual).
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v95
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v95 — что закрыто (РЭ.22.0..РЭ.22.4)
#      • docs/phase-Р22-spec.md v1.0 — §6 (Engine контракт), особо §6.5
#        про spread (двухстраничный режим).
#      • lib/rule-engine/sections/students.ts — текущая структура.
#        В fillStudentsSection уже есть приоритетная ветка для mode='page'
#        (РЭ.22.4). Для mode='spread' аналогичная ветка добавляется.
#      • Функция buildOnePerSpreadAdaptive (РЭ.21.8.15) — основа для
#        mode='spread'. Сейчас активируется только для preset.id='individual'.
#        В РЭ.22.5 расширяем активацию на любой пресет с student_layout_mode='spread'.
...
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.5 — engine semantic для mode='spread'.
#      Файлы:
#        - lib/rule-engine/sections/students.ts:
#          В fillStudentsSection после ветки mode='page' добавить ветку
#          mode='spread' → buildOnePerSpreadAdaptive (с возможно небольшим
#          рефактором последней — она использует preset.student_has_quote).
#          Когда mode='spread' активен, эта ветка перехватывает любой
#          пресет, не только preset.id='individual'.
#          Старая активация через preset.id='individual' (в ветке density=NULL)
#          может остаться для обратной совместимости — её не трогаем.
#        - lib/rule-engine/__tests__/sections-students-spread-semantic.test.ts
#          (новый файл) — 5-7 тестов: spread для custom-пресета, spread для
#          maximum через новый путь, fallback на legacy, etc.
#      Что НЕ трогаем:
#        - buildPageSemantic (РЭ.22.4) — работает как есть
#        - buildOnePerSpread (legacy для preset.id='maximum') —
#          остаётся как fallback
#        - buildAlternatingLR (legacy для density='standard'/'universal')
#        - buildGrid (РЭ.22.6)
