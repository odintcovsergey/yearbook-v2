...
# Версия: v114
# Обновлено: 20.05.2026 (после РЭ.24.5b — multi-design навигация).
#
...
# ✅ РЭ.24.5 (6c5b6f2) — страница /app/templates (одноуровневая)
# ✅ РЭ.24.5b (fc7366a + 9ebb410 + 064140f) — multi-design:
#    • spec v1.1 расширен секцией §12 (архитектура multi-design)
#    • API: новый action designs_list, фильтр design_id в существующих
#    • UI: двухуровневая навигация
#      /app/templates — выбор дизайна (новая страница)
#      /app/templates/[designId] — шаблоны внутри дизайна
#        (перенесена из старой /app/templates)
#    • template_create_blank принимает template_set_id
#    Текущая БД: 1 template_set okeybook-default. В планах ~20-30 дизайнов.
#    Каждый дизайн = template_set. Пресет жёстко привязан к дизайну.
#    Клонирование сохраняет дизайн.
#
# ⏳ Следующий подэтап: РЭ.24.6 — модалка выбора шаблона при создании альбома.
#    Текущий flow создания альбома в /app — нужно найти и расширить.
#    Сначала обсудить с Сергеем — заменить полностью или расширить.
#    Модалка должна быть двухшаговая: сначала дизайн, потом шаблон.
#    Подсветить невалидные шаблоны как disabled.
#
# Архитектура РЭ.24 — 4 уровня:
#   1. ГЛОБАЛЬНЫЕ — presets WHERE tenant_id IS NULL
#   2. КАТАЛОГ /app/templates ✅ (двухуровневая навигация)
...
#   ⏳ 24.6 — UI: модалка выбора пресета при создании альбома (двухшаговая)
