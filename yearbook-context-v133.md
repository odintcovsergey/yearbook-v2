...
# Версия: v133
# Обновлено: 21.05.2026 (после РЭ.27.6 — UI формы альбома готов).
#
# ⏳ ФАЗА РЭ.27 В РАБОТЕ — print_type в АЛЬБОМЕ + правила переплёта
# ──────────────────────────────────────────────────────────────────
# Подэтапы 27.0..27.6 закрыты. Все UI-подэтапы готовы.
# Остаются: 27.7 (миграция данных) + 27.8 (summary).
# Spec: docs/phase-Р27-spec.md v1.0.
# Последний commit фазы: c421897 (РЭ.27.6).
#
# ✅ Миграция РЭ.27.1 применена в Supabase.
#
# Что готово после 27.6 (вся пользовательская функциональность):
#   • БД: albums.print_type существует, индекс, CHECK constraint.
#   • API:
#     - create_album/update_album принимают явный print_type (РЭ.27.2).
#     - action='album' возвращает effective_print_type (РЭ.27.4).
#   • Engine: resolvePrintType + чистые модули + интеграция в layout API.
#     30 unit-тестов (РЭ.27.3).
#   • UI редактора:
#     - Жёлтая плашка для soft-альбомов (РЭ.27.4).
#     - Визуальная заглушка «Форзац» рядом с canvas для soft (РЭ.27.4).
#     - Spread-мастера серые в TemplatePickerModal для soft (РЭ.27.5).
#   • UI формы альбома:
#     - Новый селект «Тип листов в альбоме» (РЭ.27.6).
#     - Опции: «Из шаблона (по умолчанию)» / «Твёрдые» / «Мягкие».
#     - Работает на create и update.
#     - Сохраняется в albums.print_type через body.print_type.
#     - Engine применит при следующей пересборке.
#
# Защита spread-мастеров (двойная):
#   • Engine (РЭ.27.3): isMasterAllowedForPrintType фильтрует в build.
#   • UI (РЭ.27.5): TemplatePickerModal opacity-40 + tooltip.
#
# Что ещё НЕ работает:
#   • 6 NULL-альбомов в проде не заполнены (это 27.7).
#     Для них в engine работает fallback на preset.print_type — поведение
#     корректное, просто значение в БД ещё не материализовано.
#   • Дубль-пресеты «layflat»/«soft» не слиты — каталог /app/templates
#     показывает оба варианта одного дизайна (это 27.7).
#
# Что зафиксировано в spec (7 архитектурных развилок):
...
#   ✅ 27.5 — UI палитры: spread-мастера серые при soft (commit ed380a3)
#   ✅ 27.6 — UI формы: селект «Тип листов» (commit c421897)
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v133
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v133 — РЭ.27.0..27.6 закрыты, вся UI-функциональность готова
#      • docs/phase-Р27-spec.md §2.3 «Миграция 2 — данные»
#      • docs/phase-Р27-spec.md §5 «Слияние дубль-пресетов»
#   4. Применены в Supabase: все РЭ.22..РЭ.25.1 + РЭ.27.1 ✅
#   5. ПРОДОЛЖЕНИЕ ФАЗЫ РЭ.27 — следующий подэтап 27.7 (миграция данных):
#      Это ОСТОРОЖНЫЙ подэтап — затрагивает живые альбомы и пресеты.
#      ПЕРЕД ЗАПУСКОМ: снапшот таблиц albums и presets в Supabase.
#
#      Две части:
#
#      a) Заполнение albums.print_type у 6 NULL-альбомов:
#         SQL:
#           UPDATE albums a
#           SET print_type = p.print_type
#           FROM presets p
#           WHERE a.config_preset_id::text = p.id::text
#             AND a.print_type IS NULL
#             AND p.print_type IS NOT NULL;
#
#         Или через section_structure_preset_id (тот случай тоже бывает —
#         см. РЭ.21+).
#
#         Альтернатива (поскольку config_preset_id это slug, а не uuid):
#           UPDATE albums a
#           SET print_type = p.print_type
#           FROM presets p
#           WHERE a.config_preset_id = p.slug
#             AND a.print_type IS NULL
#             AND p.print_type IS NOT NULL;
#
#         После — проверить: SELECT print_type, COUNT(*) FROM albums GROUP BY print_type;
#         Ожидание: NULL ≤ 0 (или 1-2 если у пресета нет print_type).
#
#      b) Слияние дубль-пресетов «Стандарт-layflat» / «Стандарт-soft»
#         в один «Стандарт»:
#         СНАЧАЛА запросить актуальный список пресетов:
#           SELECT id, slug, name, print_type FROM presets ORDER BY slug;
#         Сергей даёт явный список пар. По ним:
#         1. UPDATE albums SET config_preset_id = '<layflat-slug>'
#            WHERE config_preset_id = '<soft-slug>';
#         2. DELETE FROM presets WHERE slug = '<soft-slug>';
#         3. Переименовать оставшиеся: убрать суффикс -layflat из slug.
#         4. Колонку presets.print_type оставляем как deprecated (NULL).
#
#      Перед миграцией данных ЛУЧШЕ создать миграционный SQL-файл:
#         migrations/2026-05-21-albums-print-type-data-and-preset-merge.sql
#      Туда положить все SQL'и с комментариями.
#
#      Дальше — 27.8 (summary + закрытие фазы).
