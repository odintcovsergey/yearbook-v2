...
# Версия: v96
# Обновлено: 20.05.2026 (после РЭ.22.5 — engine semantic для mode='spread').
#
...
#    FIXED модель: один и тот же мастер для всех учеников, photos_friend
#    берётся из пресета. Отличается от per-student adaptive в
#    buildOnePerSpreadAdaptive (РЭ.21.8.15) для Individual — та
#    остаётся активной через legacy путь (mode=NULL + preset.id='individual').
#    +7 тестов в sections-students-spread-semantic.test.ts.
#    Всего тестов в проекте: 331/331 passing (было 324/324).
#
# ⏳ Следующий подэтап: РЭ.22.6.0 — data-миграция grid-мастеров.
#    ПЕРЕД переключением engine для mode='grid' (РЭ.22.6) нужно
#    разметить существующие grid-мастера в БД:
#    M/L/N-Grid-Page, *-Combined-Page, L-2/3/4, N-4/6/9.
#    SQL: UPDATE spread_templates SET page_role=..., slot_capacity=...
#    Это data-миграция, не структурная — без ALTER TABLE.
#    Применяется Сергеем в Supabase ДО деплоя кода РЭ.22.6.
#
...
#   ⏳ Следующий подэтап: РЭ.22.6.0 — data-миграция grid-мастеров
#      Перед переключением engine для mode='grid' (РЭ.22.6) нужно
#      разметить существующие grid-мастера: page_role + slot_capacity.students.
#      M-Grid-Page, L-Grid-Page, N-Grid-Page, *-Combined-Page,
#      L-2/3/4, N-4/6/9. SQL data-миграция.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v96
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v96 — что закрыто (РЭ.22.0..РЭ.22.5)
#      • docs/phase-Р22-spec.md v1.0 — §6.4 (соглашение по slot_capacity
#        для grid-мастеров) и §10 (план подэтапов).
#      • migrations/2026-05-19-okeybook-default-student-master-tags.sql —
#        образец data-миграции (UPDATE spread_templates SET slot_capacity = ...).
#      • lib/rule-engine/sections/students.ts — buildGrid использует
#        жёсткие имена M/L/N-Grid-Page и адаптивный список L-2/3/4,
#        N-4/6/9. В РЭ.22.6 их заменим на семантический поиск.
...
#      • 2026-05-XX-okeybook-grid-master-tags.sql ⬜ БУДЕТ В РЭ.22.6.0 —
#        разметит существующие M/L/N-Grid-Page + Combined + L-N/N-N
#        тегами page_role='student_grid_left'/'right' и
#        slot_capacity.students=N + photos_full=1 для combined.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.6.0 — data-миграция grid-мастеров.
#      Файл: migrations/2026-05-XX-okeybook-grid-master-tags.sql
#      Что в нём:
#        - UPDATE spread_templates SET page_role='student_grid'/* или
#          student_grid_left/right если есть L/R вариация */, slot_capacity =
#          jsonb_build_object('students', N, 'has_quote', bool, 'has_portrait', true,
#                              'has_name', true)
#          для всех M/L/N-Grid-Page, *-Combined-Page (+ photos_full=1),
#          адаптивных L-2/L-3/L-4, N-4/N-6/N-9.
#        - Конкретные значения N зависят от реальных мастеров —
#          СПРОСИТЬ у Сергея SELECT name, slot_capacity FROM spread_templates
#          WHERE name LIKE '%Grid%' OR name LIKE 'L-%' OR name LIKE 'N-%'
#          OR name LIKE '%Combined%' ORDER BY name;
#          или взять из inventory (lib/rule-engine/album-structure-okeybook.ts).
#        - has_quote=true только если у мастера есть studentquote_N
#          для каждого ученика (см. spec §6.4).
#      Что НЕ трогаем: engine, UI, типы. Только SQL.
#      После коммита — Сергей применяет в Supabase ДО деплоя РЭ.22.6.
