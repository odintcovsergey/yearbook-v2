...
# Версия: v84
# Обновлено: 18.05.2026 (РЭ.21.6 закрыт целиком — 3 коммита:
#   миграция БД (presets.template_set_id uuid FK) → бэкенд (типы,
#   loader, build engine, API валидация) → UI (селект «Дизайн» в
#   форме создания пресета + отображение в карточке).)
#
...
#   1. РЭ.21.7 — редактор структуры пресета.
#      Кнопка «Редактировать» на карточке. Drag-and-drop секций + слотов.
#      POST action rule_preset_update.
#      Также: возможность редактировать min_pages/max_pages/template_set_id
#      существующего пресета (сейчас они задаются только при создании).
...
#   формы создания (новые пресеты) или вручную через SQL для существующих.
#
