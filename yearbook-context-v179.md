...
# Версия: v179
# Обновлено: 29.05.2026 (большой день: фоны template_set + AI-помощник Этапы 1-4)
#
# ┌────────────────────────────────────────────────────────────────────┐
# │ ЧТО НОВОГО В V179 (28-29.05.2026 — фоны template_set + AI-помощник)│
# │                                                                    │
# │ 16 коммитов, два больших независимых блока:                        │
# │   A) Фоновые изображения template_set (6 коммитов): загрузка →     │
# │      рендер канвас → миниатюры → PDF-экспорт                       │
# │   B) AI-помощник для родителей (8 коммитов): Этапы 1-4 + анкеты    │
# │      grade4/garden/grade11 + UX-полировки + feature flag → дефолт  │
# │   Плюс CLAUDE.md (рабочий гайд для Claude Code) и правка ритуала   │
# │   commit+push.                                                     │
# │                                                                    │
# │ ╔══════════════════════════════════════════════════════════════╗   │
# │ ║ БЛОК A — ФОНОВЫЕ ИЗОБРАЖЕНИЯ TEMPLATE_SET                    ║   │
# │ ╚══════════════════════════════════════════════════════════════╝   │
# │                                                                    │
# │ Контекст: дизайнер готовит PNG-разворот с фоном, рамками и         │
# │ декором. Один такой файл = один набор мастеров. Раньше в системе   │
# │ фона не было — мастера рисовались на белой странице. Теперь        │
# │ супер-админ загружает PNG/JPG в template_set, и он автоматически   │
# │ ложится первым слоем на каждый разворот: в редакторе, в миниатюрах │
# │ и в финальном PDF.                                                 │
# │                                                                    │
# │ MVP-решения (см. memory/project_background_system.md):             │
# │   • Один фон на весь template_set (несколько фонов под разные      │
# │     типы разворотов — в backlog).                                  │
# │   • Public bucket → доступ по прямой ссылке без токена.            │
# │   • Программируемых рамок/слотов поверх фона пока нет, всё рисует  │
# │     дизайнер.                                                      │
# │   • softShift и точное позиционирование под bleed — в backlog.     │
# │                                                                    │
# │ ═══════ feat 3cf16cd — API загрузки фона ═══════                   │
# │                                                                    │
# │ • Миграция 2026-05-28-template-backgrounds-bucket.sql:             │
# │     INSERT в storage.buckets bucket 'template-backgrounds'         │
# │     (public, 50 MB, image/jpeg + image/png). ON CONFLICT DO        │
# │     NOTHING — идемпотентна.                                        │
# │   Запись через service_role с сервера → политики на write не       │
# │   нужны.                                                           │
# │                                                                    │
# │ • Миграция 2026-05-28-template-sets-default-background-url.sql:    │
# │     template_sets +default_background_url text                     │
# │   Хранится как путь '<template_set_id>/default.<ext>'; public URL  │
# │   собирается на клиенте как `${SUPABASE_URL}/storage/v1/object/    │
# │   public/template-backgrounds/${path}`.                            │
# │                                                                    │
# │ • app/api/super/template-sets/[id]/background/route.ts (новый):    │
# │     POST — multipart, валидация MIME + 50 MB, загрузка через       │
# │     supabase.storage.from('template-backgrounds').upload c         │
# │     contentType, апсерт path в template_sets.default_background_   │
# │     url, удаление старого файла если был.                          │
# │     DELETE — снимает фон и зануляет колонку.                       │
# │   Только супер-админ (auth с user.role='super').                   │
# │                                                                    │
# │ ═══════ feat 6c4aab5 — UI загрузки фона в супер-админке ═══════    │
# │                                                                    │
# │ • Новый компонент app/super/templates/_components/Template-        │
# │   BackgroundPanel.tsx: превью текущего фона (img-tag, public URL), │
# │   input type=file + кнопка «Заменить»/«Загрузить», кнопка          │
# │   «Удалить» если фон есть.                                         │
# │ • Подключён в app/super/templates/[id]/page.tsx под шапкой набора. │
# │ • TEMPLATE_SET_FIELDS и тип TemplateSet расширены полем            │
# │   default_background_url (string|null).                            │
# │                                                                    │
# │ ═══════ feat 239b75d — рендер фона на канвасе редактора ═══════    │
# │                                                                    │
# │ • AlbumSpreadCanvas получил пропсы backgroundUrl?: string | null   │
# │   и pageSide: 'spread' | 'left' | 'right'.                         │
# │ • Новый внутренний слой SpreadBackgroundLayer рисует Konva.Image   │
# │   первым (до всех плейсхолдеров) с использованием use-image.       │
# │ • Логика обрезки половинок для одностраничных мастеров:            │
# │     pageSide='left'  → image тянется на 2× ширины разворота,       │
# │                        x=0 (показываем левую половину).            │
# │     pageSide='right' → image тянется на 2× ширины, x=-pageWidth    │
# │                        (правая половина видна).                    │
# │     pageSide='spread' → 1× ширины, x=0.                            │
# │ • app/app/album/[id]/layout/page.tsx подтягивает default_back-     │
# │   ground_url из template_set, собирает public URL и прокидывает    │
# │   через все level'ы канваса.                                       │
# │                                                                    │
# │ ═══════ feat 0f4705f — фон в миниатюрах разворотов ═══════         │
# │                                                                    │
# │ • LayoutPreviewStrip (страница «Обзор»): backgroundUrl читается    │
# │   из template_set.default_background_url, передаётся во все       │
# │   мини-канвасы (spread / left / right режимы).                     │
# │ • SpreadOrderStrip (нижняя полоса миниатюр в редакторе): новый     │
# │   prop backgroundUrl, проброс через SpreadCard в DraggablePage и   │
# │   во внутренний мини-канвас.                                       │
# │ • Редактор альбома прокидывает backgroundUrl одной строкой в       │
# │   полосу миниатюр.                                                 │
# │                                                                    │
# │ Результат: фон одинаково виден в трёх местах — большой канвас,    │
# │ полоса миниатюр внизу редактора, превью на вкладке «Обзор».        │
# │                                                                    │
# │ ═══════ feat e8e4db2 — рендер фона в PDF-экспорте ═══════          │
# │                                                                    │
# │ • TemplateSet (в lib/pdf-export/types) расширен опциональным       │
# │   default_background_url.                                          │
# │ • pipeline.ts: новая функция loadBackground — fetch URL один раз  │
# │   на экспорт, embedJpg/embedPng в зависимости от content-type,    │
# │   результат кэшируется в PdfExportContext.                         │
# │ • drawBackground вызывается ПЕРВЫМ перед всеми слоями страницы.    │
# │ • Логика стороны через sideByIndex (использует segmentToSpreads):  │
# │     spread → drawImage по ширине разворота                         │
# │     left  → 2× ширины, x=0                                         │
# │     right → 2× ширины, x=-pageWidth                                │
# │   Для одностраничных мастеров (pageHint='single') сторона          │
# │   определяется автоматически.                                      │
# │ • Фон рисуется в trim-зону без bleed (для теста ок; типография    │
# │   получит PNG с запасом на обрез отдельно).                        │
# │ • При ошибке fetch — warning 'image_decode_failed', экспорт        │
# │   продолжается без фона (не падаем).                               │
# │                                                                    │
# │ ═══════ fix 34562d6 — половинки через sharp вместо negative-x ═════│
# │                                                                    │
# │ КРИТИЧНЫЙ БАГ найден при первом тестовом экспорте: для правой      │
# │ страницы pdf-lib drawImage с x=-trim_w полагался на обрезку        │
# │ pdf-просмотрщиком по mediaBox. Preview macOS (и, по логам,          │
# │ некоторые другие просмотрщики) НЕ обрезают изображение по          │
# │ границам страницы и рисуют его от x=0 — обе страницы показывали   │
# │ левую половину фона.                                               │
# │                                                                    │
# │ Решение: loadBackground теперь через sharp.extract нарезает        │
# │ исходник на ДВЕ половины (left + right) ещё до pdf-lib.embed.      │
# │ Каждая половина embed-ится отдельным изображением и рисуется       │
# │ ровно по ширине trim своей страницы. Никаких выносов за границы,  │
# │ никакой зависимости от поведения просмотрщика.                     │
# │                                                                    │
# │ Кэш: { full, left, right } — три embed'а на экспорт. Расход        │
# │ памяти терпимый, ускорения не дало (sharp быстр), но корректность │
# │ железобетонная.                                                    │
# │                                                                    │
# │ ╔══════════════════════════════════════════════════════════════╗   │
# │ ║ БЛОК B — AI-ПОМОЩНИК ДЛЯ ТЕКСТОВ РОДИТЕЛЕЙ                   ║   │
# │ ╚══════════════════════════════════════════════════════════════╝   │
# │                                                                    │
# │ Контекст: на родительской странице /[token] есть поле для текста   │
# │ ребёнка — где-то это свободный текст (free), где-то готовая цитата │
# │ из списка (grade11), где-то поля по шаблону (grade4, garden).      │
# │ Родители часто пишут с ошибками, не знают что писать, повторяют   │
# │ шаблонные фразы. AI-помощник на Claude Haiku 4.5 закрывает три    │
# │ сценария:                                                          │
# │   1) Исправить ошибки (action=fix) — мягкая правка пунктуации/     │
# │      орфографии, смысл и стиль не трогаем.                         │
# │   2) Улучшить текст (action=improve) — литературная редактура,     │
# │      сохраняем суть + укладываемся в лимит maxChars.                │
# │   3) Заполнить анкету (action=form_grade4/garden/grade11) —        │
# │      генерация связного текста из ответов на 4-5 вопросов.         │
# │                                                                    │
# │ Архитектурно:                                                      │
# │   • Серверный route /api/text-assist (один endpoint, action в      │
# │     теле запроса) дёргает @anthropic-ai/sdk → Haiku 4.5.            │
# │   • Промпты A/B/C из ТЗ зафиксированы в коде route'а.               │
# │   • temperature: fix=0.3 (минимальный креатив), improve=0.8        │
# │     (литературная свобода), form_*=0.8.                            │
# │   • maxChars берётся из настроек альбома (по умолчанию 300, для   │
# │     grade11 часто 150).                                            │
# │   • Авто-сокращение: если ответ > maxChars — повторный запрос с    │
# │     явным указанием лимита (до 2 попыток), затем hardTruncate +    │
# │     флаг truncated=true в ответе.                                  │
# │   • Feature flag на уровне альбома + лимит 10 успешных вызовов на  │
# │     ребёнка (защита от слива бюджета Anthropic).                   │
# │                                                                    │
# │ ═══════ feat 22bb550 — Этап 1: исправление ошибок под флагом ═════ │
# │                                                                    │
# │ • Миграция 2026-05-28-albums-text-assist-enabled.sql:              │
# │     albums.text_assist_enabled boolean NOT NULL DEFAULT false      │
# │   Включение точечно: UPDATE albums SET text_assist_enabled=true    │
# │   WHERE id='<album-id>'. Проверяется и на фронте (показ кнопки),  │
# │   и на сервере (403 если флаг false).                              │
# │                                                                    │
# │ • app/api/text-assist/route.ts (новый, ~340 строк):                │
# │     - POST { childId, action, text?, fields?, maxChars }           │
# │     - Auth: токен ребёнка из cookie/query (тот же что у /[token])  │
# │     - Загрузка album + проверка text_assist_enabled                │
# │     - action='fix': Промпт A (Claude Haiku 4.5, temp=0.3)          │
# │     - sanitizeText: вход ≤2000 символов, ответ ≤maxChars           │
# │     - Возвращает { ok: true, text: string, truncated?: bool }      │
# │                                                                    │
# │ • UI на /[token] (app/[token]/page.tsx):                           │
# │     - Кнопка «Исправить ошибки» под textarea (если text_type ∈    │
# │       {free, grade11} И text_assist_enabled=true)                  │
# │     - Состояние: idle / loading / result / error                  │
# │     - Блок результата: оригинал слева, AI-вариант справа,         │
# │       кнопки «Принять» / «Отмена»                                  │
# │     - При Принять → setStudentText + сброс блока                   │
# │                                                                    │
# │ • /api/child прокидывает text_assist_enabled во фронт (поле        │
# │   data.album.text_assist_enabled).                                 │
# │                                                                    │
# │ • @anthropic-ai/sdk добавлен в зависимости.                        │
# │                                                                    │
# │ ═══════ feat 4a09b1e — Этап 2: «Улучшить» + Доработать/Пересоздать │
# │                       + защита лимита ═══════                      │
# │                                                                    │
# │ • API: action='improve' (Промпт B; для maxChars≤150 — отдельный   │
# │   Промпт B-короткий с более жёсткими требованиями к компактности). │
# │ • temperature=0.8 для improve.                                     │
# │ • Авто-сокращение: до 2 попыток если ответ > maxChars, потом       │
# │   hardTruncate (по последнему пробелу/точке) + флаг truncated=true.│
# │ • UI: вторая кнопка «Улучшить» (фиолетовая) рядом с «Исправить».  │
# │ • Блок результата получил 4 кнопки: Принять / Доработать /         │
# │   Пересоздать / Отмена.                                            │
# │     - Доработать → focus на textarea (родитель сам правит)        │
# │     - Пересоздать → повтор запроса с тем же action                 │
# │ • Предупреждение в UI при truncated=true («Текст пришлось         │
# │   принудительно укоротить»).                                       │
# │                                                                    │
# │ ═══════ ux f035e32 — убрать кнопку «Доработать» ═══════            │
# │                                                                    │
# │ Кнопка создавала путаницу: родители не понимали разницу между      │
# │ «Доработать» (поправить руками) и «Пересоздать» (другой вариант    │
# │ от AI). В блоке результата осталось 3 кнопки: Принять /            │
# │ Пересоздать / Отмена.                                              │
# │                                                                    │
# │ ═══════ feat 5589e0d — Этап 3: анкеты grade4 + garden + fix/improve│
# │                       для всех text_type ═══════                   │
# │                                                                    │
# │ API:                                                               │
# │   • action='form_grade4' с полями hobby / profession / dream /     │
# │     superpower / wish.                                             │
# │   • action='form_garden' с полями game / food / profession / love. │
# │   • Промпты C из ТЗ (отдельный промпт под каждый формат).          │
# │   • Пустые поля анкеты НЕ попадают в промпт (родитель отвечает    │
# │     только на интересные ему вопросы).                             │
# │   • sanitizeFields: только строки, ≤300 символов на поле,         │
# │     остальное отбрасываем.                                         │
# │   • Снято серверное ограничение text_type для fix/improve — все   │
# │     четыре типа (free, grade11, grade4, garden) теперь могут      │
# │     пользоваться правкой ошибок и улучшением.                      │
# │                                                                    │
# │ UI:                                                                │
# │   • Переключатель «Написать самому / Заполнить анкету» для        │
# │     grade4/garden если text_assist_enabled=true.                   │
# │   • В режиме «Заполнить анкету» — поля под каждый вопрос +         │
# │     кнопка «Составить текст» (action=form_*).                      │
# │   • После «Принять» в форме → formMode=‘free’, родитель видит     │
# │     итоговый текст в стандартном textarea (может ещё подправить).  │
# │   • Кнопки fix/improve теперь видны для всех text_type.            │
# │   • Блок результата AI вынесен из условного блока режима           │
# │     (общий компонент).                                             │
# │                                                                    │
# │ ═══════ fix 0f3c5ba — качество анкетных промптов + UX анкеты ═════ │
# │                                                                    │
# │ Первая итерация анкет давала шаблонные / нелепые ответы            │
# │ («меня зовут Маша, мой любимый запах волшебный», прощания          │
# │ «До свидания, детский сад!» и т.п.). Усилил промпты:               │
# │                                                                    │
# │ Промпты form_grade4 и form_garden:                                 │
# │   • Жёсткий запрет на представления типа «Меня зовут…»,            │
# │     «Привет, я».                                                   │
# │   • Требование литературного русского без канцеляризмов.           │
# │   • Конкретные плохие примеры (волшебные запахи, собственное       │
# │     мороженое) — модель видит чего НЕ делать.                       │
# │   • Запрет ШАБЛОННЫХ прощаний.                                     │
# │   • Запрет пустых строк между абзацами (одиночный \\n — ок).        │
# │                                                                    │
# │ Серверная защита для form-actions:                                 │
# │   • Нормализация переносов: \\r\\n → \\n, схлопывание ДВОЙНЫХ \\n и  │
# │     табов (но одиночные \\n как абзацы сохраняются).                │
# │                                                                    │
# │ UX:                                                                │
# │   • Подзаголовок над переключателем для grade4/garden при          │
# │     включённом AI: «Можно написать самому или заполнить анкету —   │
# │     любой вариант».                                                │
# │   • Кнопка «Далее» в режиме «Заполнить анкету» disabled пока       │
# │     studentText пустой (родитель ОБЯЗАН сгенерировать текст        │
# │     прежде чем уйти со страницы).                                  │
# │                                                                    │
# │ ═══════ feat 414ff37 — Этап 3 продолжение: анкета grade11 ═════    │
# │                                                                    │
# │ API:                                                               │
# │   • action='form_grade11' с 5 полями: memory / hobby / future /    │
# │     gratitude / wish.                                              │
# │   • Промпт со взрослым тоном, запретом шаблонов («школьные годы   │
# │     пролетели» и т.п.).                                            │
# │   • Те же ужесточения что для grade4/garden: без «Меня зовут…»,    │
# │     без пустых строк между абзацами, литературный русский, не      │
# │     выдумывать факты которых нет в полях.                           │
# │   • Подсказка про маленький лимит (≤150 для grade11): взять       │
# │     главное, остальное отбросить.                                  │
# │                                                                    │
# │ UI:                                                                │
# │   • Переключатель «Написать самому / Заполнить анкету» расширен    │
# │     на grade11.                                                    │
# │   • Синий блок «Вопросы для вдохновения» для grade11 в режиме      │
# │     free (те же 5 вопросов что в анкете, но просто как подсказка   │
# │     родителю).                                                     │
# │   • Развёрнутый плейсхолдер в textarea для grade11.                │
# │   • Подзаголовок под title для grade11 при text_assist_enabled.    │
# │   • Кнопка «Составить текст» работает с form_grade11.              │
# │   • formMode→free при «Принять» из form_grade11.                   │
# │                                                                    │
# │ ═══════ ux 484e16b — grade11: скрыть готовые цитаты в анкете ═════ │
# │                                                                    │
# │ В режиме «Заполнить анкету» список готовых цитат (которые есть    │
# │ для grade11 как альтернатива свободному тексту) НЕ показывается:   │
# │ выбор готовой цитаты и заполнение анкеты концептуально             │
# │ конфликтуют (родитель должен решить ОДИН способ).                  │
# │                                                                    │
# │ Цитаты остаются в режиме «Написать самому» как раньше.             │
# │ Подзаголовок при включённом AI больше не упоминает цитаты:         │
# │ в анкете их нет, в free родитель видит блок ниже.                  │
# │                                                                    │
# │ ═══════ feat 4e631db — Этап 4: лимит 10 вызовов на ребёнка ═══════ │
# │                                                                    │
# │ Защита от слива бюджета: один родитель в теории может бесконечно   │
# │ нажимать «Пересоздать» и съесть весь токен-лимит Anthropic.        │
# │                                                                    │
# │ Миграция 2026-05-28-children-text-assist-count.sql:                │
# │   children.text_assist_count integer NOT NULL DEFAULT 0            │
# │                                                                    │
# │ API:                                                               │
# │   • /api/text-assist: перед вызовом Claude проверяет               │
# │     children.text_assist_count >= 10 → 429 (Too Many Requests)     │
# │     с понятным сообщением.                                         │
# │   • Инкремент text_assist_count ТОЛЬКО после успешного ответа     │
# │     Claude. Ошибки Anthropic (сеть, 5xx, rate-limit) не штрафуют   │
# │     родителя.                                                      │
# │   • В ответе ok добавлены поля used: number и limit: number — UI   │
# │     показывает счётчик без отдельного запроса.                     │
# │   • /api/child тоже возвращает text_assist_count для               │
# │     первоначальной отрисовки.                                      │
# │                                                                    │
# │ UI:                                                                │
# │   • Переменная aiCallsUsed читается из data.child.text_assist_     │
# │     count при загрузке страницы.                                   │
# │   • После каждого вызова обновляется из data.used (приходит в      │
# │     ответе route'а).                                               │
# │   • Под AI-кнопками подпись «Осталось N из 10 попыток».            │
# │   • При исчерпании (used >= 10) — серая плашка «Лимит              │
# │     AI-помощника исчерпан» вместо кнопок Исправить/Улучшить.       │
# │     Кнопка «Пересоздать» в блоке результата disabled.              │
# │                                                                    │
# │ Сброс лимита для ребёнка вручную:                                  │
# │   UPDATE children SET text_assist_count = 0 WHERE id = '...';      │
# │                                                                    │
# │ ═══════ feat 3aa3222 — «Ошибок не найдено» для action=fix ═════════│
# │                                                                    │
# │ Если для «Исправить ошибки» AI вернул текст идентичный оригиналу   │
# │ (после нормализации пробелов и переносов) — сервер ставит флаг     │
# │ noChanges=true и отдаёт во фронт.                                  │
# │                                                                    │
# │ На фронте — зелёная плашка «✓ Ошибок не найдено — текст уже        │
# │ грамотный» с одной кнопкой «Закрыть» вместо обычного блока        │
# │ Принять/Пересоздать/Отмена.                                        │
# │                                                                    │
# │ ВАЖНО: попытка ВСЁ РАВНО засчитывается в text_assist_count         │
# │ (вызов Claude был, токены потрачены).                              │
# │                                                                    │
# │ ═══════ ops 5c3d42f — AI включён по умолчанию для новых альбомов ══│
# │                                                                    │
# │ После того как фичу гоняли на тестовом альбоме «Тест 25»           │
# │ (id b3994ae3-7ee3-4823-8595-2ac815272ef3) и всё работало           │
# │ корректно — раскатили на дефолт.                                   │
# │                                                                    │
# │ Миграция 2026-05-28-albums-text-assist-enabled-default-true.sql:   │
# │   ALTER TABLE albums ALTER COLUMN text_assist_enabled              │
# │     SET DEFAULT true                                               │
# │                                                                    │
# │ Существующие 16 альбомов НЕ затронуты (SET DEFAULT влияет только   │
# │ на новые INSERT). Все будущие альбомы создаются с AI-помощником    │
# │ автоматически.                                                     │
# │                                                                    │
# │ Endpoint'ы create_album и album_clone в tenant API не передают    │
# │ text_assist_enabled явно — БД проставляет дефолт сама, кода       │
# │ менять не пришлось.                                                │
# │                                                                    │
# │ ╔══════════════════════════════════════════════════════════════╗   │
# │ ║ ДОКУМЕНТАЦИЯ И РИТУАЛ                                        ║   │
# │ ╚══════════════════════════════════════════════════════════════╝   │
# │                                                                    │
# │ ═══════ docs 0c40003 — CLAUDE.md в репозиторий ═══════             │
# │                                                                    │
# │ Компактная шпаргалка (~200 строк) для ежедневной работы Claude    │
# │ Code: стиль общения с Сергеем, стек, структура папок, команды,    │
# │ правила коммитов, текущее состояние v178, что не трогать,         │
# │ ключевые файлы и точки входа, терминология.                       │
# │                                                                    │
# │ История этапов остаётся в yearbook-context-v*.md — CLAUDE.md       │
# │ только выжимка для быстрого старта.                                │
# │                                                                    │
# │ ═══════ docs f66646f — одно подтверждение на commit+push ═════════ │
# │                                                                    │
# │ Раньше требовалось ДВА отдельных OK: на commit и на push. Сергей   │
# │ попросил поменять — он работает один, лишнее подтверждение         │
# │ тормозит. Теперь после «да» от Сергея — commit и push делаются    │
# │ сразу одной операцией. Объяснение простыми словами и риски —      │
# │ по-прежнему обязательны ПЕРЕД коммитом.                            │
# │                                                                    │
# │ Правка в двух местах CLAUDE.md (раздел «Как общаться» и раздел    │
# │ «Правила коммитов»). Memory-файл feedback_commit_push_ok_ritual    │
# │ обновлён в той же сессии.                                          │
# │                                                                    │
# │ ═══════ КОММИТЫ ЗА V179 (16 шт) ═══════                            │
# │                                                                    │
# │   Блок A — фоны:                                                   │
# │     3cf16cd — feat: API загрузки фонового изображения              │
# │     6c4aab5 — feat: UI загрузки фона в супер-админке               │
# │     239b75d — feat: рендер фона на канвасе редактора               │
# │     0f4705f — feat: фон в миниатюрах разворотов                    │
# │     e8e4db2 — feat: рендер фона в PDF-экспорте                     │
# │     34562d6 — fix: PDF-фон — половинки через sharp                 │
# │                                                                    │
# │   Блок B — AI-помощник:                                            │
# │     22bb550 — feat: Этап 1 (fix) под feature flag                  │
# │     4a09b1e — feat: Этап 2 (improve + Доработать/Пересоздать)      │
# │     f035e32 — ux: убрать кнопку «Доработать»                       │
# │     5589e0d — feat: Этап 3 (анкеты grade4/garden + всё text_type)  │
# │     0f3c5ba — fix: качество анкетных промптов + UX                 │
# │     414ff37 — feat: анкета grade11                                 │
# │     484e16b — ux: grade11 без готовых цитат в анкете               │
# │     4e631db — feat: Этап 4 (лимит 10 вызовов)                      │
# │     3aa3222 — feat: «Ошибок не найдено» для fix                    │
# │     5c3d42f — ops: AI по умолчанию для новых альбомов              │
# │                                                                    │
# │   Документация:                                                    │
# │     0c40003 — docs: CLAUDE.md в репо                               │
# │     f66646f — docs: одно подтверждение на commit+push              │
# │     (этот) — docs: context v179                                    │
# │                                                                    │
# │ ═══════ МИГРАЦИИ (применены Сергеем) ═══════                       │
# │                                                                    │
# │ ✅ migrations/2026-05-28-template-backgrounds-bucket.sql           │
# │    INSERT в storage.buckets (public, 50 MB, JPG/PNG).              │
# │                                                                    │
# │ ✅ migrations/2026-05-28-template-sets-default-background-url.sql  │
# │    template_sets +default_background_url text.                     │
# │                                                                    │
# │ ✅ migrations/2026-05-28-albums-text-assist-enabled.sql            │
# │    albums.text_assist_enabled boolean NOT NULL DEFAULT false.      │
# │                                                                    │
# │ ✅ migrations/2026-05-28-children-text-assist-count.sql            │
# │    children.text_assist_count integer NOT NULL DEFAULT 0.          │
# │                                                                    │
# │ ✅ migrations/2026-05-28-albums-text-assist-enabled-default-true.sql│
# │    ALTER COLUMN text_assist_enabled SET DEFAULT true.              │
# │                                                                    │
# │ Все пять — идемпотентны (IF NOT EXISTS / ON CONFLICT / ALTER       │
# │ DEFAULT). На существующие данные не повлияли.                      │
# │                                                                    │
# │ ═══════ СОСТОЯНИЕ ПРОЕКТА (на 29.05.2026) ═══════                  │
# │                                                                    │
# │   • Тесты: 758/758 passed (последний чистый запуск; при попытке   │
# │     в этой сессии vitest упал на запуске из-за конфликта с dev    │
# │     сервером на порту — это инфра, не регрессия).                 │
# │   • npx tsc --noEmit → пусто                                       │
# │   • npx next build → зелёный                                       │
# │                                                                    │
# │   • Фоны template_set работают: загрузка через супер-админку,     │
# │     рендер на канвасе + в миниатюрах + в PDF (через sharp split). │
# │   • AI-помощник работает на 4 типах текста (free / grade11 /       │
# │     grade4 / garden) с тремя сценариями (fix / improve / form).    │
# │     Включён по умолчанию для всех новых альбомов. Лимит 10        │
# │     вызовов на ребёнка.                                            │
# │   • На Claude Haiku 4.5 (@anthropic-ai/sdk).                       │
# │                                                                    │
# │   • БД: 1 template_set okeybook-default + поле background_url      │
# │     (пока без файла, ждём PNG от дизайнера — см. memory/           │
# │     project_background_designer_followup.md).                      │
# │   • 16 существующих альбомов, ~1 с включённым AI («Тест 25»)      │
# │     для отладки, остальные с дефолтом true для будущих заказов.   │
# │   • children.text_assist_count = 0 у всех (никто пока не дёргал). │
# │                                                                    │
# │ ═══════ ЧТО ДАЛЬШЕ ═══════                                         │
# │                                                                    │
# │ Активное (ждём входов от Сергея/дизайнера):                        │
# │                                                                    │
# │ 1. **PNG-фон от дизайнера** — Сергей обратился 28.05.2026,         │
# │    обещано «на днях». При получении: загрузить через супер-        │
# │    админку в okeybook-default, проверить рендер на канвасе и в    │
# │    PDF. Если рамки/слоты не совпадают с координатами мастеров —   │
# │    итеративно править IDML или фон.                                │
# │    (memory: project_background_designer_followup.md)               │
# │                                                                    │
# │ 2. **N-Grid-Page-9** — Сергей нарисовал в IDML, ждёт загрузки.    │
# │    После загрузки: family-mapping + предпочтение exact-match в    │
# │    findStudentGridMaster (~30-45 мин кода).                        │
# │                                                                    │
# │ 3. **5 J-Combined-Tail мастеров** (Tail-2 / Tail-2-Right /         │
# │    Tail-3 / Tail-3-Right / Tail-4-Right) — Сергей дорисует,        │
# │    family-mapping уже готов из РЭ.58, кода менять не нужно.        │
# │                                                                    │
# │ Backlog (большие задачи на будущее):                               │
# │                                                                    │
# │ 4. **J-Collage интеграция** (~3-4 часа) — после готовности этого  │
# │    кода Сергей нарисует J-Collage-3/5/7/8. См. v178 раздел        │
# │    «Задачи для кода».                                              │
# │                                                                    │
# │ 5. **PDF-export каскада стилей/шрифтов** — pipeline пока НЕ        │
# │    применяет ни одного override (size/color/halign/valign/font).   │
# │    Отложено до серьёзной работы с PDF.                             │
# │                                                                    │
# │ 6. **Адаптация под форматы типографий** (гибридная D+E стратегия,  │
# │    см. v178). Делать когда появится вторая типография.             │
# │                                                                    │
# │ 7. **Backlog по фонам** (см. memory/project_background_system.md): │
# │    • Несколько фонов на template_set (разные типы разворотов)      │
# │    • Программируемые рамки/слоты поверх фона                       │
# │    • softShift и точное позиционирование под bleed                 │
# │    • Bleed-зона в PDF (сейчас фон ложится в trim)                  │
# │                                                                    │
# │ 8. **AI-помощник — расширения** (если потребуется по фидбэку):     │
# │    • Лимит на альбом (сейчас только на ребёнка)                    │
# │    • Логирование вызовов для аналитики                             │
# │    • Анкета для других text_type если появятся                     │
# │    • Подключение более крупной модели для improve если Haiku      │
# │      окажется слабым (сейчас качества пока хватает)                │
# │                                                                    │
# │ 9. **Миграция на российские серверы** (memory/project_migration_   │
# │    to_ru.md) — план готов, делать когда придёт время.              │
# │                                                                    │
# │ ═══════ КРИТИЧЕСКИЕ ЗАМЕТКИ ═══════                                │
# │                                                                    │
# │ 1. AI-помощник стоит денег за каждый вызов Claude Haiku 4.5.       │
# │    Лимит 10 на ребёнка — мягкая защита. Если бюджет начнёт         │
# │    утекать — добавить лимит на альбом + дашборд в супер-админке.   │
# │                                                                    │
# │ 2. text_assist_enabled теперь true по умолчанию для НОВЫХ          │
# │    альбомов. Старые 16 остались как были. Если нужно отключить    │
# │    для конкретного альбома — UPDATE albums SET                    │
# │    text_assist_enabled=false WHERE id='...'.                       │
# │                                                                    │
# │ 3. Фон в PDF режется через sharp.extract — нельзя полагаться на   │
# │    обрезку pdf-просмотрщиком по mediaBox (баг 34562d6). Это        │
# │    важно помнить при любых будущих правках drawBackground.         │
# │                                                                    │
# │ 4. Bucket template-backgrounds — PUBLIC. Любой по прямой ссылке   │
# │    может скачать фон. Если потребуется приват — менять и bucket   │
# │    policy, и логику сборки URL в TemplateBackgroundPanel.          │
# │                                                                    │
# │ 5. Промпты AI-помощника содержат конкретные запреты на шаблоны    │
# │    («волшебные запахи», «До свидания, детский сад», «Меня         │
# │    зовут…»). Если модель снова начнёт их использовать — усилить   │
# │    промпт, не правя UI.                                            │
# │                                                                    │
# │ 6. Авто-сокращение до maxChars: до 2 retry, потом hardTruncate +   │
# │    флаг truncated. UI показывает предупреждение. Если родителям   │
# │    эти сокращения не нравятся — увеличить maxChars в настройках    │
# │    альбома.                                                        │
# │                                                                    │
# └────────────────────────────────────────────────────────────────────┘
#
