...
# Версия: v99
# Обновлено: 20.05.2026 (после РЭ.22.7.1 — data-миграция teachers-мастеров).
#
# ФАЗА РЭ.22 — Конструктор пресетов с двух-осевой моделью (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.22.0..РЭ.22.6 — students секция полностью на семантическом поиске
#    (8 коммитов, см. v98 для деталей).
# ✅ РЭ.22.7.1 (fea1a75) — data-миграция teachers-мастеров.
#    migrations/2026-05-20-okeybook-teacher-master-tags.sql
#    UPDATE для F-Head-LargeGrid/SmallGrid/WithPhoto/WithClassPhoto-L
#    → page_role='teacher_left', и G-FullClass/HalfClass/Teachers-3x3/3x4/4x4
#    → page_role='teacher_right'. slot_capacity вычисляется динамически
#    через jsonb_array_elements (head_teacher, teachers, photos_full,
#    photos_half).
#    ⚠️ Вскрыт скрытый баг #3: код ищет 'G-Teachers-4x3', в БД 'G-Teachers-3x4'.
#    После РЭ.22.7.2 баг исчезнет (семантический поиск по slot_capacity).
#    Дополнительно: F-Head-WithClassPhoto-L — мастер которого код не знал.
#    Будет доступен engine'у после семантического перехода.
#    ⬜ Применить в Supabase SQL Editor.
#
# ⏳ Следующий подэтап: РЭ.22.7.2 — engine teachers семантический.
#    - Новая findTeacherMaster в master-finder.ts (фильтры:
#      page_role + slot_capacity head_teacher / teachers / photos_full / photos_half).
#    - Переписать fillTeachersSection (lib/rule-engine/sections/teachers.ts)
#      через семантический поиск. Сохранить таблицу выбора мастера
#      (subjects 0/1-4/5-8/9/10-12/13-16/17+), но имена → семантика.
#    - Fallback на legacy slot-chains когда мастера не размечены (для
#      обратной совместимости).
#    - Тесты для нового пути + закрытие бага G-Teachers-3x4.
#
...
#      ⚠️ Вскрыт скрытый баг #3 фазы: код ищет 'G-Teachers-4x3', в БД
#         мастер 'G-Teachers-3x4'. После РЭ.22.7.2 баг исчезнет.
#      Бонус: F-Head-WithClassPhoto-L (head=1+classphoto=1, одна страница)
#         — мастер которого код не знал, доступен engine'у после семантики.
#      ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.22.7.2.
#
#   ⏳ Следующий подэтап: РЭ.22.7.2 — engine teachers семантический
#      Новая findTeacherMaster в master-finder.ts. Переписать
#      fillTeachersSection через семантический поиск. Сохранить таблицу
#      выбора по числу subjects (0/1-4/5-8/9/10-12/13-16/17+), но имена
#      → семантика. Тесты.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v99
#   3. ОБЯЗАТЕЛЬНО прочитать в этом порядке:
#      • Шапка v99 — что закрыто (включая РЭ.22.7.1)
#      • docs/phase-Р22-spec.md v1.0 — §6.5 (teachers семантика).
#      • migrations/2026-05-20-okeybook-teacher-master-tags.sql — какие
#        теги теперь у teachers-мастеров.
#      • lib/rule-engine/sections/teachers.ts — таблица pickLeftMaster /
#        pickRightMaster (subjects 0/1-4/5-8/9/10-12/13-16/17+).
#        Алгоритм правильный, но имена жёсткие. Цель РЭ.22.7.2 — те же
#        алгоритмы, но имена через семантический поиск.
#      • lib/rule-engine/master-finder.ts — образец для findTeacherMaster.
...
#      • 2026-05-20-okeybook-grid-master-tags.sql ✅ ПРИМЕНЕНА (РЭ.22.6.0)
#      • 2026-05-20-okeybook-teacher-master-tags.sql ⬜ РЭ.22.7.1 —
#        ПРИМЕНИТЬ ДО деплоя РЭ.22.7.2.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.22.7.2 — engine teachers семантический.
#      Файлы:
#        - lib/rule-engine/master-finder.ts: новая findTeacherMaster.
#          Параметры: pageRole ('teacher_left' / 'teacher_right'),
#          teachersCount (для G-Teachers-*), headTeacher (true для F-*),
#          photosFull / photosHalf (0/1/2 для G-* и F-Head-WithClassPhoto-L).
#          Алгоритм: точное совпадение по teachers, либо min_fit.
#        - lib/rule-engine/sections/teachers.ts:
#          Переписать pickLeftMaster и pickRightMaster через
#          findTeacherMaster. Сохранить таблицу subjects → нужная
#          конфигурация мастера, но имена меняются на запросы по
#          slot_capacity. Например: subjects=9 → запрос
#          {pageRole: 'teacher_right', teachersCount: 9, match: 'exact'}.
#          Закрытие бага G-Teachers-3x4 = автоматически работает,
#          потому что engine ищет teachers=12 не имя.
#        - lib/rule-engine/__tests__/sections-teachers-semantic.test.ts
#          (новый): тесты на subjects 0/3/5/9/10/14/20, master_not_found,
#          закрытие бага 10-12 предметников.
#      Что НЕ трогаем: students секция (закрыты РЭ.22.6),
#                      soft_intro/soft_final (РЭ.22.8).
