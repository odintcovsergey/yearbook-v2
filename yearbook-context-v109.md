...
# Версия: v109
# Обновлено: 20.05.2026 (после РЭ.24.0+24.1 — старт фазы готовых шаблонов).
#
# ФАЗА РЭ.24 — Готовые шаблоны для быстрого старта партнёра (В РАБОТЕ)
# ──────────────────────────────────────────────────────────────────
# ✅ РЭ.24.0 (614e6a7) — docs/phase-Р24-spec.md v1.0 (227 строк).
#    Все 9 архитектурных развилок (A-I) зафиксированы:
#      A: is_recommended bool, B: 1+3 превью, C: клонирование,
#      D: /app/templates, E: строгая валидация, F: одна кнопка,
#      G: блокировка удаления при активных альбомах,
#      H: 'Мои' + 'Готовые' в одной модалке,
#      I: dry-run серверный, клиент кэширует.
# ✅ РЭ.24.1 (037f9ff) — миграция БД.
...
#    Аддитивная, идемпотентная, не блокирует таблицу.
#    ⬜ Применить в Supabase SQL Editor ДО деплоя РЭ.24.4.
#
# ⏳ Следующий подэтап: РЭ.24.2 — утилита validatePreset.
#    lib/presets/validate.ts: чистая функция
#      validatePreset(preset: Preset) → { valid: boolean, errors: string[] }
#    Проверяет:
#      1. display_name непустое
#      2. print_type ∈ {'layflat', 'soft'}
#      3. template_set_id не NULL
#      4. section_structure — массив длиной ≥ 1
#      5. Для секций students — student_layout_mode не NULL
#      6. Если mode='grid' — student_grid_size в 2..12
#    Тесты (~8-10): валидный, каждая невалидная ситуация по отдельности.
#    Используется на сервере (фильтрация в API) и в UI (флаг 'Доработай').
#
# Архитектура РЭ.24 — 4 уровня сущностей:
#   1. ГЛОБАЛЬНЫЕ — presets WHERE tenant_id IS NULL (Сергея)
#   2. КАТАЛОГ /app/templates — витрина (новая страница)
#   3. ЛИЧНАЯ БИБЛИОТЕКА — presets WHERE tenant_id = X
#   4. ВЫБОР ПРИ АЛЬБОМЕ — модалка (правка существующей)
#
# Подэтапы фазы (8 шт):
#   ✅ 24.0 — spec
#   ✅ 24.1 — миграция (is_recommended + parent_preset_id)
#   ⏳ 24.2 — validatePreset + тесты
#      24.3 — buildPresetPreviewBundle (4 SVG через dry-run engine'а)
#      24.4 — API: templates_list_global/_my/_clone/_create_blank/_delete
#      24.5 — UI: страница /app/templates
#      24.6 — UI: модалка выбора пресета при создании альбома
#      24.7 — UI: галка is_recommended в /super/presets
#      24.8 — summary + контекст
#
# 🎉 ФАЗА РЭ.23 ЗАКРЫТА — визуальный каталог мастеров.
# Summary: docs/phase-Р23-summary.md (116 строк). 11 коммитов на main.
# Готов renderPreviewSvg + API + страница /super/master-catalog.
# Используем эту инфраструктуру в РЭ.24.3 для превью карточек.
#
...
#   2. ls yearbook-context-v*.md | sort -V | tail -1  → должен показать v109
#   3. ОБЯЗАТЕЛЬНО прочитать:
#      • Шапка v109 — что закрыто (РЭ.24.0+24.1)
#      • docs/phase-Р24-spec.md v1.0 — полный план фазы (227 строк)
#      • docs/phase-Р22-summary.md + Р23-summary.md — предыдущие фазы
#   4. Применены в Supabase:
#      • Все миграции до v87 ✅
#      • 2026-05-19-presets-student-layout-fields.sql ✅
#      • 2026-05-19-okeybook-default-student-master-tags.sql ✅
#      • 2026-05-20-albums-drop-rules-preset-id.sql ⬜ (не блокер)
...
#      • 2026-05-20-presets-recommended-and-parent.sql ⬜ РЭ.24.1 —
#        ПРИМЕНИТЬ ДО деплоя РЭ.24.4.
#   5. СЛЕДУЮЩИЙ ПОДЭТАП: РЭ.24.2 — утилита validatePreset.
#      Файл: lib/presets/validate.ts (новый)
#      Сигнатура: validatePreset(preset: Preset) → { valid: boolean,
#                                                   errors: string[] }
#      Правила валидности (см. spec §3):
#        1. display_name непустое (после .trim())
#        2. print_type ∈ {'layflat', 'soft'}
#        3. template_set_id не NULL и не пустая строка
#        4. section_structure — массив длиной ≥ 1
#        5. Для секций {type: 'students'} в section_structure —
#           preset.student_layout_mode не NULL
#        6. Если student_layout_mode === 'grid' — preset.student_grid_size
#           не NULL и в диапазоне 2..12
#      errors — массив человеко-читаемых строк ('Название не может быть
#      пустым', 'Не выбран набор шаблонов вёрстки', и т.д.)
#      Тесты: lib/presets/__tests__/validate.test.ts (~8-10 тестов):
#      валидный, каждая невалидная ситуация отдельно, граничные случаи
#      (student_grid_size=1, =13, NULL для grid mode).
#      Что НЕ трогаем: API, UI — это РЭ.24.4+.
